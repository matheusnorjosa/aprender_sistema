...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f"Erro ao analisar planilhas: {str(e)}"))
    ...
    def _analyze_spreadsheets(self, year: int) -> List[Dict[str, Any]]:
        ...
        if not os.path.exists(spreadsheets_dir):
            self.stdout.write(
                self.style.WARNING("Diretório archive/spreadsheets não encontrado")
            )
        ...
        for filename in os.listdir(spreadsheets_dir):
            if filename.endswith(".xlsx"):
                ...
                try:
                    planilha_info = self._analyze_single_spreadsheet(
                        file_path, filename, year
                    )
                    if planilha_info:
    ...
    def _analyze_single_spreadsheet(
        self, file_path: str, filename: str, year: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for sheet_name in excel_file.sheet_names[
                :5
            ]:  # Limitar a 5 abas por performance
                try:
                    ...
                    for col in df.columns:
                        try:
                            ...
                            if len(valid_dates) > 0:
                                ...
                                if len(dates_2025) > 0:
                                    sheet_analysis["year_2025_references"] += len(
                                        dates_2025
                                    )
                    ...
                    for municipio in municipios_ce:
                        if any(
                            municipio.lower() in str(cell).lower()
                    ...
                    self.stderr.write(
                        self.style.WARNING(
                            f"Erro na aba {sheet_name} de {filename}: {str(e)}"
                        )
                    )
    ...
    def _generate_spreadsheet_statistics(
        self, planilhas_data: List[Dict[str, Any]], year: int
    ) -> Dict[str, Any]:
        ...
        planilhas_por_eventos = sorted(
            planilhas_data, key=lambda p: p.get("events_2025", 0), reverse=True
        )
        ...
        return {
            ...
            "file_size_distribution": {
                "smallest_mb": (
                    round(min(file_sizes) / (1024 * 1024), 2) if file_sizes else 0
                ),
                "largest_mb": (
                    round(max(file_sizes) / (1024 * 1024), 2) if file_sizes else 0
                ),
                "average_mb": (
                    round(sum(file_sizes) / len(file_sizes) / (1024 * 1024), 2)
                    if file_sizes
                    else 0
                ),
            },
    ...
    def _display_summary(self, result: Dict[str, Any]):
        ...
        self.stdout.write(
            f'Total de planilhas: {self.style.SUCCESS(str(summary.get("total_planilhas", 0)))}'
        )
        self.stdout.write(f'Total de abas: {summary.get("total_sheets", 0)}')
        self.stdout.write(
            f'Eventos de 2025 encontrados: {summary.get("total_events_2025", 0)}'
        )
        ...
        if unique_formadores > 0:
            self.stdout.write(
                f"Possíveis formadores identificados: {unique_formadores}"
            )
