...
from core.models import Formador, Solicitacao, Projeto, Municipio, TipoEvento, SolicitacaoStatus
...
def test_admin_formador_view():
    ...
    try:
        ...
        print(f"   ✅ Usuário encontrado: {user_admin.username} (superuser: {user_admin.is_superuser})")
    except User.DoesNotExist:
        print("   ❌ Usuário matheusadm não encontrado")
        return False
    
    # Login como matheusadm
    login_success = client.login(username='matheusadm', password='admin123')  # Assumindo senha padrão
    ...
    if response.status_code == 200:
        ...
        print(f"   📋 Formadores disponíveis: {len(context.get('formadores_disponiveis', []))}")
    ...
    if formador_teste:
        ...
        response = client.get(f'/formador/eventos/?admin_mode=formador&admin_formador_id={formador_teste.id}')
    ...
    try:
        ...
        print(f"   ✅ Usuário encontrado: {user_coord.username} (superuser: {user_coord.is_superuser})")
        ...
        if response.status_code == 200:
            ...
            if not admin_mode and erro:
                print("   ✅ Comportamento correto: usuário normal vê erro sem modo admin")
            else:
