...
| **RF02** Solicitação | Criação de pré-agenda, decide status inicial por `projeto.fluxo` | [`services/solicitacao_create.py`](../../../backend/apps/core/services/solicitacao_create.py) (`resolve_initial_status`); `SolicitacaoViewSet` em [`views_solicitacao.py`](../../../backend/apps/core/views_solicitacao.py) (`perform_create`) |
...
- **RF04 / PA-01..PA-07 (CP-02)**: SUPER **nunca** auto-aprova; aprovar/reprovar exige autorização (`CanAccessSolicitationApprovals`); lote limitado a 100 ids, com `select_for_update(skip_locked=True)` para evitar dupla aprovação; `approve` e `batch_approve` **revalidam disponibilidade** (#1452). Idioma RBAC: `permission_classes=[HasPerm("codename")]` (grupos diretos banidos por `scripts/rbac_lint.py`). As divergências antes listadas aqui foram **fechadas e corrigidas**: `M03-01` (#1610 — gate superuser + allowlist no import de grupos), `M10-02` (#1624 — troca de projeto p/ SUPER rebaixa e re-exige aprovação) e `M11-04` (#1650 — `ids` de lote validados como lista de inteiros). Detalhe e residuais (ex.: escopo ator×alvo abrangente em #1656) em [`politica-aprovacao.spec.md`](./politica-aprovacao.spec.md) §Divergências.
...
2. **Solicitação → publicação (RF02→RF05/RF06)**: coordenador cria solicitação → sistema checa conflitos de todos os participantes e **bloqueia** se houver (RF03, #1452) → `NAO_SUPER` auto-aprova; `SUPER` fica `pendente` → Gerente da Superintendência ou Assistente Administrativo do Controle aprova (RF04, revalida RF03 e grava AuditLog) → Controle/Super faz `preview-gcal` (não persiste) → `publish` enfileira Celery → evento criado no GCal; se `is_online=True`, `meet_link` extraído de `hangoutLink` e persistido (RF06). **DAT não aprova** — o composite de PA-02 o exclui (`_user_has_solicitation_approvals` em [`policies.py`](../../../backend/apps/core/rbac/policies.py)).
