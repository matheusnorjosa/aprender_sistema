...
| D6 | Policy ator×alvo em participantes | **Feito (#2033, 2026-09-15):** `participants_out_of_setor` barra formador/coord de outro **setor** no `_create_participants`, `_update_formadores` e no `/lookup/usuarios/`. ⚑A regra medida mudou o default: ancora no **setor do CRIADOR** (`Gerencia.setor_canonico` via `EquipeGerencia`), **não** na "gerência do projeto" — a medição achou `Projeto.gerencia` VAZIO em prod. Superintendência/privilegiado isento; criador sem setor = fail-open; guests por e-mail seguem externos. `M10-04` resolvido; escopo de **projeto** e `can_act_on` central seguem no épico **#1656**; reconciliação-SSOT de participantes no **#1666**. |
...
- [x] **3.2 Update** — #2033 aplicou o mesmo helper de escopo (`participants_out_of_setor`, um service) nos dois call-sites (create e update) + no lookup. (A reconciliação-SSOT de convidados/COORD_ACOMPANHA segue no #1666.)
