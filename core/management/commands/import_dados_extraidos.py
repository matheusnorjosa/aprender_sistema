...
class Command(BaseCommand):
        ...
        for nome, is_online in tipos_evento:
            if not self.dry_run:
                tipo, created = TipoEvento.objects.get_or_create(
                    nome=nome, defaults={"ativo": True, "online": is_online}
                )
                if created:
    ...
    def importar_formadores_exemplo(self):
        ...
        for nome_formador in formadores_identificados:
            try:
                ...
                if (
                    not self.dry_run
                    and Usuario.objects.select_related("municipio", "projeto")
                ...
                if not self.dry_run:
                    ...
                    formador = Formador.objects.create(
                        nome=nome_formador, email=email, usuario=usuario, ativo=True
                    )
        ...
        for nome_projeto, nome_setor in projetos_identificados:
            try:
                if not self.dry_run:
                    ...
                    if created:
                        self.logger.debug(
                            f"📋 Projeto criado: {nome_projeto} (Setor: {nome_setor})"
                        )
                        projetos_criados += 1
                else:
                    projetos_criados += 1
    ...
    def criar_solicitacoes_exemplo(self):
        ...
        if self.dry_run:
            self.logger.info(
                "🔍 Modo dry-run: 10 solicitações de exemplo seriam criadas"
            )
            return 10
        ...
        try:
            ...
            formadores = list(
                FormadorService.get_formadores_queryset()[:5]
            )  # Primeiros 5 formadores
