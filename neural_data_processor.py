...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("neural_processing.log"), logging.StreamHandler()],
)
...
class NeuralDataProcessor:
        ...
        default_config = {
            ...
            "organizational_structure": {
                "categorias": {
                    ...
                    "acerta": {"nome": "ACerta", "cor": "#ff7f0e", "projetos": []},
                    "brincando": {
                        "nome": "Brincando",
                        "cor": "#2ca02c",
                        "projetos": [],
        ...
        for tab_data in sheet_data.get("abas", []):
            ...
            try:
                ...
                if "usuarios" in sheet_name.lower() or "usuários" in sheet_name.lower():
                    ...
                    "acompanhamento" in sheet_name.lower()
                    or "agenda" in sheet_name.lower()
                ):
                    self._process_agenda_tab(tab_name, tab_data, processed_data)
                elif "disponibilidade" in sheet_name.lower():
                    self._process_disponibilidade_tab(
                        tab_name, tab_data, processed_data
                    )

                self.processing_stats["records_processed"] += len(
                    tab_data.get("records", [])
                )
    ...
    def _process_usuarios_tab(
        self, tab_name: str, tab_data: Dict[str, Any], processed_data: Dict[str, Any]
    ) -> None:
        """
    ...
    def _process_disponibilidade_tab(
        self, tab_name: str, tab_data: Dict[str, Any], processed_data: Dict[str, Any]
    ) -> None:
        """
        ...
        if "eventos" in tab_name.lower():
            self._process_eventos_disponibilidade_tab(
                tab_name, tab_data, processed_data
            )
    ...
    def _extract_evento_data(
        self, record: Dict[str, str], categoria: str
    ...
    def _extract_atribuicoes_from_evento(
        self, record: Dict[str, str], categoria: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _determine_papel(self, cargo: str, gerencia: str) -> str:
        ...
        if "coordenador" in cargo_lower:
            ...
            "superintendência" in gerencia_lower or "superintendencia" in gerencia_lower
        ):
            return "superintendencia"
    ...
    def _generate_pessoa_id(self, pessoa_data: Dict[str, Any]) -> str:
        ...
        if pessoa_data.get("cpf"):
            ...
            nome_hash = hashlib.md5(pessoa_data["nome_completo"].encode()).hexdigest()[
                :8
            ]
    ...
    def save_processed_data(self, data: Dict[str, Any], output_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"📊 Estatísticas: {self.processing_stats['files_processed']} arquivos, "
