...
class Command(BaseCommand):
    ...
    def processar_e_mostrar_dados(self):
        ...
        for i, linha in enumerate(dados_brutos, 1):
            ...
            dados_tratado = {
                "linha_original": i,
                "data_evento": (
                    data_evento.strftime("%Y-%m-%d") if data_evento else "INVÁLIDA"
                ),
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            with open(
                "dados_planilhas_originais/previa_aba_super.json", "w", encoding="utf-8"
            ) as f:
                json.dump(dados_tratados, f, indent=2, ensure_ascii=False, default=str)

            self.logger.info(
                f"Prévia salva em: dados_planilhas_originais/previa_aba_super.json"
            )
