            ...
            resultado["problemas_detectados"].append(
                {
                    ...
                    "exemplos": list(
                        projetos_contaminados.values_list("nome", flat=True)[:3]
                    ),
                }
            )
            resultado["dados_limpos"] = False
...
class ProjetoService(BaseService):
    ...
    def para_formulario(cls) -> QuerySet:
        ...
        return (
            cls.ativos()
            .exclude(nome__icontains="exemplo")
            .exclude(nome__icontains="teste")
            .exclude(nome__icontains="projeto formacao")
            .exclude(nome__icontains="biblioteca digital")
        )
...
class MunicipioServiceCentralizado(BaseService):
    ...
    def originais_planilhas(cls) -> QuerySet:
        ...
        return cls.get_base_queryset().filter(
            Q(nome__in=municipios_originais) | Q(nome__startswith="Amigos do Bem")
        )
...
class TipoEventoService(BaseService):
    ...
    def originais_planilhas(cls) -> QuerySet:
        ...
        tipos_originais = ["1 - Presencial", "2 - Online", "Presencial", "Online"]
