...
class Command(BaseCommand):
    ...
    def consolidate_formador_model(self, dry_run=False, force=False):
        ...
        usuarios_formadores_count = Usuario.objects.select_related("municipio", "projeto").prefetch_related("groups", "user_permissions").filter(formador_ativo=True).count()
        ...
        if formadores_count == 0:
            print("✅ Nenhum formador no modelo Formador encontrado - consolidação desnecessária")
            return
    ...
    def migrate_formador_data(self):
        ...
        for formador in FormadorService.get_formadores_queryset():
            ...
            existing_user = Usuario.objects.select_related("municipio", "projeto").prefetch_related("groups", "user_permissions").filter(email=formador.email).first()
            ...
            if existing_user:
                ...
                # Criar novo usuário
                username = formador.email.split('@')[0]  # Usar email como base do username
        ...
        for deslocamento in Deslocamento.objects.all():
            ...
            if deslocamento.pessoa_1:
                usuario = Usuario.objects.select_related("municipio", "projeto").prefetch_related("groups", "user_permissions").filter(
                    email=deslocamento.pessoa_1.email,
                    formador_ativo=True
                ).first()
                if usuario:
    ...
    def simulate_migration(self):
        ...
        for formador in FormadorService.get_formadores_queryset()[:5]:  # Mostrar apenas 5 exemplos
            existing_user = Usuario.objects.select_related("municipio", "projeto").prefetch_related("groups", "user_permissions").filter(email=formador.email).first()
