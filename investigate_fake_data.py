...
class FakeDataInvestigator:
    ...
    def investigate_usuarios(self):
        ...
        try:
            ...
            fake_patterns = [
                'admin', 'administrador', 'teste', 'test', 'exemplo', 'example',
                'demo', 'user', 'usuario', 'fake', 'dummy', 'sample'
            ]
            ...
            for usuario in usuarios:
                ...
                for pattern in fake_patterns:
                    if pattern.lower() in nome_completo.lower() or pattern.lower() in email.lower():
                        is_fake = True
    ...
    def investigate_setores(self):
        ...
        try:
            ...
            fake_patterns = [
                'exemplo', 'example', 'teste', 'test', 'demo', 'fake',
                'dummy', 'sample', 'não definido', 'outros'
            ]
    ...
    def investigate_solicitacoes(self):
        ...
        try:
            ...
            fake_patterns = [
                'exemplo', 'example', 'teste', 'test', 'demo', 'fake',
                'dummy', 'sample', 'solicitação', 'evento'
            ]
            ...
            for solicitacao in solicitacoes:
                ...
                # Verificar padrões de dados falsos
                for pattern in fake_patterns:
                    if pattern.lower() in solicitacao.observacoes.lower() if solicitacao.observacoes else False:
                        is_fake = True
                ...
                if is_fake:
                    self.fake_data_found['solicitacoes'].append({
                        'id': solicitacao.id,
                        'projeto': solicitacao.projeto.nome if solicitacao.projeto else None,
                        'municipio': solicitacao.municipio.nome if solicitacao.municipio else None,
                        'reason': fake_reason
                    })
            ...
            print(f"  📊 Solicitações falsas: {len(self.fake_data_found['solicitacoes'])}")
    ...
    def print_investigation_report(self):
        ...
        print(f"  • Percentual de dados reais: {(total_real/(total_real+total_fake)*100):.1f}%")
        ...
        for entity in entities:
            ...
            if total_count > 0:
                ...
                print(f"  {status} {entity.title()}: {real_count}/{total_count} reais ({percentage:.1f}%)")
        ...
        if total_fake > 0:
            ...
            for entity, items in self.fake_data_found.items():
                if items:
                    ...
                    for item in items:
                        print(f"    • {item.get('nome', item.get('id', 'N/A'))} - {item.get('reason', 'N/A')}")
        else:
        ...
        if total_fake == 0:
            ...
            print("⚠️ Sistema contém principalmente dados reais, mas há alguns dados falsos")
        else:
