...
class Command(BaseCommand):
    ...
    def _validar_solicitacoes(self, resultado):
        ...
        com_admin = (
            ...
            .prefetch_related("formadores")
            .filter(usuario_solicitante__username="admin")
            .count()
        )

        # Por projeto
        por_projeto = dict(
            Solicitacao.objects.values("projeto__nome")
        ...
        coordenadores_query = (
            Solicitacao.objects.exclude(usuario_solicitante__username="admin")
        ...
        resultado["solicitacoes"] = {
            ...
            "percentual_aprovadas": (
                round((aprovadas / total) * 100, 1) if total > 0 else 0
            ),
            "percentual_pendentes": (
                round((pendentes / total) * 100, 1) if total > 0 else 0
            ),
        }
    ...
    def _validar_usuarios(self, resultado):
        ...
        coordenadores = (
            ...
            .filter(cargo="coordenador", is_active=True)
            .count()
        )
        formadores_ativos = (
            Usuario.objects.select_related("municipio", "projeto")
            .prefetch_related("groups", "user_permissions")
            .filter(formador_ativo=True, is_active=True)
            .count()
        )

        # Usuários com eventos
        usuarios_com_eventos = (
            Usuario.objects.select_related("municipio", "projeto")
            .prefetch_related("groups", "user_permissions")
            .filter(solicitacoes_criadas__isnull=False)
            .distinct()
            .count()
        )
    ...
    def _validar_formadores(self, resultado):
        ...
        solicitacoes_com_formador = (
            Solicitacao.objects.select_related(
                "municipio", "projeto", "tipo_evento", "solicitante"
            )
            .prefetch_related("formadores")
            .filter(formadoressolicitacao__isnull=False)
            .distinct()
            .count()
        )

        total_solicitacoes = Solicitacao.objects.count()
        percentual_com_formador = (
            round((solicitacoes_com_formador / total_solicitacoes) * 100, 1)
        ...
        formadores_query = (
            FormadoresSolicitacao.objects.values(
    ...
    def _comparar_com_planilhas(self, resultado):
        ...
        try:
            with open(
                "dados/extraidos/dados_completos_tratados_20250919_153743.json",
                "r",
                encoding="utf-8",
            ) as f:
            ...
            aprovados_planilhas = stats_planilhas.get("por_status", {}).get(
                "APROVADO", 0
            )
            pendentes_planilhas = stats_planilhas.get("por_status", {}).get(
                "PENDENTE", 0
            )
            ...
            resultado["comparacao_planilhas"] = {
                ...
                "taxa_importacao": (
                    round((total_sistema / total_planilhas) * 100, 1)
                    if total_planilhas > 0
                    else 0
                ),
    ...
    def _calcular_qualidade(self, resultado):
        ...
        score_coordenadores = (
            100 - ((sol["com_admin"] / sol["total"]) * 100) if sol["total"] > 0 else 0
        )
        score_formadores = form["percentual_com_formador"]
        score_estrutura = (
            100 if resultado["estrutura"]["projetos_com_eventos"] > 0 else 0
        )
    ...
    def _identificar_problemas(self, resultado):
        ...
        if resultado["solicitacoes"]["com_admin"] > 0:
            problemas.append(
                f"{resultado['solicitacoes']['com_admin']} eventos ainda com usuário admin"
            )

        taxa_importacao = resultado.get("comparacao_planilhas", {}).get(
            "taxa_importacao", 0
        )
        if taxa_importacao < 100:
            eventos_perdidos = resultado.get("comparacao_planilhas", {}).get(
                "eventos_perdidos", 0
            )
            problemas.append(
                f"{eventos_perdidos} eventos das planilhas não foram importados"
            )

        if resultado["formadores"]["percentual_com_formador"] < 80:
            problemas.append(
                f"Apenas {resultado['formadores']['percentual_com_formador']}% dos eventos têm formador associado"
            )

        # Recomendações
        if resultado["solicitacoes"]["com_admin"] > 0:
            recomendacoes.append(
                "Executar novamente o comando de correção de coordenadores"
            )
    ...
    def _exibir_relatorio(self, resultado):
        ...
        self.stdout.write(
            f"   Aprovadas: {sol['aprovadas']} ({sol['percentual_aprovadas']}%)"
        )
        self.stdout.write(
            f"   Pendentes: {sol['pendentes']} ({sol['percentual_pendentes']}%)"
        )
        ...
        self.stdout.write(f"\n🏗️ ESTRUTURA:")
        self.stdout.write(
            f"   Usuários: {usr['total']} ({usr['coordenadores']} coordenadores)"
        )
        self.stdout.write(
            f"   Municípios: {est['municipios']} ({est['municipios_com_eventos']} com eventos)"
        )
        self.stdout.write(
            f"   Projetos: {est['projetos']} ({est['projetos_com_eventos']} com eventos)"
        )
        ...
        self.stdout.write(
            f"   Eventos com formador: {form['solicitacoes_com_formador']} ({form['percentual_com_formador']}%)"
        )
