"""
ETL Command: Import AcaoDAT from CSV/XLSX
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument(
            "file_path",
            type=str,
            help="Path to CSV or XLSX file with AcaoDAT data",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        self.stdout.write(self.style.SUCCESS("IMPORT DAT CADASTROS"))
        self.stdout.write(self.style.SUCCESS("=" * 80))
        ...
        # Run import
        try:
            report = import_dat_cadastros(str(file_path), dry_run=dry_run)
        ...
        if total_skipped > 0:
            ...
            if skipped["tipo_acao"] > 0:
                self.stdout.write(self.style.WARNING(f"  - Tipo de ação missing: {skipped['tipo_acao']}"))
            if skipped["responsavel"] > 0:
                self.stdout.write(self.style.WARNING(f"  - Responsável not found: {skipped['responsavel']}"))
            if skipped["other"] > 0:
        ...
        if total_pendencias > 0:
            ...
            if pendencias["tipo_acao"]:
                self.stdout.write(self.style.WARNING(f"  - Tipos de Ação: {len(pendencias['tipo_acao'])}"))
            if pendencias["responsaveis"]:
                self.stdout.write(self.style.WARNING(f"  - Responsáveis: {len(pendencias['responsaveis'])}"))
        ...
        self.stdout.write("=" * 80)
        self.stdout.write(f"Full report saved to: out_etl/import_dat_cadastros_report.json")
