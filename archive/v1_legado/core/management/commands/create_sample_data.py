...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Erro na criação de dados: {str(e)}"))
            raise
                ...
                try:
                    ...
                    for group_name in user_data["groups"]:
                        try:
                            ...
                            self.stdout.write(
                                self.style.WARNING(
                                    f"Grupo não encontrado: {group_name}"
                                )
                            )
                    ...
                    if "formador" in user_data["groups"]:
                        ...
                        if not FormadorService.filter_formadores(
                            email=usuario.email
                        ).exists():
    ...
    def create_sample_solicitacoes(self):
        ...
        if not all(
            ...
            self.stdout.write(
                self.style.WARNING(
                    "Dados insuficientes para criar solicitações de exemplo"
                )
            )
            return
            ...
            if not self.dry_run:
                try:
                    ...
                    formadores_selecionados = random.sample(
                        list(formadores), num_formadores
                    )
                    ...
                    created_count += 1
                    self.stdout.write(
                        f"  Solicitacao criada: {sol_data['titulo'][:50]}..."
                    )
