...
class DashboardStatsAPITestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.tipo_evento = TipoEvento.objects.create(nome="Workshop API", online=False)

        self.formador1 = Formador.objects.create(
            nome="João API", email="joao.api@test.com", ativo=True
        )

        self.formador2 = Formador.objects.create(
            nome="Maria API", email="maria.api@test.com", ativo=True
        )

        self.formador_inativo = Formador.objects.create(
            nome="Pedro Inativo", email="pedro.inativo@test.com", ativo=False
        )
