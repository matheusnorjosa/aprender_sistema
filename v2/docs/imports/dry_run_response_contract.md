...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | `usuarios_import.py` | `POST /api/usuarios/import/` | `ImportUsuariosView` (`views_import_usuarios.py`) | `import_usuarios_from_file(*, path, dry_run=True, actor=None)` | `True` | `cpf,nome,email,telefone,cargo,is_active,grupos` | `Usuario` (+ `Group` M2M) | ✅ | ✅ (campos não-chave) | ✅ ASQ-016 | CPF unique | — | dict aninhado |
