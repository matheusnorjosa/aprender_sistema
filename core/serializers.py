...
try:
    ...
    class SolicitacaoListSerializer(serializers.ModelSerializer):
        ...
        coordenador_nome = serializers.CharField(
            source="coordenador.get_full_name", read_only=True
        )
        formador_nome = serializers.CharField(source="formador.nome", read_only=True)
        projeto_nome = serializers.CharField(source="projeto.nome", read_only=True)
        municipio_nome = serializers.CharField(source="municipio.nome", read_only=True)
        tipo_evento_nome = serializers.CharField(
            source="tipo_evento.nome", read_only=True
        )
    ...
    class AprovacaoSerializer(serializers.ModelSerializer):
        ...
        aprovador_nome = serializers.CharField(
            source="aprovador.get_full_name", read_only=True
        )
    ...
    class HealthCheckSerializer(serializers.Serializer):
        ...
        status = serializers.CharField(
            help_text="Status geral do sistema (healthy/unhealthy)"
        )
        timestamp = serializers.DateTimeField(help_text="Timestamp da verificação")
        environment = serializers.CharField(
            help_text="Ambiente (development/staging/production)"
        )
    ...
    class DatabaseCheckSerializer(serializers.Serializer):
        ...
        status = serializers.CharField(
            help_text="Status do database (healthy/unhealthy)"
        )
        response_time_ms = serializers.IntegerField(
            help_text="Tempo de resposta em ms", required=False
        )
        error = serializers.CharField(
            help_text="Mensagem de erro se houver", required=False
        )
    ...
    class SystemMetricsSerializer(serializers.Serializer):
        ...
        cpu_percent = serializers.FloatField(
            help_text="Uso de CPU em %", required=False
        )
        memory_percent = serializers.FloatField(
            help_text="Uso de memória em %", required=False
        )
        disk_usage_percent = serializers.FloatField(
            help_text="Uso de disco em %", required=False
        )
        load_average = serializers.ListField(
            help_text="Load average do sistema", required=False
        )
    ...
    class ApplicationMetricsSerializer(serializers.Serializer):
        ...
        solicitacoes_pendentes = serializers.IntegerField(
            help_text="Solicitações pendentes"
        )
        recent_audit_logs = serializers.IntegerField(
            help_text="Logs de auditoria recentes"
        )
