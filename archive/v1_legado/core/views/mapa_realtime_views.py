...
from core.models import Municipio, Projeto, Solicitacao, SolicitacaoStatus
            ...
            while True:
                try:
                    ...
                    if current_update and current_update != last_update:
                        # Houve mudança! Buscar dados atualizados (APROVADO, AGENDADO, REALIZADO)
                        novas_solicitacoes = (
...
class MapaStatusAPIView(View):
    ...
    def get(self, request):
        try:
            ...
            estados_afetados = [
                item["municipio__uf"] for item in estados_com_novos_projetos
            ]
    ...
    def post(self, request):
        try:
            ...
            if not municipio_id or not acao:
                return JsonResponse(
                    {"success": False, "error": "municipio_id e acao são obrigatórios"},
                    status=400,
                )
            ...
            try:
                ...
                return JsonResponse(
                    {"success": False, "error": "Município ou projeto não encontrado"},
                    status=404,
                )
            ...
            return JsonResponse(
                {
                    "success": True,
                    "timestamp": timezone.now().isoformat(),
                    "municipio": {"nome": municipio.nome, "uf": municipio.uf},
                    "projeto": {"nome": projeto.nome if projeto else None},
                    "acao": acao,
