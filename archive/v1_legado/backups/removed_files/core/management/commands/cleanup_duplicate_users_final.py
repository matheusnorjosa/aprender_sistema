...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("🧹 LIMPEZA FINAL DE USUÁRIOS DUPLICADOS"))
    ...
    def _process_duplicate_group_selective(self, group_key, users, stats):
        ...
        for user in duplicate_users:
            ...
            if can_remove:
                ...
                if self.verbose:
                    self.stdout.write(
                        f"  ⚠️ PULANDO: {user.first_name} {user.last_name} "
    ...
    def _can_safely_remove_user(self, user):
        ...
        with connection.cursor() as cursor:
            ...
            try:
                cursor.execute(
                    "SELECT COUNT(*) FROM core_formadoressolicitacao WHERE formador_id = %s",
                    [str(user.id)],
                )
            ...
            try:
                cursor.execute(
                    "SELECT COUNT(*) FROM core_formador WHERE usuario_id = %s",
                    [str(user.id)],
                )
                if cursor.fetchone()[0] > 0:
                    return False
            ...
            try:
                cursor.execute(
                    "SELECT COUNT(*) FROM core_disponibilidadeformadores WHERE formador_id = %s",
                    [str(user.id)],
                )
                if cursor.fetchone()[0] > 0:
                    return False
            ...
            important_groups = user.groups.filter(
            ...
            if user.email and "@planilha." not in user.email and user.email != "":
                ...
                same_name_users = Usuario.objects.filter(
                    ...
                    last_name__icontains=(
                        user.last_name.split()[-1] if user.last_name else ""
                    ),
                ).exclude(id=user.id)

                real_email_alternatives = same_name_users.exclude(
    ...
    def _print_final_stats(self, stats, before_count, after_count):
        ...
        self.stdout.write(
            f'🔍 Grupos de duplicatas encontrados: {stats["duplicates_found"]}'
        )
        self.stdout.write(
            f'✅ Usuários removidos com segurança: {stats["users_removed"]}'
        )
        self.stdout.write(
            f'⚠️ Usuários pulados (relacionamentos): {stats["users_skipped"]}'
        )
