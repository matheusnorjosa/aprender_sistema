...
from core.models import (
    ...
    SolicitacaoStatus, Aprovacao, AprovacaoStatus
)
...
def run():
    ...
    for i, formador in enumerate(formadores[:5]):
        ...
        if created:
            bloqueios_criados += 1
            print(f"✅ Bloqueio parcial criado para {formador.nome} em {data_bloqueio_parcial}")
    ...
    print(f"   🗓️ Solicitações aprovadas: {Solicitacao.objects.filter(status=SolicitacaoStatus.APROVADO).count()}")
