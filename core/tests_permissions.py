...
from core.models import Usuario, Solicitacao, Projeto, Municipio, TipoEvento, Aprovacao, LogAuditoria
...
class GroupsAndPermissionsSetupTest(TestCase):
    ...
    def test_groups_exist(self):
        ...
        expected_groups = ['superintendencia', 'coordenador', 'formador', 'controle', 'diretoria', 'admin']
    ...
    def test_group_permissions_assignment(self):
        ...
        perm_codes = list(superintendencia.permissions.values_list('codename', flat=True))
        expected_perms = ['view_solicitacao', 'change_solicitacao', 'view_aprovacao', 'add_aprovacao', 'view_logauditoria']
        
        for perm in expected_perms:
            with self.subTest(group='superintendencia', permission=perm):
                self.assertIn(perm, perm_codes, f"Superintendência deveria ter permissão '{perm}'")
        ...
        for perm in expected_perms:
            with self.subTest(group='coordenador', permission=perm):
                self.assertIn(perm, perm_codes, f"Coordenador deveria ter permissão '{perm}'")
        
        # Controle
        controle = Group.objects.get(name='controle')
        perm_codes = list(controle.permissions.values_list('codename', flat=True))
        self.assertIn('sync_calendar', perm_codes, "Controle deveria ter permissão 'sync_calendar'")
        
        # Diretoria
        diretoria = Group.objects.get(name='diretoria')
        perm_codes = list(diretoria.permissions.values_list('codename', flat=True))
        self.assertIn('view_relatorios', perm_codes, "Diretoria deveria ter permissão 'view_relatorios'")
...
class UserGroupSyncTest(TestCase):
    ...
    def test_user_with_invalid_papel_not_assigned_to_group(self):
        ...
        role_groups = ['coordenador', 'formador', 'superintendencia', 'controle', 'diretoria', 'admin']
...
class PermissionBasedViewsTest(TestCase):
    ...
    def setUp(self):
        ...
        # Coordenador
        coord_perms = Permission.objects.filter(codename__in=['add_solicitacao', 'view_solicitacao'])
        self.coord_group.permissions.set(coord_perms)
        
        # Superintendência  
        super_perms = Permission.objects.filter(codename__in=['view_aprovacao', 'add_aprovacao', 'view_solicitacao'])
        self.super_group.permissions.set(super_perms)
        
        # Formador
        form_perms = Permission.objects.filter(codename__in=['add_disponibilidadeformadores', 'view_solicitacao'])
...
class TemplatePermissionsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.form_group = Group.objects.get_or_create(name='formador')[0]
        form_perms = Permission.objects.filter(codename__in=['add_disponibilidadeformadores'])
        self.form_group.permissions.set(form_perms)
...
class PermissionMigrationIntegrityTest(TestCase):
    ...
    def test_all_views_using_permission_system(self):
        ...
        from core.views import (
            SolicitacaoCreateView, AprovacoesPendentesView, AprovacaoDetailView,
            BloqueioCreateView, GoogleCalendarMonitorView, DiretoriaExecutiveDashboardView
        )
    ...
    def test_superuser_bypass_works(self):
        ...
        for url_name in urls_to_test:
            with self.subTest(url=url_name):
                url = reverse(url_name)
                response = self.client.get(url)
                self.assertIn(response.status_code, [200, 302])  # 200 OK ou 302 redirect válido
