...
class TestGCalExport(TestCase):
    ...
    def test_export_edge_cases_timezone_boundaries(self):
        ...
        self.client.force_authenticate(user=self.user_controle)
        response = self.client.get(self.url, {"export_format": "json", "start": "2025-02-10"})
        data = response.json()
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(self.url, {"export_format": "json", "end": "2025-02-10"})
        data = response.json()
        self.assertEqual(response.status_code, 200)
        returned_ids = [item["id"] for item in data["results"]]
        self.assertIn(sol_midnight.id, returned_ids, "Evento às 00:00 deve ser incluído")
        ...
        response = self.client.get(self.url, {"export_format": "json", "start": "2025-02-10", "end": "2025-02-10"})
        data = response.json()
        self.assertEqual(response.status_code, 200)
        returned_ids = [item["id"] for item in data["results"]]
