...
- **Multi-formador**: considera eventos onde o usuário é `usuario` OU participante (`participations__usuario`). **Com filtro de papel** (`_check_conflicts_impl`, achado M08-07 / épico #1664 — **resolvido** na PR #1794, commit `cd60882a`): a query de eventos existentes filtra por `participations__role__in=ENFORCED_ROLES` (= COORDENADOR, FORMADOR, COORD_ACOMPANHA). `CONVIDADO` **não** conta como ocupação em RD-01/RD-04/RD-05; `COORD_ACOMPANHA` ocupa por design. O predicado é **único**: o mesmo `ENFORCED_ROLES` vale no serviço e no guard de enforcement (que o importa de `availability_service`, `solicitacao_availability.py`), então a antiga assimetria com `CONVIDADO` acabou.
