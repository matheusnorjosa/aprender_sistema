...
class DATAcao(models.Model):
    ...
    municipio: models.ForeignKey[Municipio] = models.ForeignKey(  # type: ignore[assignment]
        "core.Municipio", on_delete=models.PROTECT, related_name="dat_acoes", verbose_name="Município"
    )
    projeto: models.ForeignKey[Projeto] = models.ForeignKey(  # type: ignore[assignment]
        "core.Projeto", on_delete=models.PROTECT, related_name="dat_acoes", verbose_name="Projeto"
    )
    ...
    status_carta = models.CharField(
        max_length=20, choices=StatusEtapa.choices, default=StatusEtapa.PENDENTE, verbose_name="Status da Carta"
    )
    data_carta = models.DateField(null=True, blank=True, verbose_name="Data de Envio da Carta")
    observacao_carta = models.TextField(blank=True, max_length=500, verbose_name="Observação Carta")

    # Etapa 2: Contato
    status_contato = models.CharField(
        max_length=20, choices=StatusEtapa.choices, default=StatusEtapa.PENDENTE, verbose_name="Status do Contato"
    )
    data_contato = models.DateField(null=True, blank=True, verbose_name="Data do Contato")
    observacao_contato = models.TextField(blank=True, max_length=500, verbose_name="Observação Contato")

    # Etapa 3: Reunião
    status_reuniao = models.CharField(
        max_length=20, choices=StatusEtapa.choices, default=StatusEtapa.PENDENTE, verbose_name="Status da Reunião"
    )
    data_reuniao = models.DateField(null=True, blank=True, verbose_name="Data da Reunião")
    observacao_reuniao = models.TextField(blank=True, max_length=500, verbose_name="Observação Reunião")

    # Etapa 4: Entrega
    status_entrega = models.CharField(
        max_length=20, choices=StatusEtapa.choices, default=StatusEtapa.PENDENTE, verbose_name="Status da Entrega"
    )
    data_entrega = models.DateField(null=True, blank=True, verbose_name="Data da Entrega")
    observacao_entrega = models.TextField(blank=True, max_length=500, verbose_name="Observação Entrega")

    # Status geral
    ativo = models.BooleanField(default=True)
    prioridade = models.PositiveSmallIntegerField(
        default=0, verbose_name="Prioridade", help_text="0 = normal, 1+ = alta prioridade"
    )

    # Auditoria
    created_by: models.ForeignKey[Usuario] = models.ForeignKey(  # type: ignore[assignment]
        "core.Usuario", on_delete=models.PROTECT, related_name="dat_acoes_criadas", verbose_name="Criado por"
    )
