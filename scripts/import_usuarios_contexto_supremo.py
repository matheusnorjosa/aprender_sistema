...
class ImportadorUsuariosContextoSupremo:
    ...
    def processar_gerencia_especial(self, gerencia: str) -> Dict[str, str]:
        ...
        """
        if gerencia == "Outros":
            return {"tipo": "especial", "mapeamentos": self.gerencias_especiais}
        else:
            return {"tipo": "normal", "gerencia": gerencia}
    ...
    def importar_usuario(self, usuario_data: Dict) -> bool:
        ...
        try:
            if self.dry_run:
                logger.info(
                    f"🔍 [DRY RUN] Usuário: {usuario_data.get('Nome', 'N/A')} - {usuario_data.get('Cargo', 'N/A')}"
                )
                return True
            ...
            if gerencia_info["tipo"] == "especial":
                ...
                logger.info(
                    f"🏢 Gerência especial mapeada para {nome}: {gerencia_info['mapeamentos']}"
                )
            ...
            logger.error(
                f"❌ Erro ao importar usuário {usuario_data.get('Nome', 'N/A')}: {str(e)}"
            )
            return False
    ...
    def gerar_relatorio_final(self):
        ...
        logger.info(
            f"   - Usuários importados: {self.estatisticas['usuarios_importados']}"
        )
        logger.info(
            f"   - Formadores importados: {self.estatisticas['formadores_importados']}"
        )
        logger.info(
            f"   - Coordenadores importados: {self.estatisticas['coordenadores_importados']}"
        )
        logger.info(
            f"   - Gerentes importados: {self.estatisticas['gerentes_importados']}"
        )
        logger.info(
            f"   - Gerências especiais mapeadas: {self.estatisticas['gerencias_especiais_mapeadas']}"
        )
