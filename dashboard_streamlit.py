...
def get_metrics(ano):
    ...
    total_solicitacoes = Solicitacao.objects.filter(
    ...
    taxa_aprovacao = round(
        (solicitacoes_aprovadas / total_solicitacoes * 100) if total_solicitacoes > 0 else 0,
        1
    )
...
with col2:
    st.metric(
        ...
        delta=f"+{metrics['solicitacoes_aprovadas'] - 180}" if metrics['solicitacoes_aprovadas'] > 180 else None
    )
...
with col3:
    st.metric(
        ...
        delta=f"+{metrics['taxa_aprovacao'] - 85}%" if metrics['taxa_aprovacao'] > 85 else None
    )

with col4:
    st.metric(
        label="👥 Formadores Ativos",
        value=metrics['formadores_ativos'],
        delta=None
    )
...
def get_chart_data(ano):
    ...
    for mes in range(1, 13):
        ...
        monthly_data.append({
            'Mês': datetime(ano, mes, 1).strftime('%b'),
            'Eventos': count
        })
    ...
    for formador in top_formadores:
        formadores_data.append({
            'Formador': formador['formador__nome'],
...
status_data = [
    ...
    {'Status': 'Pendentes', 'Quantidade': max(0, metrics['total_solicitacoes'] - metrics['solicitacoes_aprovadas'])},
]
