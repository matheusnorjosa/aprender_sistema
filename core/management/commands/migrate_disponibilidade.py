...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(source_file):
            self.stdout.write(self.style.ERROR(f'Arquivo não encontrado: {source_file}'))
            return
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING('MODO DRY RUN - Execute sem --dry-run para aplicar'))
            ...
            try:
                ...
                try:
                    ...
                    usuario = Usuario.objects.get(
                        first_name__icontains=usuario_nome.split()[0] if usuario_nome else ''
                    )
                    # Depois buscar formador vinculado
                    try:
                        formador = Formador.objects.get(usuario=usuario)
                    except Formador.DoesNotExist:
                        self.stdout.write(f'Formador não encontrado para usuário: {usuario_nome}')
                        continue
                    ...
                    usuario = Usuario.objects.filter(
                        first_name__iexact=usuario_nome.split()[0] if usuario_nome else ''
                    ).first()
                    if not usuario:
                        continue
                    try:
                ...
                hora_inicio = time(0, 0) if 'total' in tipo_bloqueio.lower() else time(8, 0)
                hora_fim = time(23, 59) if 'total' in tipo_bloqueio.lower() else time(18, 0)
