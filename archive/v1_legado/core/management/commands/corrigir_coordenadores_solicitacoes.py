...
from core.models import Municipio, Projeto, Solicitacao, TipoEvento, Usuario
...
class Command(BaseCommand):
    ...
    def _carregar_dados_planilha(self):
        ...
        self.stdout.write(
            f"✅ {len(eventos_com_coordenador)} eventos com coordenador identificado"
        )
        self.stdout.write(
            f"✅ {len(coordenadores_unicos)} coordenadores únicos na planilha"
        )
    ...
    def _criar_mapeamento_coordenadores(self, eventos_planilha):
        ...
        for coord_planilha in coordenadores_planilha:
            ...
            for usuario in usuarios_coordenadores:
                ...
                if (
                    coord_planilha.lower() in nome_completo
                    or first_name in coord_planilha.lower()
                ):
            ...
            if melhor_match:
                ...
                if self.verbose:
                    self.stdout.write(
                        f"  ✅ {coord_planilha} → {melhor_match.nome_completo} ({melhor_match.username})"
                    )
            else:
                self.stdout.write(
        ...
        self.stdout.write(
            f"✅ {len(mapeamento)} coordenadores mapeados de {len(coordenadores_planilha)} total"
        )
    ...
    def _corrigir_solicitacoes(self, mapeamento):
        ...
        solicitacoes_admin = (
            ...
            .filter(usuario_solicitante=admin_user)
        )
        ...
        if not self.dry_run:
            with transaction.atomic():
                ...
                for solicitacao in solicitacoes_admin:
                    ...
                    for evento_planilha in eventos_planilha:
                        ...
                        if (
                            municipio_sol == municipio_plan
                            and data_str == data_plan
                            and projeto_sol == projeto_plan
                        ):
                    ...
                    if not evento_encontrado:
                        for evento_planilha in eventos_planilha:
                            municipio_plan = normalizar_texto(
                                evento_planilha["municipio"]
                            )
                            data_plan = evento_planilha["data"]
                    ...
                    if not evento_encontrado:
                        ...
                        if municipio_palavras:
                            palavra_principal = municipio_palavras[
                                0
                            ]  # Primeira palavra (cidade)
                            ...
                            for evento_planilha in eventos_planilha:
                                ...
                                if (
                                    palavra_principal in municipio_plan
                                    and data_str == data_plan
                                ):
                                    evento_encontrado = evento_planilha
                                    break
                    ...
                    if evento_encontrado:
                        ...
                        if coordenador_planilha in mapeamento:
                            ...
                            if self.verbose and corrigidas <= 10:
                                ...
                                self.stdout.write(
                                    f"  ✅ {solicitacao.municipio.nome} ({municipio_match}) → {novo_usuario.first_name} {novo_usuario.last_name}"
                                )
                        else:
                            nao_mapeadas += 1
                            if self.verbose and nao_mapeadas <= 5:
                                self.stdout.write(
                                    f"  ⚠️  Coordenador '{coordenador_planilha}' não mapeado para {solicitacao.municipio.nome}"
                                )
                    else:
                        nao_mapeadas += 1
                        if self.verbose and nao_mapeadas <= 5:
                            self.stdout.write(
                                f"  ❌ Não encontrado: {solicitacao.municipio.nome} - {data_str} - {solicitacao.projeto.nome}"
                            )

                self.stdout.write(f"✅ {corrigidas} solicitações corrigidas")
                self.stdout.write(
                    f"⚠️  {nao_mapeadas} solicitações não puderam ser mapeadas"
                )
            ...
            for coord_planilha, usuario in mapeamento.items():
                if count < 5:
                    self.stdout.write(
                        f"  • Eventos de '{coord_planilha}' → {usuario.nome_completo}"
                    )
                    count += 1
