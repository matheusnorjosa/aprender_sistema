...
def analisar_projetos_e_colecoes(gc):
    ...
    try:
        ...
        for produto in sorted(
            list(produtos_identificados)[:20]
...
def analisar_formadores_coordenadores(gc):
    ...
    try:
        ...
        for formador in formadores[:10]:  # Mostrar primeiros 10
            print(
                f"   - {formador.get('Nome Completo', '')} - {formador.get('Gerência', '')}"
            )
        ...
        for coordenador in coordenadores:
            print(
                f"   - {coordenador.get('Nome Completo', '')} - {coordenador.get('Gerência', '')}"
            )

        print(f"\n👔 GERENTES: {len(gerentes)}")
        for gerente in gerentes:
            print(
                f"   - {gerente.get('Nome Completo', '')} - {gerente.get('Gerência', '')}"
            )

        print(f"\n🔍 CONTROLE: {len(controle)}")
        for pessoa in controle:
            print(
                f"   - {pessoa.get('Nome Completo', '')} - {pessoa.get('Gerência', '')}"
            )
        ...
        for pessoa in outros:
            print(
                f"   - {pessoa.get('Nome Completo', '')} - {pessoa.get('Cargo', '')} - {pessoa.get('Gerência', '')}"
            )
...
def main():
    ...
    print(
        f"   👥 Total de usuários ativos: {len(usuarios_hierarquia.get('usuarios', []))}"
    )
