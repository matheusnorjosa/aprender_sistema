    ...
    if response.status_code == 403:
        ...
        if admin_response.status_code == 200:
            ...
            if formador_ids:
                ...
                formador_test_url = f"{base_url}/formador/eventos/?formador_id={test_formador_id}"
                
                response = session.get(formador_test_url)
                if response.status_code != 200:
                    print(f"❌ Erro ao acessar página com formador_id: {response.status_code}")
                    return False
                
                print(f"✅ Página acessível como superuser com formador_id: {test_formador_id}")
            else:
                print("⚠️ Nenhum formador encontrado para testar")
                return True  # Não é erro, apenas não há dados
        else:
            print(f"❌ Não foi possível acessar admin de formadores: {admin_response.status_code}")
            return False
