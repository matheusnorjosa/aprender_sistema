...
class Command(BaseCommand):
    ...
    def process_produtos(self):
        ...
        try:
            ...
            for projeto_planilha, produtos in produtos_por_projeto.items():
                try:
                    projeto_nome = projeto_mappings.get(projeto_planilha, projeto_planilha)
                    ...
                    if codigos_produtos:
                        ...
                        if len(codigos_produtos) > 5:
                            projeto.codigo_produto += f' (e mais {len(codigos_produtos)-5})'
                    
                    projeto.save()
                    
                    status_setor = "SUPER" if projeto.setor and projeto.setor.vinculado_superintendencia else "DIRETO"
                    self.stdout.write(
    ...
    def determine_setor_for_project(self, project_name):
        ...
        if any(keyword in project_lower for keyword in ['tema', 'novo lendo', 'lendo e escrevendo', 'cirandar', 'transito']):
        ...
        elif any(keyword in project_lower for keyword in ['cor da gente', 'avançando juntos', 'educação financeira', 'sou da paz', 'ler ouvir']):
