...
if ENVIRONMENT == "production" and not MCP_TOOLS_ENABLED:
    print(
        "❌ MCP Tools bloqueados em produção. Defina MCP_TOOLS_ENABLED=True para habilitar."
    )
    sys.exit(1)
    ...
    try:
        if name == "get_architecture_patterns":
            ...
            return [
                {
                    "type": "text",
                    "text": "📋 Arquivo de arquitetura não encontrado. Verifique se ARQUITETURA_REFERENCIA.md existe na raiz do projeto.",
                }
            ]
            ...
            return [
                {
                    "type": "text",
                    "text": "📋 Arquivo de padrões Python não encontrado. Verifique se PADROES_CODIGO_PYTHON.md existe na raiz do projeto.",
                }
            ]
            ...
            return [
                {
                    "type": "text",
                    "text": "📋 Arquivo de segurança não encontrado. Verifique se GUIA_SEGURANCA.md existe na raiz do projeto.",
                }
            ]
            ...
            if category == "approval":
                ...
                approval_info = []
                if context_content:
                    section = _extract_section_from_content(
                        context_content, "aprovação"
                    )
...
"""
            ...
            return [
                {
                    "type": "text",
                    "text": f"📋 Regras para categoria '{category}' não encontradas nos documentos disponíveis.",
                }
            ]
            ...
            if "def " in code:
                ...
                if "->" not in code and ":" not in code.split("def ")[1].split("(")[0]:
                    validation_results.append(
                        "⚠️ AVISO: Função sem type hints (requerido no Sistema APRENDER)"
                    )

                # Verificar docstring
                if '"""' not in code and "'''" not in code:
                    validation_results.append(
                        "⚠️ AVISO: Função sem docstring (padrão Sistema APRENDER)"
                    )

            # Verificar uso de logging em vez de print
            if "print(" in code:
                validation_results.append(
                    "❌ ERRO: Usar logging em vez de print (padrão Sistema APRENDER)"
                )

            # Verificar tratamento de exceções
            if "except:" in code and "except Exception" not in code:
                validation_results.append(
                    "⚠️ AVISO: Bloco except muito genérico (especificar Exception)"
                )
            ...
            if context == "view":
                if "request.POST" in code or "request.GET" in code:
                    if "clean(" not in code and "is_valid(" not in code:
                        validation_results.append(
                            "❌ ERRO: Dados de entrada não validados (usar forms.is_valid())"
                        )

                if not any(
                    perm in code
                    for perm in ["@login_required", "@permission_required", "has_perm"]
                ):
                    validation_results.append(
                        "❌ ERRO: View sem verificação de permissão (obrigatório no Sistema APRENDER)"
                    )

                # Verificar timezone awareness
                if "datetime.now()" in code:
                    validation_results.append(
                        "⚠️ AVISO: Usar timezone.now() em vez de datetime.now() (timezone aware)"
                    )

            elif context == "model":
                if "models.Model" not in code and "class" in code:
                    validation_results.append(
                        "❌ ERRO: Model não herda de models.Model"
                    )

                if "class Meta:" not in code and "class" in code:
                    validation_results.append(
                        "⚠️ AVISO: Model sem classe Meta (recomendado para Sistema APRENDER)"
                    )

                # Verificar timezone em DateTimeField
                if "DateTimeField" in code and "timezone" not in code:
                    validation_results.append(
                        "⚠️ AVISO: DateTimeField deve ser timezone-aware"
                    )
                ...
                if "from core.models import" not in code and "import" in code:
                    validation_results.append(
                        "⚠️ AVISO: Services devem importar models da core"
                    )

                if "transaction.atomic" not in code and (
                    "save(" in code or "delete(" in code
                ):
                    validation_results.append(
                        "⚠️ AVISO: Operações de BD em services devem usar transaction.atomic"
                    )
                ...
                if "BaseCommand" not in code:
                    validation_results.append(
                        "❌ ERRO: Command deve herdar de BaseCommand"
                    )

                if "handle(" not in code:
                    validation_results.append(
                        "❌ ERRO: Command deve implementar método handle()"
                    )
                ...
                if "forms.Form" not in code and "forms.ModelForm" not in code:
                    validation_results.append(
                        "❌ ERRO: Form deve herdar de forms.Form ou forms.ModelForm"
                    )

                if "clean_" in code and "ValidationError" not in code:
                    validation_results.append(
                        "⚠️ AVISO: Método clean_ deve usar ValidationError para erros"
                    )

            result = (
                "\n".join(validation_results)
                if validation_results
                else "✅ Código segue os padrões do Sistema APRENDER"
            )
            ...
            if "execute(" in code and (
                "%" in code or ".format(" in code or 'f"' in code
            ):
                vulnerabilities.append(
                    "🔴 RISCO: Possível SQL injection - usar parâmetros nomeados ou ORM Django"
                )
            ...
            if any(secret in code.lower() for secret in secret_indicators):
                if "=" in code and ("'" in code or '"' in code):
                    vulnerabilities.append(
                        "🔴 RISCO: Possível segredo hardcoded - usar variáveis de ambiente"
                    )
            ...
            if "request.POST" in code or "request.GET" in code:
                if "is_valid(" not in code and "clean(" not in code:
                    vulnerabilities.append(
                        "🔴 RISCO: Dados de entrada não validados - usar Django Forms"
                    )

            # Verificar XSS em templates/responses
            if "HttpResponse" in code and "mark_safe" in code:
                vulnerabilities.append(
                    "🟡 RISCO: Uso de mark_safe pode permitir XSS - validar conteúdo"
                )
            ...
            if any(op in code for op in critical_operations):
                if "has_perm" not in code and "permission_required" not in code:
                    vulnerabilities.append(
                        "🔴 RISCO: Operação crítica sem verificação de permissão"
                    )

            # Verificar uso seguro de eval/exec
            if "eval(" in code or "exec(" in code:
                vulnerabilities.append(
                    "🔴 RISCO: Uso de eval/exec é perigoso - evitar ou validar entrada"
                )

            # Verificar configurações Django em produção
            if "DEBUG = True" in code:
                vulnerabilities.append(
                    "🟡 RISCO: DEBUG=True não deve ser usado em produção"
                )

            # Verificar CSRF protection
            if "@csrf_exempt" in code:
                vulnerabilities.append(
                    "🟡 RISCO: csrf_exempt remove proteção CSRF - usar apenas se necessário"
                )

            result = (
                "\n".join(vulnerabilities)
                if vulnerabilities
                else "✅ Nenhuma vulnerabilidade óbvia detectada"
            )
            return [{"type": "text", "text": result}]
            ...
            return [
                {
                    "type": "text",
                    "text": "📋 Contexto do sistema não encontrado. Verifique se CLAUDE_CONTEXT_PACKAGE.md existe.",
                }
            ]
...
"""
            ...
            try:
                ...
                if analysis.get("error"):
                    return [
                        {
                            "type": "text",
                            "text": f"❌ Erro ao analisar arquivo: {analysis['error']}",
                        }
                    ]
                ...
                if method == "streaming":
                    ...
                    result = (
                        f"✅ Processado via streaming: {len(chunks)} itens encontrados"
                    )
                else:
                ...
                result += (
                    f"• Tokens estimados: {analysis.get('estimated_tokens', 0):,}\n"
                )
                ...
                return [
                    {"type": "text", "text": f"❌ Erro ao processar arquivo: {str(e)}"}
                ]
            ...
            try:
                ...
                if method == "sections":
                    ...
                    result = (
                        f"✅ Processado por seções: {len(sections)} seções encontradas"
                    )
                else:
                    chunks = list(_process_markdown_tokens(filename))
                    result = (
                        f"✅ Processado por tokens: {len(chunks)} chunks encontrados"
                    )
            ...
            try:
                ...
                return [
                    {"type": "text", "text": f"❌ Erro ao analisar arquivo: {str(e)}"}
                ]
            ...
            if not data_source or not analysis_type:
                return [
                    {
                        "type": "text",
                        "text": "❌ Erro: data_source e analysis_type são obrigatórios",
                    }
                ]
            ...
            try:
                ...
                if df is None:
                    return [
                        {
                            "type": "text",
                            "text": f"❌ Erro: Não foi possível carregar dados de {data_source}",
                        }
                    ]
                ...
                return [
                    {"type": "text", "text": f"❌ Erro na análise com Pandas: {str(e)}"}
                ]
    ...
    try:
        ...
        if file_path.suffix.lower() == ".json":
            analysis["recommended_method"] = (
                "streaming" if file_size > 5 * 1024 * 1024 else "chunks"
            )
            ...
            try:
                ...
                analysis["structure_info"] = {
                    "error": f"Não foi possível analisar estrutura: {str(e)}"
                }
            ...
            if column in df.columns:
                if isinstance(condition, dict):
                    ...
                    if "equals" in condition:
                        ...
                        df = df[
                            df[column].str.contains(condition["contains"], na=False)
                        ]
...
def _perform_pandas_analysis(
    df: pd.DataFrame, analysis_type: str, group_by: str = ""
) -> str:
    ...
    try:
        ...
        if analysis_type == "statistics":
            ...
            if group_by and group_by in df.columns:
                ...
                result += (
                    "⚠️ Coluna para agrupamento não especificada ou não encontrada\n"
                )
...
def _get_filtering_analysis(df: pd.DataFrame) -> str:
    ...
    for col in df.columns:
        if df[col].dtype == "object":
            ...
            result += (
                f"• {col}: numérico (min: {df[col].min()}, max: {df[col].max()})\n"
            )
        result += "\n"
