...
- [ ] Permissions: canonical `permission_classes = [HasPerm("codename")]` from `apps.core.rbac` (compose with `|`/`&`/`~`); no `user.groups.filter(name=...)` (banned by `scripts/rbac_lint.py`); ver `v2/docs/RBAC_NAMING.md`
