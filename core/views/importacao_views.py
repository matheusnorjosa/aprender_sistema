...
from core.models import CursoPlataforma, ImportacaoCursosCSV, Projeto, ProjetoCursoLink
...
class CursosCSVUploadView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def test_func(self):
        ...
        return (
            self.request.user.groups.filter(name="dat").exists()
    ...
    def get_context_data(self, **kwargs):
        ...
        context["importacoes_recentes"] = ImportacaoCursosCSV.objects.order_by(
            "-data_inicio"
        )[:5]
    ...
    def post(self, request):
        try:
            if "csv_file" not in request.FILES:
                return JsonResponse(
                    {"success": False, "error": "Arquivo CSV não encontrado"}
                )
            ...
            if not csv_file.name.endswith(".csv"):
                return JsonResponse(
                    {"success": False, "error": "Arquivo deve ser do tipo CSV"}
                )
            ...
            ano_filter = (
                int(ano_filter) if ano_filter and ano_filter.isdigit() else None
            )
            ...
            return JsonResponse(
                {"success": False, "error": f"Erro no processamento: {str(e)}"}
            )
...
class CursosCSVPreviewView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request):
        try:
            ...
            return JsonResponse(
                {"success": False, "error": f"Erro no preview: {str(e)}"}
            )
...
class VinculosProjetoCursoListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_queryset(self):
        return ProjetoCursoLink.objects.select_related(
            "projeto", "curso_plataforma"
        ).order_by("projeto__nome", "curso_plataforma__nome_breve")

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["total_vinculos"] = ProjetoCursoLink.objects.count()
        context["vinculos_manuais"] = ProjetoCursoLink.objects.filter(
            mapeamento_manual=True
        ).count()
        context["vinculos_automaticos"] = ProjetoCursoLink.objects.filter(
            mapeamento_manual=False
        ).count()
        return context
...
class CursosSemVinculoListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return CursoPlataforma.objects.filter(
            projetocursolink__isnull=True, ativo=True
        ).order_by("nome_breve")
    ...
    def post(self, request):
        try:
            ...
            if not curso_id or not projeto_id:
                return JsonResponse(
                    {"success": False, "error": "Curso e Projeto são obrigatórios"}
                )
            ...
            if ProjetoCursoLink.objects.filter(
                curso_plataforma=curso, projeto=projeto
            ).exists():
                return JsonResponse(
                    {
                        "success": False,
            ...
            return JsonResponse(
                {"success": False, "error": f"Erro ao criar vínculo: {str(e)}"}
            )
...
class DesvincularCursoView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request):
        try:
            ...
            if not vinculo_id:
                return JsonResponse(
                    {"success": False, "error": "ID do vínculo é obrigatório"}
                )
            ...
            return JsonResponse(
                {"success": False, "error": f"Erro ao remover vínculo: {str(e)}"}
            )


class ReprocessarVinculacaoAutomaticaView(
    ...
    def post(self, request):
        try:
            ...
            return JsonResponse(
                {"success": False, "error": f"Erro no reprocessamento: {str(e)}"}
            )
