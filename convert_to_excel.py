...
class ExcelConverter:
    ...
    def criar_aba_dados_principais(self, dados: List[Dict[str, Any]]):
        ...
        for row_idx, evento in enumerate(dados, 2):
            ...
            row_data = [
                ...
                (
                    evento.get("formadores", ["", ""])[1]
                    if len(evento.get("formadores", [])) > 1
                    else ""
                ),
                (
                    evento.get("formadores", ["", "", ""])[2]
                    if len(evento.get("formadores", [])) > 2
                    else ""
                ),
    ...
    def criar_aba_relatorio_municipios(self, dados: List[Dict[str, Any]]):
        ...
        for row, (municipio, count) in enumerate(
            ...
            ws.cell(row=row, column=3, value=f"{percentual:.1f}%").border = (
                self.border_all
            )
    ...
    def criar_aba_relatorio_formadores(self, dados: List[Dict[str, Any]]):
        ...
        # Dados
        for row, (formador, count) in enumerate(
            sorted(formadores.items(), key=lambda x: x[1], reverse=True), 2
        ):
    ...
    def extrair_coordenador(self, observacoes: str) -> str:
        ...
        if "Coordenador Responsável:" in observacoes:
            try:
                return (
                    observacoes.split("Coordenador Responsável:")[1]
                    .split("|")[0]
                    .strip()
                )
            except:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Converte dados Google Sheets para Excel"
    )
