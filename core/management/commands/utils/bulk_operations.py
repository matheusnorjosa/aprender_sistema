...
"""
from typing import List, Any, Dict, Iterator, Type
...
class BulkOperationsManager:
    ...
    def __init__(self, batch_size: int = None):
        self.batch_size = batch_size or getattr(settings, 'BULK_CREATE_BATCH_SIZE', 1000)
        self.stats = {
            'created': 0,
            'updated': 0,
            'skipped': 0,
    ...
    def chunked(self, iterable: List[Any], chunk_size: int = None) -> Iterator[List[Any]]:
        """
    ...
    def bulk_create_safe(self, model_class: Type[models.Model], 
                        objects: List[models.Model],
                        batch_size: int = None,
                        ignore_conflicts: bool = False) -> Dict[str, int]:
        """
            ...
            try:
                with transaction.atomic():
                    ...
                    if valid_objects:
                        ...
                        logger.info(f"Criados {len(created_objects)} objetos {model_class.__name__}")
    ...
    def bulk_update_safe(self, objects: List[models.Model], 
                        fields: List[str],
    ...
    def bulk_create_or_update(self, model_class: Type[models.Model],
                             objects: List[models.Model],
        ...
        for chunk in self.chunked(objects, batch_size):
            try:
                with transaction.atomic():
                    ...
                    match_values = [getattr(obj, match_field) for obj in chunk if hasattr(obj, match_field)]
                    
                    if match_values:
                        existing_objects = model_class.objects.filter(**{f"{match_field}__in": match_values})
                        existing_matches = {getattr(obj, match_field): obj for obj in existing_objects}
                        ...
                        if objects_to_create:
                            result = self.bulk_create_safe(model_class, objects_to_create, 
                                                         batch_size, ignore_conflicts=True)
                            created_count += result['created']
                        
                        if objects_to_update:
                            result = self.bulk_update_safe(objects_to_update, update_fields, batch_size)
                            updated_count += result['updated']
                    else:
                        # Todos são novos
                        result = self.bulk_create_safe(model_class, chunk, batch_size, ignore_conflicts=True)
                        created_count += result['created']
