...
class MapeadorUsuariosGrupos:
    ...
    def criar_formadores_faltantes(self):
        ...
        try:
            ...
            print(f"   👥 Encontrados {usuarios_formador.count()} usuários no grupo 'formador'")
            ...
            print(f"   📊 Usuários sem registro de Formador: {len(usuarios_sem_formador)}")
                ...
                try:
                    with transaction.atomic():
                        ...
                        if self.formadores_criados % 10 == 0:
                            print(f"   ✅ {self.formadores_criados} formadores criados...")

                except Exception as e:
                    self.erros.append(f"Erro criando formador para {usuario.username}: {e}")
    ...
    def determinar_area_atuacao(self, username):
        ...
        if any(termo in username_lower for termo in ['mat', 'matematica', 'calculo', 'numero']):
            area_nome = 'area_matematica'
        elif any(termo in username_lower for termo in ['port', 'lingua', 'texto', 'escrita', 'leitura']):
            area_nome = 'area_lingua_portuguesa'
        elif any(termo in username_lower for termo in ['cien', 'bio', 'fis', 'quim', 'natura']):
            area_nome = 'area_ciencias'
        elif any(termo in username_lower for termo in ['inf', 'crianca', 'bebe', 'pequeno']):
            area_nome = 'area_educacao_infantil'
        elif any(termo in username_lower for termo in ['arte', 'music', 'danca', 'teatro']):
            area_nome = 'area_artes'
        elif any(termo in username_lower for termo in ['hist', 'geo', 'social', 'humana']):
    ...
    def validar_mapeamento_grupos(self):
        ...
        if formadores_sem_grupo:
            print(f"   ⚠️ {len(formadores_sem_grupo)} formadores cujo usuário não está no grupo 'formador'")
        ...
        if grupo_formador:
            ...
            if usuarios_grupo_formador == registros_formador:
                print("   ✅ Mapeamento grupo 'formador' ↔ registros Formador: CONSISTENTE")
            else:
                print("   ⚠️ Mapeamento grupo 'formador' ↔ registros Formador: INCONSISTENTE")
    ...
    def gerar_relatorio(self, analise_inicial):
        ...
        relatorio = {
            ...
            'mudancas': {
                'gap_formadores_resolvido': analise_inicial['gap_formadores'] - analise_final.get('gap_formadores', 0)
            }
        }

        with open('/app/relatorio_mapeamento_usuarios_grupos.json', 'w', encoding='utf-8') as f:
...
def main():
    ...
    try:
        ...
        if relatorio['mudancas']['gap_formadores_resolvido'] > 0:
            print(f"   🔧 Gap de formadores resolvido: {relatorio['mudancas']['gap_formadores_resolvido']}")
