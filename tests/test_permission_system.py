...
from core.mixins import user_has_role, user_has_any_role, get_user_role_names
...
def test_groups_and_permissions():
    ...
    custom_permissions = [
        'sync_calendar', 'view_own_solicitacoes', 'view_all_solicitacoes',
        'view_calendar', 'view_relatorios', 'view_own_events'
    ]
    
    for perm_codename in custom_permissions:
        try:
            perm = Permission.objects.get(codename=perm_codename, content_type__app_label='core')
    ...
    print(f"   - user_has_role(user, 'coordenador'): {user_has_role(user, 'coordenador')}")
    print(f"   - user_has_role(user, 'formador'): {user_has_role(user, 'formador')}")
    print(f"   - user_has_any_role(user, ['coordenador', 'admin']): {user_has_any_role(user, ['coordenador', 'admin'])}")
