...
from typing import Any, Dict, List, Optional, QuerySet
...
class OptimizedQueryManager:
    ...
    def get_formadores_optimized(ativo=True, with_stats=False):
        ...
        def fetch_data():
            queryset = (
                Formador.objects.filter(ativo=ativo)
    ...
    @staticmethod
    def get_availability_conflicts_optimized(
        formador_ids: List[str], start_date: str, end_date: str
    ):
        """
        Optimized query for availability conflicts checking
        """
        cache_key = cache_service.generate_key(
            "conflicts", "check", "_".join(sorted(formador_ids)), start_date, end_date
        )
        ...
        def fetch_data():
            ...
            conflicts = {"bloqueios": [], "eventos_existentes": [], "deslocamentos": []}

            # Check availability blocks (optimized)
            bloqueios = (
                DisponibilidadeFormadores.objects.filter(
            ...
            deslocamentos = (
                ...
                .select_related(
                    "formador__usuario", "municipio_origem", "municipio_destino"
                )
                .values(
                    "formador__usuario__first_name",
                    "formador__usuario__last_name",
                    "data",
                    "municipio_origem__nome",
                    "municipio_destino__nome",
    ...
    def get_monthly_availability_map(ano: int, mes: int):
        ...
        def fetch_data():
            ...
            formadores = (
                Formador.objects.filter(ativo=True)
    ...
    def get_dashboard_analytics():
        ...
        def fetch_data():
            ...
            recent_activity = (
                Solicitacao.objects.filter(data_solicitacao__gte=thirty_days_ago)
            ...
            top_formadores = (
                ...
                .annotate(
                    eventos_count=Count(
                        "solicitacao", filter=Q(solicitacao__status="Aprovado")
                    )
                )
                .filter(eventos_count__gt=0)
                .select_related("usuario")
                .order_by("-eventos_count")[:5]
                .values("usuario__first_name", "usuario__last_name", "eventos_count")
            )
    ...
    def invalidate_related_caches(model_name: str, obj_id: Optional[str] = None):
        ...
        if model_name.lower() == "solicitacao":
            patterns_to_clear.extend(
                ["solicitacoes:*", "dashboard:*", "analytics:*", "conflicts:*"]
            )
        elif model_name.lower() == "formador":
            patterns_to_clear.extend(
                ["formadores:*", "conflicts:*", "availability_map:*"]
            )
        elif model_name.lower() in ["municipio", "projeto", "tipoevento"]:
            patterns_to_clear.extend(["dashboard:*", "analytics:*"])
