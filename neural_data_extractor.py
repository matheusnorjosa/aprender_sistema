...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('neural_extraction.log'),
...
class NeuralDataExtractor:
    ...
    def _process_worksheet_data(self, all_values: List[List[str]], tab_name: str) -> Dict[str, Any]:
        """
        ...
        for row_idx, row in enumerate(data_rows, start=2):  # Começar da linha 2
            try:
                ...
                if self._validate_record(record, tab_name):
                    ...
                    warning_msg = f"Registro inválido na linha {row_idx} da aba {tab_name}"
                    logger.warning(warning_msg)
                    self.extraction_stats['warnings'].append(warning_msg)
                    
            except Exception as e:
                error_msg = f"Erro ao processar linha {row_idx} da aba {tab_name}: {str(e)}"
                logger.error(error_msg)
                self.extraction_stats['errors'].append(error_msg)
    ...
    def _analyze_data_types(self, records: List[Dict[str, str]], headers: List[str]) -> Dict[str, str]:
        """
    ...
    def save_extraction(self, data: Dict[str, Any], output_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"📊 Estatísticas: {self.extraction_stats['files_processed']} arquivos, "
