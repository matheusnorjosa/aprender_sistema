...
from core.models import TipoEvento, LogAuditoria
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            # Processar dados
            resultado = self.process_tipos_evento(tipos_data, dry_run, user, update_existing)
        ...
        try:
            with open(arquivo, 'r', encoding=encoding, newline='') as csvfile:
                ...
                for row_num, row in enumerate(reader, 1):
                    ...
                    nome = self.extract_field(normalized_row, ['nome', 'tipo', 'tipo_evento', 'evento', 'title'])
                    online_str = self.extract_field(normalized_row, ['online', 'modalidade', 'formato', 'tipo_modalidade'], 'False')
                    ativo_str = self.extract_field(normalized_row, ['ativo', 'status', 'ativo_inativo'], 'True')
                    
                    if not nome:
                        self.stdout.write(self.style.WARNING(f'Linha {row_num}: Nome do tipo de evento nao encontrado. Pulando.'))
                        continue
        ...
        try:
            with open(arquivo, 'r', encoding='utf-8') as jsonfile:
                ...
                if isinstance(data, list):
                    ...
                    raise CommandError('Formato JSON invalido. Esperado lista ou {tipos_evento: [...]} ou {tipos: [...]}')
                
                tipos = []
                
                for item in tipos_data:
                    nome = item.get('nome', '').strip()
                    online = self.parse_online_field(item.get('online', item.get('modalidade', False)))
    ...
    def load_from_excel(self, arquivo):
        ...
        try:
            ...
            for row_num, row in enumerate(sheet.iter_rows(min_row=2), 2):
                ...
                online_str = self.extract_field(row_data, ['online', 'modalidade', 'formato'], 'False')
    ...
    def parse_online_field(self, value):
        ...
        if isinstance(value, str):
            ...
            online_values = [
                'true', '1', 'sim', 'online', 'virtual', 'remoto', 'ead', 
                'distancia', 'web', 'digital', 'internet'
            ]
            # Valores que indicam modalidade presencial
            presencial_values = [
                'false', '0', 'nao', 'presencial', 'fisico', 'local', 
                'sala', 'auditorio', 'classroom'
            ]
    ...
    def process_tipos_evento(self, tipos_data, dry_run, user, update_existing):
        ...
        for tipo_data in tipos_data:
            try:
                ...
                if len(nome) > 255:
                    resultado['erros'] += 1
                    resultado['detalhes_erros'].append(f'Nome muito longo: {nome[:50]}...')
                    continue
                ...
                if not dry_run:
                    ...
                    if created:
                        ...
                        if update_existing:
                            ...
                            if updated:
                                ...
                                modalidade = '[ONLINE]' if online else '[PRESENCIAL]'
                                self.stdout.write(f'  [~] Atualizado: {nome} {modalidade}')
                            else:
                                resultado['duplicados'] += 1
                                self.stdout.write(f'  [=] Ja existe (sem alteracoes): {nome}')
                        else:
                            resultado['duplicados'] += 1
                            modalidade = '[ONLINE]' if tipo_evento.online else '[PRESENCIAL]'
                    ...
                    if exists:
                        ...
                        modalidade = '[ONLINE]' if online else '[PRESENCIAL]'
                        status = '[ATIVO]' if ativo else '[INATIVO]'
                        self.stdout.write(f'  [+] [DRY] Seria criado: {nome} {modalidade} {status}')
            
            except Exception as e:
                resultado['erros'] += 1
                resultado['detalhes_erros'].append(f'Erro ao processar {nome}: {str(e)}')
