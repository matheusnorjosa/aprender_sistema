...
class DeslocamentoListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = (
            Deslocamento.objects.select_related()
            .prefetch_related(
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {
                "formadores": FormadorService.get_formadores_queryset().order_by(
                    "nome"
                ),
                "filtros_ativos": any(
                    [
...
class DeslocamentoCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Deslocamento criado com sucesso: {self.object}"
        )
        return response
...
class DeslocamentoAPI(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        try:
                ...
                deslocamentos.append(
                    {
                        ...
                        "pessoas": [
                            getattr(
                                p,
                                "nome_completo",
                                p.nome if hasattr(p, "nome") else str(p),
                            )
                            for p in desloc.pessoas
                        ],
            ...
            return JsonResponse(
                {"success": False, "error": f"Erro ao buscar deslocamentos: {str(e)}"}
            )
        ...
        try:
            ...
            for field in required_fields:
                if not data.get(field):
                    return JsonResponse(
                        {"success": False, "error": f"Campo obrigatório: {field}"}
                    )
            ...
            return JsonResponse(
                {"success": False, "error": f"Erro ao criar deslocamento: {str(e)}"}
            )
