...
class NeuralPostgreSQLImporter:
    ...
    def create_schema(self) -> bool:
        ...
        try:
            ...
            if not schema_file.exists():
                logger.error("❌ Arquivo de schema não encontrado: database_schema_design.sql")
                return False
                    ...
                    if 'pessoas_unicas' in data:
                        for pessoa_info in data['pessoas_unicas']:
                            pessoas_data.append({
                                ...
                                'papel': pessoa_info.get('papel_principal', 'formador'),
                                'ativo': True
                            })
    ...
    def _import_projetos_from_analysis(self):
        ...
        projetos_data = [
            # Superintendência
            {'nome': 'Projetos Superintendência', 'categoria': 'superintendencia', 'tipo': 'colecao'},
            
            # ACerta
            {'nome': 'ACerta', 'categoria': 'acerta', 'tipo': 'colecao'},
            
            # Brincando
            {'nome': 'Brincando e Aprendendo', 'categoria': 'brincando', 'tipo': 'colecao'},
            ...
            {'nome': 'A Cor da Gente', 'categoria': 'projetos_especificos', 'tipo': 'colecao'},
            {'nome': 'Sou da Paz', 'categoria': 'projetos_especificos', 'tipo': 'colecao'},
            {'nome': 'TRÂNSITO LEGAL', 'categoria': 'projetos_especificos', 'tipo': 'colecao'},
            {'nome': 'ED FINANCEIRA', 'categoria': 'projetos_especificos', 'tipo': 'colecao'},
            {'nome': 'LER, OUVIR E CONTAR', 'categoria': 'projetos_especificos', 'tipo': 'colecao'},
            {'nome': 'LEIO ESCREVO E CALCULO', 'categoria': 'projetos_especificos', 'tipo': 'colecao'},
            ...
            {'nome': 'IDEB10 - ESQUENTA SAEB', 'categoria': 'projetos_especificos', 'tipo': 'evento'},
        ]
    ...
    def _import_atribuicoes_from_analysis(self):
        ...
        atribuicoes_data = [
            ...
            {'pessoa_nome': 'Ana Paula', 'projeto': 'ACerta', 'papel': 'coordenador', 'municipio': 'Recife'},
            {'pessoa_nome': 'Ana Paula', 'projeto': 'Vida e Ciências', 'papel': 'coordenador', 'municipio': 'Jaboatão dos Guararapes'},
            {'pessoa_nome': 'Ana Paula', 'projeto': 'Vida e Matemática', 'papel': 'coordenador', 'municipio': 'Olinda'},
            {'pessoa_nome': 'Ana Paula', 'projeto': 'Vida e Linguagem', 'papel': 'coordenador', 'municipio': 'Caruaru'},
            
            # Gerentes
            {'pessoa_nome': 'Gleice Anne', 'projeto': 'Brincando e Aprendendo', 'papel': 'gerente', 'municipio': None},
            
            # Formadores (exemplos)
            {'pessoa_nome': 'João Silva', 'projeto': 'ACerta', 'papel': 'formador', 'municipio': 'Recife'},
            {'pessoa_nome': 'Maria Santos', 'projeto': 'Vida e Ciências', 'papel': 'formador', 'municipio': 'Jaboatão dos Guararapes'},
            {'pessoa_nome': 'Pedro Costa', 'projeto': 'Vida e Matemática', 'papel': 'formador', 'municipio': 'Olinda'},
        ]
        ...
        try:
            for pessoa in pessoas_data:
                ...
                """, {
                    ...
                    'primeiro_nome': self._extract_first_name(pessoa['nome_completo']),
    ...
    def _insert_projetos(self, projetos_data: List[Dict[str, Any]]):
        ...
        try:
            for projeto in projetos_data:
                # Obter categoria_id
                categoria_id = self._get_or_create_categoria_projeto(projeto['categoria'])
    ...
    def _insert_atribuicoes(self, atribuicoes_data: List[Dict[str, Any]]):
        ...
        try:
            for atribuicao in atribuicoes_data:
                ...
                if not pessoa_id:
                    logger.warning(f"⚠️ Pessoa não encontrada: {atribuicao['pessoa_nome']}")
                    continue
    ...
    def _get_or_create_categoria_projeto(self, categoria: str) -> int:
        ...
        cursor.execute("SELECT id FROM categorias_projetos WHERE codigo = %s", (categoria,))
        result = cursor.fetchone()
    ...
    def _get_pessoa_by_name(self, nome: str) -> Optional[int]:
        ...
        cursor = self.connection.cursor()
        cursor.execute("SELECT id FROM pessoas WHERE nome_completo ILIKE %s", (f"%{nome}%",))
        result = cursor.fetchone()
    ...
    try:
        ...
        try:
            ...
            importer.import_stats['duration_seconds'] = (
                importer.import_stats['end_time'] - 
                importer.import_stats['start_time']
            ).total_seconds()
