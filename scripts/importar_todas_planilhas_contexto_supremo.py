...
class ImportadorConsolidadoContextoSupremo:
    ...
    def executar_importacao_completa(self, planilhas_especificas: Optional[List[str]] = None):
        """
        ...
        logger.info(f"📋 Planilhas a serem processadas: {', '.join(planilhas_para_processar)}")
        ...
        for planilha in planilhas_para_processar:
            if planilha in self.scripts:
                ...
                if resultado['status'] == 'sucesso':
                    ...
                    if resultado['relatorio'] and 'estatisticas' in resultado['relatorio']:
                        stats = resultado['relatorio']['estatisticas']
                        self.estatisticas_consolidadas['total_registros'] += stats.get('total_registros', 0)
                        self.estatisticas_consolidadas['total_erros'] += stats.get('erros', 0)
                else:
        ...
        self.estatisticas_consolidadas['duracao_segundos'] = (fim - inicio).total_seconds()
    ...
    def gerar_relatorio_consolidado(self):
        ...
        logger.info(f"⏱️ DURAÇÃO TOTAL: {self.estatisticas_consolidadas['duracao_segundos']:.2f} segundos")
        logger.info(f"📋 PLANILHAS PROCESSADAS: {self.estatisticas_consolidadas['planilhas_processadas']}/{len(self.scripts)}")
        logger.info(f"📊 TOTAL DE REGISTROS: {self.estatisticas_consolidadas['total_registros']}")
        logger.info(f"❌ TOTAL DE ERROS: {self.estatisticas_consolidadas['total_erros']}")
        ...
        with open(relatorio_path, 'w', encoding='utf-8') as f:
            json.dump(self.estatisticas_consolidadas, f, indent=2, ensure_ascii=False, default=str)
...
def main():
    ...
    parser.add_argument('--planilhas', nargs='+', 
    ...
    resultado = importador.executar_importacao_completa(planilhas_especificas=args.planilhas)
    
    if resultado['total_erros'] == 0:
        logger.info("🎉 IMPORTAÇÃO CONSOLIDADA CONCLUÍDA COM SUCESSO!")
        sys.exit(0)
    else:
        logger.error(f"❌ IMPORTAÇÃO CONSOLIDADA CONCLUÍDA COM {resultado['total_erros']} ERRO(S)!")
        sys.exit(1)
