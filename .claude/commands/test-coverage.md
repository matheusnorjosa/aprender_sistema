---
description: Run tests with coverage report and enforce the 85% gate
...
**AS v2 Standards**:
- **Overall gate**: 85% coverage required (CI `--cov-fail-under=85`)
...
#### solicitacao_approval.py (PA-01 to PA-07)
...
Coverage runs in CI via the reusable backend workflow `.github/workflows/_backend-test.yml`
(callable from the backend gate). It runs pytest under `pytest-xdist` (`-n auto --dist loadscope`)
on Python 3.12 inside Docker and enforces the **85%** gate (`--cov-fail-under=85`). Do not duplicate
the pipeline in an ad-hoc workflow — reference the reusable workflow instead.
...
5. Re-run coverage
6. Repeat until the 85% gate is met (critical modules: 100%)
...
3. Re-run coverage until the 85% gate is met
...
- ✅ Meet the 85% overall gate, 100% critical
...
| `--cov-fail-under=85` | Fail if below the 85% gate |
...
**Focus**: Enforce quality gate (85%), test behavior, cover critical paths.
