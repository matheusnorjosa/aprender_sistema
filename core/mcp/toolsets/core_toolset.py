...
from core.models import Usuario, Formador, Municipio, Projeto, TipoEvento
...
class CoreModelToolset(ModelQueryToolset):
    ...
    accessible_fields = {
        'Usuario': [
            ...
            'is_active', 'date_joined', 'groups', 'primary_role'
        ],
        'Formador': [
            'id', 'nome', 'email', 'ativo', 'area_atuacao', 'usuario'
        ],
        'Municipio': [
            'id', 'nome', 'uf', 'ativo'
        ],
        'Projeto': [
            'id', 'nome', 'descricao', 'ativo', 'vinculado_superintendencia'
        ],
        'TipoEvento': [
            'id', 'nome', 'online', 'ativo'
        ],
    }
    ...
    def get_queryset(self, model):
        ...
        if model == Usuario:
            ...
            return model.objects.filter(ativo=True).select_related('usuario', 'area_atuacao')
        else:
            # Default: return active records
            return model.objects.filter(ativo=True) if hasattr(model, 'ativo') else model.objects.all()
    ...
    def can_create(self, model, user=None):
        ...
        if model == Usuario:
            ...
            return user.has_perm('core.add_formador') or user.groups.filter(name__in=['admin', 'superintendencia']).exists()
        else:
    ...
    def can_update(self, model, obj, user=None):
        ...
        if model == Formador and hasattr(user, 'formador_profile') and obj == user.formador_profile:
            return True
        
        # Admin and superintendencia can update most things
        if user.is_superuser or user.groups.filter(name__in=['admin', 'superintendencia']).exists():
            return True
