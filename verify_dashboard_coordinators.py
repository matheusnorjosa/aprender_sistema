...
def verify_rondonopolis_data():
    ...
    for solicitacao in solicitacoes:
        if solicitacao.usuario_solicitante and solicitacao.usuario_solicitante.cargo == 'coordenador':
    ...
    for nome, count in sorted(coordenadores_count.items(), key=lambda x: x[1], reverse=True):
...
def verify_control_sheet_rondonopolis():
    ...
    for item in rondonopolis_data:
        ...
        if coordenador:
            coordenadores_planilha[coordenador] = coordenadores_planilha.get(coordenador, 0) + 1
    
    print(f"\n👨‍💼 COORDENADORES NA PLANILHA:")
    for nome, count in sorted(coordenadores_planilha.items(), key=lambda x: x[1], reverse=True):
...
def compare_data(dashboard_data, planilha_data):
    ...
    for coord in sorted(both):
        ...
        print(f"   {status} {coord}: Dashboard={dashboard_count}, Planilha={planilha_count} (diff={diff:+d})")
