        ...
        if file_path.exists():
            ...
            try:
                ...
                if 'usuarios' in data:
                    ...
                    usuarios_bahia = [u for u in usuarios if 'BA' in str(u.get('UF', ''))]
                    print(f"   🌴 Usuários da Bahia: {len(usuarios_bahia)}")
                    
                    # Verificar Atibaia
                    usuarios_atibaia = [u for u in usuarios if 'Atibaia' in str(u.get('Municipio', ''))]
                    ...
                    eventos_bahia = [e for e in eventos if 'BA' in str(e.get('dados', {}).get('municipio', ''))]
                    print(f"   🌴 Eventos da Bahia: {len(eventos_bahia)}")
                    
                    # Verificar Atibaia
                    eventos_atibaia = [e for e in eventos if 'Atibaia' in str(e.get('dados', {}).get('municipio', ''))]
                            ...
                            for aba in planilha_data['abas']:
                                if 'dados' in aba:
                                    ...
                                    if dados:
                                        ...
                                        dados_bahia = [d for d in dados if 'BA' in str(d.get('municipio', ''))]
                                        if dados_bahia:
                                            print(f"     🌴 {planilha_nome} - {aba['nome']}: {len(dados_bahia)} registros da Bahia")
                                        
                                        # Verificar Atibaia
                                        dados_atibaia = [d for d in dados if 'Atibaia' in str(d.get('municipio', ''))]
                                        if dados_atibaia:
                                            print(f"     🏔️ {planilha_nome} - {aba['nome']}: {len(dados_atibaia)} registros de Atibaia")
...
def investigate_solicitations_by_municipality():
    ...
    solicitacoes_por_municipio = Solicitacao.objects.values(
    ...
    municipios_sem_solicitacoes = Municipio.objects.exclude(
...
def investigate_missing_municipalities():
    ...
    municipios_esperados = [
        'Atibaia', 'São Paulo', 'Santos', 'Campinas', 'Ribeirão Preto',
        'Salvador', 'Feira de Santana', 'Vitória da Conquista', 'Camaçari',
        'Fortaleza', 'Caucaia', 'Sobral', 'Juazeiro do Norte', 'Maracanaú'
    ]
