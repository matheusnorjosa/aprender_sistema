...
- Idempotência: SHA1(municipio_id|projeto_id|data_entrega|data_reuniao)
...
def _process_row(row: Dict[str, Any], idx: int, stats: dict, pendencias: dict) -> Optional[str]:
    ...
    # Criar external_hash
    hash_key = f"{municipio.id}|{projeto.id}|{data_entrega}|{data_reuniao}"
    ...
    obj, created = AcaoControle.objects.update_or_create(
    ...
    if created:
        ...
        for k, v in defaults.items():
            if getattr(obj, k) != v:
                changed = True
                break
        if changed or obj.municipio != municipio or obj.projeto != projeto:
            stats["updated"] += 1
        else:
            stats["unchanged"] += 1
