    ...
    def get_or_create_municipio(self, nome_municipio):
        ...
        try:
            ...
            municipio = Municipio.objects.create(
                nome=nome_limpo, estado="CE"  # Padrão Ceará
            )
    ...
    def get_or_create_tipo_evento(self, nome_tipo):
        ...
        if not nome_tipo or nome_tipo.strip() == "":
            ...
            return TipoEvento.objects.get_or_create(
                nome="Formação", defaults={"descricao": "Tipo padrão de formação"}
            )[0]
    ...
    def processar_linha_super(self, linha, linha_num):
        ...
        try:
            ...
            dados_solicitacao = {
                ...
                "observacoes": (
                    f"Importado da aba Super - {observacoes}"
                    if observacoes
                    else "Importado da aba Super"
                ),
                "status": status,
    ...
    def importar_aba_super(self, dry_run=False):
        ...
        try:
            ...
            dados = google_sheets_service.get_worksheet_data(
                spreadsheet_key=spreadsheet_key, worksheet_name="Super"
            )
                    ...
                    try:
                        with transaction.atomic():
                            ...
                            if resultado["formador"]:
                                solicitacao.formadores_solicitados.add(
                                    resultado["formador"]
                                )
