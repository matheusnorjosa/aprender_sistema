...
def test_google_calendar_integration():
    ...
    vars_check = [
        ...
        ('GOOGLE_CALENDAR_CALENDAR_ID', getattr(settings, 'GOOGLE_CALENDAR_CALENDAR_ID', 'primary')),
    ...
    print(f'OK Data: {sol.data_inicio.strftime("%d/%m/%Y %H:%M")} - {sol.data_fim.strftime("%d/%m/%Y %H:%M")}')
...
{chr(10).join([f"- {f.nome}" for f in sol.formadores.all()])}
    ...
    print(f'  - Request ID: {event_data["conferenceData"]["createRequest"]["requestId"]}')
    ...
    for item, status in checklist:
        status_symbol = 'OK' if status is True else 'INFO' if isinstance(status, str) else 'X'
        print(f'  {status_symbol} {item}: {status if isinstance(status, str) else "Sim" if status else "Não"}')
