...
def analyze_security_settings():
    ...
    security_config = {
        ...
        'SECRET_KEY_PREFIX': getattr(settings, 'SECRET_KEY', '')[:20] + '...' if hasattr(settings, 'SECRET_KEY') else None,
        'SECRET_KEY_SECURE': not getattr(settings, 'SECRET_KEY', '').startswith('django-insecure-'),
...
if __name__ == '__main__':
    ...
    result = {
        ...
        'risk_level': 'HIGH' if any(i['level'] == 'HIGH' for i in issues) else 'MEDIUM' if issues else 'LOW'
    }
