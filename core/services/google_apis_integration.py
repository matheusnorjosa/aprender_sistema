...
class GoogleAPIsManager:
    ...
    def __init__(self):
        ...
        if not self.enabled:
            logger.warning(
                "Google APIs not available - install google-api-python-client"
            )
            return
    ...
    def _setup_credentials(self):
        ...
        try:
            ...
            credentials_file = getattr(
                settings, "GOOGLE_CREDENTIALS_FILE", "credentials.json"
            )
...
class AdvancedCalendarService:
    ...
    def update_educational_event(
        self, solicitacao: Solicitacao, calendar_id: str = "primary"
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            event = (
                self.calendar_service.events()
                .get(calendarId=calendar_id, eventId=solicitacao.evento_google)
                .execute()
            )
            ...
            return {"event_id": updated_event["id"], "status": "updated"}
    ...
    def delete_educational_event(
        ...
        try:
            self.calendar_service.events().delete(
                calendarId=calendar_id, eventId=solicitacao.evento_google
            ).execute()
    ...
    def get_calendar_conflicts(
        self, start_time: datetime, end_time: datetime, calendar_ids: List[str] = None
    ) -> List[Dict[str, Any]]:
        """
...
class GoogleDriveService:
    ...
    def create_event_folder(
        self, solicitacao: Solicitacao, parent_folder_id: Optional[str] = None
    ) -> Optional[str]:
        """
    ...
    def _set_folder_permissions(self, folder_id: str, solicitacao: Solicitacao):
        ...
        try:
            ...
            if not solicitacao.formadores.filter(
...
Sistema Aprender
    ...
    def _get_notification_recipients(
        self, solicitacao: Solicitacao, notification_type: str
    ) -> List[Dict[str, str]]:
...
def create_calendar_event(
    solicitacao: Solicitacao, add_meet: bool = True
) -> Optional[Dict[str, Any]]:
    """Create calendar event for solicitacao"""
    return calendar_service.create_educational_event(
        solicitacao, add_meet_link=add_meet
    )
...
def send_notification_email(
    solicitacao: Solicitacao, notification_type: str, message: Optional[str] = None
) -> bool:
    """Send notification email"""
    return gmail_service.send_event_notification(
        solicitacao, notification_type, message
    )
