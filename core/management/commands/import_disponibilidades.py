...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            user = None
            self.stdout.write(
                self.style.WARNING(f'Usuario "{username}" nao encontrado. Auditoria sera registrada sem usuario.')
            )
        ...
        self.stdout.write(f'Atualizar existentes: {"Sim" if update_existing else "Nao"}')
        if formador_filter:
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING('MODO DRY-RUN: Nenhuma alteracao sera salva no banco'))
        ...
        try:
            ...
            # Importar dados
            if formato == 'csv':
                disponibilidades_data = self.load_from_csv(arquivo, encoding, date_format)
            elif formato == 'json':
            ...
            if formador_filter:
                disponibilidades_data = self.filter_by_formador(disponibilidades_data, formador_filter)
            
            # Processar dados
            resultado = self.process_disponibilidades(disponibilidades_data, dry_run, user, update_existing)
        ...
        try:
            with open(arquivo, 'r', encoding=encoding, newline='') as csvfile:
                ...
                for row_num, row in enumerate(reader, 1):
                    ...
                    try:
                        disp_data = self.parse_row_data(normalized_row, date_format, row_num)
                        if disp_data:
                            disp_data['linha'] = row_num
                            disponibilidades.append(disp_data)
                    except Exception as e:
                        self.stdout.write(self.style.WARNING(f'Linha {row_num}: Erro ao processar - {e}'))
                        continue
        ...
        try:
            with open(arquivo, 'r', encoding='utf-8') as jsonfile:
                ...
                if isinstance(data, list):
                    ...
                    raise CommandError('Formato JSON invalido. Esperado lista ou {disponibilidades: [...]} ou {bloqueios: [...]}')
                ...
                for item in disponibilidades_data:
                    try:
                        ...
                        self.stdout.write(self.style.WARNING(f'Item JSON: Erro ao processar - {e}'))
                        continue
    ...
    def load_from_excel(self, arquivo, date_format):
        ...
        try:
            ...
            raise CommandError('openpyxl nao esta instalado. Execute: pip install openpyxl')
        ...
        try:
            ...
            for row_num, row in enumerate(sheet.iter_rows(min_row=2), 2):
                row_data = {headers[i]: cell.value if cell.value else '' for i, cell in enumerate(row) if i < len(headers)}
    ...
    def parse_row_data(self, row_data, date_format, row_num):
        ...
        # Extrair campos
        formador_nome = self.extract_field(row_data, ['formador', 'nome_formador', 'nome', 'instrutor'])
        formador_email = self.extract_field(row_data, ['email', 'email_formador', 'formador_email'])
        data_str = self.extract_field(row_data, ['data', 'data_bloqueio', 'date'])
        hora_inicio_str = self.extract_field(row_data, ['hora_inicio', 'inicio', 'start', 'de'])
        hora_fim_str = self.extract_field(row_data, ['hora_fim', 'fim', 'end', 'ate'])
        tipo_bloqueio = self.extract_field(row_data, ['tipo', 'tipo_bloqueio', 'categoria', 'motivo_categoria'])
        motivo = self.extract_field(row_data, ['motivo', 'observacao', 'descricao', 'detalhes'])
        
        if not (formador_nome or formador_email):
            self.stdout.write(self.style.WARNING(f'Linha {row_num}: Formador nao identificado. Pulando.'))
            return None
        
        if not data_str:
            self.stdout.write(self.style.WARNING(f'Linha {row_num}: Data nao encontrada. Pulando.'))
            return None
        ...
        # Parse de horários
        hora_inicio = self.parse_time_field(hora_inicio_str) if hora_inicio_str else time(9, 0)
        hora_fim = self.parse_time_field(hora_fim_str) if hora_fim_str else time(17, 0)
        
        if hora_fim <= hora_inicio:
            raise ValueError(f'Hora fim ({hora_fim}) deve ser posterior a hora inicio ({hora_inicio})')
    ...
    def parse_json_data(self, item, date_format):
        """Parse de dados JSON"""
        # Extrair campos JSON
        formador_nome = item.get('formador') or item.get('nome_formador') or item.get('nome')
        ...
        if not (formador_nome or formador_email):
            self.stdout.write(self.style.WARNING(f'Item JSON: Formador nao identificado. Pulando.'))
            return None
        
        if not data_str:
            self.stdout.write(self.style.WARNING(f'Item JSON: Data nao encontrada. Pulando.'))
            return None
    ...
    def filter_by_formador(self, disponibilidades_data, formador_filter):
        ...
        formador = self.get_formador_by_name_or_email(formador_filter)
        if not formador:
            raise CommandError(f'Formador "{formador_filter}" nao encontrado para filtro')
        
        filtered_data = []
        for disp_data in disponibilidades_data:
            if (disp_data['formador_nome'] and formador.nome.lower() in disp_data['formador_nome'].lower()) or \
               (disp_data['formador_email'] and formador.email.lower() == disp_data['formador_email'].lower()):
                filtered_data.append(disp_data)
        
        self.stdout.write(f'Filtradas {len(filtered_data)} disponibilidades para {formador.nome}')
        return filtered_data
    
    def process_disponibilidades(self, disponibilidades_data, dry_run, user, update_existing):
        ...
        for disp_data in disponibilidades_data:
            try:
                ...
                if disp_data['formador_email']:
                    formador = self.get_formador_by_name_or_email(disp_data['formador_email'])
                if not formador and disp_data['formador_nome']:
                    formador = self.get_formador_by_name_or_email(disp_data['formador_nome'])
                
                if not formador:
                    resultado['erros'] += 1
                    resultado['detalhes_erros'].append(f'Formador nao encontrado: {disp_data["formador_nome"]} / {disp_data["formador_email"]}')
                    continue
                ...
                if not dry_run:
                    ...
                    disponibilidade, created = DisponibilidadeFormadores.objects.get_or_create(
                        formador=formador,
                        data_bloqueio=data_bloqueio,
                        hora_inicio=hora_inicio,
                        hora_fim=hora_fim,
                    ...
                    if created:
                        resultado['criados'] += 1
                        self.stdout.write(f'  [+] Criado: {formador.nome} - {data_bloqueio} {hora_inicio}-{hora_fim} [{tipo_bloqueio}]')
                    else:
                        ...
                        if update_existing:
                            ...
                            if updated:
                                ...
                                self.stdout.write(f'  [~] Atualizado: {formador.nome} - {data_bloqueio} {hora_inicio}-{hora_fim}')
                            else:
                                resultado['duplicados'] += 1
                                self.stdout.write(f'  [=] Ja existe (sem alteracoes): {formador.nome} - {data_bloqueio}')
                        else:
                            resultado['duplicados'] += 1
                            self.stdout.write(f'  [=] Ja existe: {formador.nome} - {data_bloqueio} {hora_inicio}-{hora_fim}')
                else:
                    ...
                    if exists:
                        if update_existing:
                            resultado['atualizados'] += 1
                            self.stdout.write(f'  [~] [DRY] Seria atualizado: {formador.nome} - {data_bloqueio}')
                        else:
                            resultado['duplicados'] += 1
                            self.stdout.write(f'  [=] [DRY] Ja existe: {formador.nome} - {data_bloqueio}')
                    else:
                        resultado['criados'] += 1
                        self.stdout.write(f'  [+] [DRY] Seria criado: {formador.nome} - {data_bloqueio} {hora_inicio}-{hora_fim} [{tipo_bloqueio}]')
            
            except Exception as e:
                resultado['erros'] += 1
                formador_info = disp_data.get('formador_nome', 'Desconhecido')
                resultado['detalhes_erros'].append(f'Erro ao processar {formador_info}: {str(e)}')
                self.stdout.write(self.style.ERROR(f'  [X] Erro: {formador_info} - {e}'))
    ...
    def display_result(self, resultado):
        ...
        if resultado["erros"] > 0:
            ...
            for erro in resultado["detalhes_erros"][:10]:  # Mostrar apenas os primeiros 10
                self.stdout.write(f'  - {erro}')
            if len(resultado["detalhes_erros"]) > 10:
                self.stdout.write(f'  ... e mais {len(resultado["detalhes_erros"]) - 10} erros')
        ...
        if sucesso > 0 and resultado["erros"] == 0:
            ...
            self.stdout.write(self.style.WARNING('Importacao concluida com alguns erros'))
        else:
