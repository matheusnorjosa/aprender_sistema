...
class Command(BaseCommand):
    ...
    def reset_completo_tabelas(self):
        ...
        with transaction.atomic():
            ...
            for nome_tabela, modelo in modelos_para_limpar:
                try:
                    if modelo == User:
                        ...
                        usuarios_removidos = User.objects.exclude(
                            is_superuser=True
                        ).count()
                        User.objects.exclude(is_superuser=True).delete()
                        self.logger.info(
                            f"  ✅ {nome_tabela}: {usuarios_removidos} removidos (admin mantido)"
                        )
                    else:
                        count = modelo.objects.count()
                        modelo.objects.all().delete()
                        self.logger.info(
                            f"  ✅ {nome_tabela}: {count} registros removidos"
                        )
            ...
            self.logger.info(
                f"  ✅ Groups extra: {grupos_removidos} removidos (essenciais mantidos)"
            )
    ...
    def verificar_estado_final(self):
        ...
        try:
            ...
            if usuarios == 1:
                ...
                self.logger.info(
                    f"Único usuário: {admin.username} (superuser: {admin.is_superuser})"
                )
    ...
    def handle(self, *args, **options):
        ...
        if not options["confirmar_reset_total"]:
            ...
            self.logger.error(
                "Use --confirmar-reset-total para confirmar o reset completo"
            )
            raise CommandError(
                "Reset cancelado. Use --confirmar-reset-total para prosseguir."
            )
