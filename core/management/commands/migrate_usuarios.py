...
class Command(BaseMigrationCommand):
    ...
    def migrate_data(self, source_file: str, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        worksheet_name = options.get('worksheet', 'Ativos')  # Default para usuários ativos
        ...
        if usuarios_to_create and not self.dry_run:
            groups_assigned = self.assign_users_to_groups(usuarios_to_create, worksheet_name)
    ...
    def assign_users_to_groups(self, usuarios: list, worksheet_name: str) -> int:
        ...
        try:
            ...
            emails = [u.get('email') for u in usuarios if isinstance(u, dict) and u.get('email')]
            ...
            if emails:
                ...
                for user in created_users:
                    ...
                    assigned_count += 1
                    self.log_success(f"Usuário {user.email} vinculado ao grupo '{group_name}'")
            
        except Group.DoesNotExist:
            self.log_error(f"Grupo '{group_name}' não encontrado. Execute setup_groups primeiro.")
        except Exception as e:
    ...
    def validate_data(self, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            missing_worksheets = [ws for ws in required_worksheets if ws not in worksheets]
