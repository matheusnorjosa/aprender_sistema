            ...
            if first_chars.strip().startswith('['):
                ...
                if isinstance(data, list):
                    results['structure_analysis'] = {
                        'type': 'array',
                        'length': len(data),
                        'first_item_type': type(data[0]).__name__ if data else 'unknown'
                    }

                    # Analyze first item if it's a dict
                    if data and isinstance(data[0], dict):
                        first_item = data[0]
                        results['structure_analysis']['first_item_keys'] = list(first_item.keys())

                        # Chat-specific analysis
                        if 'chat' in first_item and isinstance(first_item['chat'], dict):
                            chat = first_item['chat']
                            results['structure_analysis']['chat_structure'] = {
                                'has_chat': True,
                                'chat_keys': list(chat.keys()),
                                'has_history': 'history' in chat,
                                'has_messages': 'history' in chat and 'messages' in chat['history']
                            }
                            ...
                            if chat.get('history', {}).get('messages'):
                                ...
                                results['structure_analysis']['message_count'] = message_count
                                ...
                                results['structure_analysis']['roles_found'] = list(set(roles))
                        ...
                        if 'chat' in sample_item and 'history' in sample_item['chat']:
                            ...
                            if 'messages' in sample_item['chat']['history']:
                                ...
                                if len(messages) > 2:
                                    ...
                                    sample_item['chat']['history']['_truncated'] = f"Showing 2 of {len(messages)} messages"

                        results['sample_data'] = {
                            'first_chat_sample': sample_item
                        }
        ...
        if isinstance(data, list) and data:
            ...
            for i, chat_export in enumerate(data):
                if isinstance(chat_export, dict):
                    ...
                    # Process messages
                    if ('chat' in chat_export and
                        'history' in chat_export['chat'] and
                        'messages' in chat_export['chat']['history']):
                        ...
                        for msg_id, msg in messages.items():
                            if isinstance(msg, dict):
                                ...
                                if 'files' in msg and msg['files']:
                                    results['chunk_analysis']['file_attachments'] += len(msg['files'])

                                # Track content types
                                if 'content' in msg:
                                    content = msg['content']
                                    if '<details type="reasoning"' in content:
                                        results['chunk_analysis']['content_types'].add('reasoning')
                                    if 'Sistema APRENDER' in content:
                                        results['chunk_analysis']['content_types'].add('system_analysis')
                                    if len(content) > 1000:
                                        results['chunk_analysis']['content_types'].add('long_form')
            ...
            results['chunk_analysis']['content_types'] = list(results['chunk_analysis']['content_types'])
            results['chunk_analysis']['models_used'] = list(results['chunk_analysis']['models_used'])
            ...
            results['content_summary'] = {
                ...
                'avg_messages_per_chat': round(results['chunk_analysis']['total_messages'] / max(results['chunk_analysis']['chunks_processed'], 1), 1),
                'has_file_attachments': results['chunk_analysis']['file_attachments'] > 0,
                'has_reasoning_traces': 'reasoning' in results['chunk_analysis']['content_types']
            }
...
def main():
    ...
    print(f"  - File size: {structure_result.get('file_info', {}).get('size_mb', 'unknown')} MB")
    print(f"  - Processing time: {structure_result.get('performance_metrics', {}).get('processing_time_seconds', 'unknown')} seconds")
    print(f"  - Structure type: {structure_result.get('structure_analysis', {}).get('type', 'unknown')}")
    print(f"  - Array length: {structure_result.get('structure_analysis', {}).get('length', 'unknown')}")
    print()

    print("Chunked Processing Analysis:")
    print(f"  - Processing time: {chunked_result.get('performance_metrics', {}).get('processing_time_seconds', 'unknown')} seconds")
    print(f"  - Total chats: {chunked_result.get('chunk_analysis', {}).get('total_chats', 'unknown')}")
    print(f"  - Total messages: {chunked_result.get('chunk_analysis', {}).get('total_messages', 'unknown')}")
    print(f"  - Content types: {chunked_result.get('chunk_analysis', {}).get('content_types', [])}")
    print(f"  - Models used: {chunked_result.get('chunk_analysis', {}).get('models_used', [])}")
    print()
    ...
    for topic in content_summary.get('primary_topics', []):
        print(f"  - {topic}")
    print(f"Average Messages per Chat: {content_summary.get('avg_messages_per_chat', 'unknown')}")
