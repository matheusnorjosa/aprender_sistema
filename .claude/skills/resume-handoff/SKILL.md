...
Resume work from a handoff document through an interactive process. Handoffs (written by `create-handoff`) carry context, learnings, and next steps from a previous session. Validate that the handoff still matches the codebase, then continue — never assume the snapshot is current.
...
Tip: You can invoke this command directly with a handoff path: `/resume-handoff thoughts/shared/handoffs/{session-name}/YYYY-MM-DD_HH-MM-SS_description.md`

or using a session name (or related GitHub issue) to resume from the most recent handoff for that work stream: `/resume-handoff rbac-refactor`
```
