...
- **Escrita: owner-forced com delegacao por capability** (#1454, audit 2026-07-10) — **isto NAO estava documentado**:
  - `perform_create`: `usuario` omitido ou == `request.user` -> grava para o proprio (`delegated=False`). `usuario` != `request.user` -> exige `user_can_delegate_deslocamento`, senao **403**.
  - `perform_update`/`perform_destroy` passam por `_ensure_owner_or_delegate`: mexer no registro de outro exige a mesma capability, senao 403.
  - `user_can_delegate_deslocamento` (`rbac/policies.py`) so aceita superuser, `operate_preagenda` ou `view_all_availability`. O docstring e explicito: **Coordenador, Apoio, Gerente, Diretoria e Formador -> `False`**.
  - ⚠️ Consequencia real (achado `M09-05`, issue #1621, épico #1655): o formulario da UI marca "Formador" como **obrigatorio** (`DeslocamentosPage.tsx`, `Form.Item name="usuario"`) e sempre envia `usuario` de terceiro, com opcoes vindas de `/api/options/formadores-do-setor/`, que filtra `papel="FORMADOR"` (`views_options.py`, `formadores_do_setor_options`). Um Coordenador **nao aparece na propria lista**, nao consegue se auto-selecionar nem omitir o campo — logo todo POST dele cai no ramo de delegacao e recebe **403**. O comentario de `v2/frontend/src/api/deslocamentos.ts` (`createDeslocamento`) ("Backend força `usuario=request.user`") so vale quando o campo e omitido, o que a UI impede.
- **Auditoria**: CREATE e DELETE (antes de deletar) gravam sempre `AuditLog` com `usuario`, IP (X-Forwarded-For -> X-Real-IP -> REMOTE_ADDR), user-agent truncado em 200 e — no CREATE — o flag `delegated`. **O UPDATE só grava se houver mudança** (`if changed_fields:`) e o diff cobre apenas `origem, destino, start_date, end_date, observacao`: **troca de `usuario` (transferência do registro) não entra no diff nem gera log**.
...
- `GET /api/deslocamentos/` — lista paginada (50/pagina; `page_size_query_param="page_size"` **esta definido** em `views_deslocamento.py` (`DeslocamentoPagination`), com `max_page_size=100`, entao `?page_size=N` e respeitado), ordenada por `-start_date`.
- `GET /api/deslocamentos/{id}/`, `POST /api/deslocamentos/`, `PUT/PATCH /api/deslocamentos/{id}/`, `DELETE /api/deslocamentos/{id}/`.
- **Permissao do ViewSet**: `[IsAuthenticated]` no nivel de classe, **mas a escrita nao e livre**: `perform_create`/`perform_update`/`perform_destroy` aplicam o gate owner-or-delegate descrito em §Contratos. Nao ha `HasPerm` de classe porque a leitura e controlada por *visibilidade* (queryset).
