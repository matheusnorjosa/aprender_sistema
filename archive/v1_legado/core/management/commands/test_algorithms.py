...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--all", action="store_true", help="Run all algorithm tests"
        )
    ...
    def test_recommendations(self):
        ...
        try:
            recommendations = get_formador_recommendations(
            ...
            for i, rec in enumerate(recommendations, 1):
                ...
                self.stdout.write(
                    f"   Recommendation Strength: {rec['recommendation_strength']}"
                )
    ...
    def test_scheduling(self):
        ...
        try:
            ...
            for i, event in enumerate(result["schedule"], 1):
                ...
                self.stdout.write(
                    f"   Formadores: {', '.join(f['name'] for f in event['formadores'])}"
                )
    ...
    def test_performance(self):
        ...
        try:
            ...
            if "error" in prediction:
                self.stdout.write(self.style.ERROR(f"[ERROR] {prediction['error']}"))
                return
            ...
            self.stdout.write(
                f"Predicted Performance: {prediction['predicted_performance']:.3f}"
            )
            self.stdout.write(
                f"Performance Category: {prediction['performance_category']}"
            )
            ...
            self.stdout.write(
                f"At-Risk Student Prediction: {risk_prediction['performance_category']} "
                f"({risk_prediction['predicted_performance']:.3f})"
            )
            self.stdout.write(
                f"Risk Factors: {', '.join(risk_prediction.get('risk_factors', []))}"
            )
