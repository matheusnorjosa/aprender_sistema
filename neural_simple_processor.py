    ...
    for sheet_name, sheet_data in raw_data.get('planilhas', {}).items():
        ...
        for tab_data in sheet_data.get('abas', []):
            ...
            if 'usuarios' in sheet_name.lower() or 'usuários' in sheet_name.lower():
                ...
                extract_controle_data(tab_data, pessoas_unicas, projetos_unicos, atribuicoes_unicas)
            elif 'acompanhamento' in sheet_name.lower() or 'agenda' in sheet_name.lower():
                extract_agenda_data(tab_data, sheet_name, pessoas_unicas, projetos_unicos, 
                                  municipios_unicos, eventos_unicos, atribuicoes_unicas)
            elif 'disponibilidade' in sheet_name.lower():
                extract_disponibilidade_data(tab_data, pessoas_unicas, projetos_unicos, 
                                           municipios_unicos, eventos_unicos, atribuicoes_unicas)
...
def extract_usuarios_data(tab_data: Dict[str, Any], pessoas_unicas: Set, atribuicoes_unicas: Set):
    ...
    logger.info(f"    📝 Extraindo dados de usuários da aba: {tab_data.get('nome', '')}")

def extract_controle_data(tab_data: Dict[str, Any], pessoas_unicas: Set, projetos_unicos: Set, atribuicoes_unicas: Set):
    """Extrai dados de controle"""
    tab_name = tab_data.get('nome', '').lower()
    
    if 'coord' in tab_name:
        # Extrair coordenadores
        logger.info(f"    👥 Extraindo coordenadores da aba: {tab_data.get('nome', '')}")
...
def extract_agenda_data(tab_data: Dict[str, Any], sheet_name: str, pessoas_unicas: Set, 
                       projetos_unicos: Set, municipios_unicos: Set, eventos_unicos: Set, 
                       atribuicoes_unicas: Set):
    ...
    logger.info(f"    📅 Extraindo dados de agenda da aba: {tab_data.get('nome', '')} (Categoria: {categoria})")
    
    # Simular extração de dados
    # Em uma implementação real, você processaria os registros da aba

def extract_disponibilidade_data(tab_data: Dict[str, Any], pessoas_unicas: Set, projetos_unicos: Set,
                                municipios_unicos: Set, eventos_unicos: Set, atribuicoes_unicas: Set):
