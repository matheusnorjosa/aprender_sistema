...
class GoogleSheetsDataAnalyst:
    ...
    def verificar_credenciais(self):
        ...
        try:
            ...
            campos_obrigatorios = ['type', 'project_id', 'private_key_id', 'private_key', 'client_email', 'client_id']
            campos_faltando = [campo for campo in campos_obrigatorios if campo not in credenciais]
    ...
    def listar_planilhas_disponiveis(self):
        ...
        try:
            ...
            for planilha in planilhas:
                nome = planilha['name']
                if any(palavra in nome for palavra in ['2025', 'Acompanhamento', 'Disponibilidade', 'Usuários']):
    ...
    def analisar_estrutura_dados(self, dados: List[Dict], nome_aba: str):
        ...
        for campo, preenchidos in analise['campos_preenchidos'].items():
            ...
            if percentual < 50:
                analise['problemas_identificados'].append(f"Campo '{campo}' com apenas {percentual:.1f}% preenchido")
        ...
        for campo, preenchidos in analise['campos_preenchidos'].items():
            percentual = (preenchidos / analise['total_registros']) * 100
            print(f"   • {campo}: {preenchidos}/{analise['total_registros']} ({percentual:.1f}%)")
    ...
    def tratar_dados_extraidos(self, dados: List[Dict], nome_aba: str):
        ...
        for i, registro in enumerate(dados):
            try:
                registro_tratado = self._tratar_registro_individual(registro, nome_aba, i+1)
        ...
        print(f"   • Registros processados: {estatisticas_tratamento['registros_processados']}")
        print(f"   • Registros válidos: {estatisticas_tratamento['registros_validos']}")
        print(f"   • Registros removidos: {estatisticas_tratamento['registros_removidos']}")
        
        return dados_tratados, estatisticas_tratamento
    
    def _tratar_registro_individual(self, registro: Dict, nome_aba: str, linha: int) -> Optional[Dict]:
        ...
        registro_tratado = {
            ...
            'dados_extras': {
                ...
                'status': campos_mapeados.get('status', 'Aprovado' if nome_aba != 'Super' else 'Pendente'),
                'dados_originais': registro
            }
        }
        
        # Validar se registro é válido
        if (registro_tratado['municipio']['nome'] and 
            registro_tratado['projeto']['nome'] and 
            registro_tratado['evento']['tipo'] and 
            registro_tratado['formadores']):
        ...
        for planilha_info in planilhas_disponiveis:
            ...
            if nome_planilha in self.planilhas_config:
                ...
                for nome_aba, config in config_abas.items():
                    ...
                    # Tratar dados
                    dados_tratados, estatisticas = self.tratar_dados_extraidos(dados_brutos, nome_aba)
    ...
    def gerar_relatorio_extracao(self):
        ...
        for nome_aba, dados in self.dados_extraidos.items():
            ...
            print(f"   • Taxa de sucesso: {(registros_tratados/registros_brutos)*100:.1f}%")
        ...
        return {
            ...
            'dados_por_aba': {nome: len(dados['dados_tratados']) for nome, dados in self.dados_extraidos.items()}
        }
