    ...
    def clean_test_projects(self):
        ...
        projetos_sem_produto = Projeto.objects.filter(
            ...
            nome__in=[
                ...
                'AMMA', 'CATAVENTOS', 'MIUDEZAS', 'UNI DUNI TÊ', 'IDEB 10'
            ]
        )
        
        for project in projetos_sem_produto:
            # Se o nome sugere ser teste, adicionar
            if any(pattern in project.nome for pattern in ['Projeto ', 'Teste', 'Alpha', 'Beta']):
                if project not in test_projects:
                    test_projects.append(project)
        ...
        for project in test_projects:
            ...
            self.stdout.write(f'• {project.nome} ({setor_nome}) - {solicitacoes} solicitações')
        
        # Verificar impacto nas solicitações
        total_solicitacoes = sum(Solicitacao.objects.filter(projeto=project).count() for project in test_projects)
        ...
        if not self.dry_run and test_projects:
            ...
            for project in test_projects:
                ...
                self.stdout.write(f'✓ Removido: {project.nome} ({solicitacoes_count} solicitações)')
            
            self.stdout.write(f'\nEstatísticas:')
            self.stdout.write(f'- Projetos removidos: {stats["projetos_removidos"]}')
            self.stdout.write(f'- Solicitações removidas: {stats["solicitacoes_removidas"]}')
