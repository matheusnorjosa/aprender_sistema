...
class ImportadorAgenda2025ContextoSupremo:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.abas_config = {
            'ACerta': {
                ...
                'nomes_colunas': [
                    'Criado na Agenda', 'município', 'encontro', 'tipo', 'data',
                    'hora início', 'hora fim', 'projeto', 'segmento', 'Coord Acompanha',
                    'Coordenador', 'Formador 1', 'Formador 2', 'Formador 3',
                    'Formador 4', 'Formador 5', 'Convidados'
                ],
                'setor': 'ACerta'
            },
            'Outros': {
            ...
            'Super': {
                ...
                'nomes_colunas': [
                    'Criado na Agenda', 'Aprovação', 'Municípios', 'encontro', 'tipo',
                    'data', 'hora início', 'hora fim', 'projeto', 'Coord Acompanha',
                    'Coordenador', 'Formador 1', 'Formador 2', 'Formador 3',
                    'Formador 4', 'Formador 5', 'Convidados'
                ],
    ...
    def extrair_dados_aba(self, spreadsheet, nome_aba: str) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"📊 Aba {nome_aba}: {len(dados_processados)} registros extraídos")
        ...
        if data_evento_str:
            try:
                ...
                for formato in ['%d/%m/%Y', '%d-%m-%Y', '%Y-%m-%d']:
                    ...
                    logger.warning(f"⚠️ Formato de data não reconhecido: {data_evento_str}")
                    data_evento = None
    ...
    def processar_formadores(self, evento_data: Dict) -> List[str]:
        ...
        for i in range(1, 6):  # Formador 1 a Formador 5
            ...
            if formador_key in evento_data and evento_data[formador_key]:
                ...
                if formador_nome and formador_nome.lower() not in ['', 'n/a', 'não informado']:
    ...
    def importar_evento(self, evento_data: Dict) -> bool:
        ...
        try:
            if self.dry_run:
                logger.info(f"🔍 [DRY RUN] Evento: {evento_data.get('encontro', 'N/A')} - {evento_data.get('município', 'N/A')}")
                return True
            ...
            municipio, created = Municipio.objects.get_or_create(
                nome=municipio_nome,
                defaults={'uf': 'TO'}  # Assumir TO como padrão
            )
            ...
            projeto, created = Projeto.objects.get_or_create(
                nome=projeto_nome,
                defaults={'descricao': f'Projeto {projeto_nome}'}
            )
            ...
            tipo_evento, created = TipoEvento.objects.get_or_create(
                nome=tipo_nome,
                defaults={'descricao': f'Tipo de evento: {tipo_nome}'}
            )
    ...
    def gerar_relatorio_final(self):
        ...
        logger.info(f"   - Total de registros processados: {self.estatisticas['total_registros']}")
        logger.info(f"   - Eventos importados: {self.estatisticas['eventos_importados']}")
        logger.info(f"   - Eventos já aconteceram: {self.estatisticas['eventos_ja_aconteceram']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Importar dados da planilha Agenda 2025')
