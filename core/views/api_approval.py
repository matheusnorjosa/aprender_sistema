...
from core.models import (
    Solicitacao, Aprovacao, LogAuditoria, 
    SolicitacaoStatus, AprovacaoStatus
)
...
class BulkApprovalAPI(View):
    ...
    def _processar_solicitacao(self, sol_id, acao, justificativa, usuario):
        ...
        try:
            ...
            from core.services.notifications_simplified import notify_solicitacao_approved
            try:
                if acao == 'aprovar':
        ...
        try:
            ...
            for sol in page_obj:
                ...
                solicitacoes.append({
                    ...
                    'data_inicio': sol.data_inicio.isoformat() if sol.data_inicio else None,
                    'data_fim': sol.data_fim.isoformat() if sol.data_fim else None,
                    'formadores': [f.nome for f in sol.formadores.all()],
                    'usuario_solicitante': sol.usuario_solicitante.username if sol.usuario_solicitante else '',
            ...
            return JsonResponse({
                'success': False,
                'error': f'Erro ao buscar solicitações: {str(e)}'
            })
    ...
    def get(self, request, solicitacao_id):
        try:
            ...
            for bloqueio in conflitos.get('bloqueios', []):
                tipo_codigo = 'T' if bloqueio.tipo_bloqueio.upper() in ['T', 'TOTAL'] else 'P'
            ...
            for sol_conflito in conflitos.get('solicitacoes', []):
                formadores_nomes = ", ".join([f.nome for f in sol_conflito.formadores.all()])
                conflict_details.append({
                    'type': 'evento',
                    'code': 'E',
                    'severity': 'error',
            ...
            has_blocking_conflicts = any(c['severity'] == 'error' for c in conflict_details)
            ...
            return JsonResponse({
                ...
                'recommendation': self._get_recommendation(has_blocking_conflicts, conflict_details)
            })
            
        except Exception as e:
            return JsonResponse({
                'success': False,
                'error': f'Erro ao analisar conflitos: {str(e)}'
            })
