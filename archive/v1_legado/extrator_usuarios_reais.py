            ...
            try:
                with transaction.atomic():
                    ...
                    if Usuario.objects.filter(
                        ...
                        print(
                            f"   ⚠️ Usuário {usuario_data['username']} já existe, pulando..."
                        )
                        continue
                    ...
                    if usuario_data["setor"]:
                        setor, _ = Setor.objects.get_or_create(
                            nome=usuario_data["setor"], defaults={"ativo": True}
                        )
                    ...
                    usuario = Usuario.objects.create(
                        ...
                        or f"{usuario_data['username']}@aprender.com",
                        first_name=(
                            usuario_data["nome"].split()[0]
                            if usuario_data["nome"]
                            else ""
                        ),
                        last_name=(
                            " ".join(usuario_data["nome"].split()[1:])
                            if len(usuario_data["nome"].split()) > 1
                            else ""
                        ),
                        is_active=True,
                    )
                    ...
                    if self.usuarios_importados % 10 == 0:
                        print(
                            f"   ✅ {self.usuarios_importados} usuários importados..."
                        )
    ...
    def gerar_relatorio(self):
        ...
        relatorio = {
            ...
            "estatisticas": {
                ...
                "usuarios_por_grupo": {
                    grupo.name: grupo.user_set.count() for grupo in Group.objects.all()
                },
            },
        }

        with open(
            "/app/relatorio_importacao_usuarios_reais.json", "w", encoding="utf-8"
        ) as f:
...
def main():
    ...
    try:
        ...
        print(
            f"   👥 Total no sistema: {relatorio['estatisticas']['total_usuarios_sistema']}"
        )
