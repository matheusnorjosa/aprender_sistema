...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--dry-run", action="store_true", help="Simulação sem alterar dados"
        )
        parser.add_argument(
            "--force",
            action="store_true",
    ...
    def handle(self, *args, **options):
        if not options["docker"]:
            ...
            self.stdout.write(
                self.style.WARNING(
                    "Use: docker-compose exec web python manage.py unify_data_references --docker"
                )
            )
            return
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING("⚠️  MODO SIMULAÇÃO - Nenhum dado será alterado")
            )
        ...
        with transaction.atomic():
            try:
                ...
                if not dry_run:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            "🔍 SIMULAÇÃO COMPLETA - Use sem --dry-run para aplicar"
                        )
                    )

            except Exception as e:
                self.stdout.write(self.style.ERROR(f"❌ ERRO NA UNIFICAÇÃO: {str(e)}"))
                raise
    ...
    def _verificar_ambiente_docker(self):
        ...
        self.stdout.write(
            f"📊 Estado atual: {total_usuarios} usuários, {total_formadores} formadores"
        )
    ...
    def _migrar_formadores_para_usuarios(self, dry_run, force):
        ...
        for formador in formadores:
            try:
                if formador.usuario:
                    ...
                    migrados += 1
                    self.stdout.write(
                        f"📝 Migrado: {formador.nome} → {usuario.username}"
                    )
                    ...
                    if not dry_run:
                        usuario = Usuario.objects.create_user(
                            ...
                            first_name=(
                                formador.nome.split()[0]
                                if formador.nome.split()
                                else ""
                            ),
                            last_name=(
                                " ".join(formador.nome.split()[1:])
                                if len(formador.nome.split()) > 1
                                else ""
                            ),
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✅ Migração concluída: {migrados} atualizados, {criados} criados"
            )
        )
    ...
    def _atualizar_relacionamentos(self, dry_run):
        ...
        inconsistencias = (
            ...
            .filter(usuario__formador_ativo=False)
            .count()
        )
        ...
        if inconsistencias > 0:
            self.stdout.write(
                self.style.WARNING(
                    f"⚠️  {inconsistencias} inconsistências em FormadoresSolicitacao"
                )
            )
        ...
        if disponibilidades > 0:
            self.stdout.write(
                self.style.WARNING(
                    f"⚠️  {disponibilidades} disponibilidades sem usuário"
                )
            )
    ...
    def _verificar_integridade(self):
        ...
        formadores_sem_usuario = FormadorService.filter_formadores(
            usuario__isnull=True
        ).count()
        if formadores_sem_usuario > 0:
            raise Exception(
                f"ERRO: {formadores_sem_usuario} formadores ainda sem usuário"
            )
        ...
        stats = {
            "total_usuarios": Usuario.objects.count(),
            "usuarios_formadores": Usuario.objects.select_related(
                "municipio", "projeto"
            )
            .prefetch_related("groups", "user_permissions")
            .filter(formador_ativo=True)
