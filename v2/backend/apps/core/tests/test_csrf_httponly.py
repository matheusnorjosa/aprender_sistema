...
def test_csrf_endpoint_returns_token(api_client):
    ...
    assert response.status_code == status.HTTP_200_OK, f"Esperado 200 OK, recebido {response.status_code}"

    assert "csrfToken" in response.data, "Response body deve conter campo 'csrfToken'"
...
def test_csrf_endpoint_sets_cookie(api_client):
    ...
    assert "csrftoken" in response.cookies, "Endpoint /api/csrf/ deve definir cookie 'csrftoken'"
    ...
    assert (
        csrf_cookie.get("httponly") or settings.CSRF_COOKIE_HTTPONLY
...
def test_csrf_endpoint_no_authentication_required(api_client):
    ...
    assert response.status_code == status.HTTP_200_OK, "Endpoint /api/csrf/ deve ser acessível sem autenticação"
...
@pytest.mark.skip(
    reason="Login endpoint não requer CSRF (entry point de autenticação). CSRF é enforced em endpoints autenticados via SessionAuthentication."
)
@override_settings(CSRF_COOKIE_HTTPONLY=True)
...
def test_csrf_cookie_httponly_is_true():
    ...
    """
    assert settings.CSRF_COOKIE_HTTPONLY is True, "CSRF_COOKIE_HTTPONLY deve ser True (Issue #135)"
...
def test_csrf_cookie_samesite_is_lax():
    ...
    """
    assert settings.CSRF_COOKIE_SAMESITE == "Lax", "CSRF_COOKIE_SAMESITE deve ser 'Lax' (balanço segurança/usabilidade)"
...
def test_csrf_endpoint_accepts_only_get(api_client):
    ...
    assert response_post.status_code == status.HTTP_405_METHOD_NOT_ALLOWED, "POST /api/csrf/ deve retornar 405"

    # PUT
    response_put = api_client.put("/api/csrf/")
    assert response_put.status_code == status.HTTP_405_METHOD_NOT_ALLOWED, "PUT /api/csrf/ deve retornar 405"

    # DELETE
    response_delete = api_client.delete("/api/csrf/")
    assert response_delete.status_code == status.HTTP_405_METHOD_NOT_ALLOWED, "DELETE /api/csrf/ deve retornar 405"
