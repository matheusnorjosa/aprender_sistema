...
- **`pages/Solicitacoes.tsx` é dead code.** O arquivo raiz [`pages/Solicitacoes.tsx`](../../../frontend/src/pages/Solicitacoes.tsx) (~15 KB) **não é importado por nenhum módulo** — nenhuma rota o alcança. Já `pages/Disponibilidade.tsx` (também na raiz) **está** roteado; os dois não devem ser tratados como o mesmo caso.
