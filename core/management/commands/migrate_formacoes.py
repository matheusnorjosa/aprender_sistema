...
from core.models import Solicitacao, Formador, Municipio, TipoEvento, Projeto
...
class Command(BaseMigrationCommand):
    ...
    help = 'Migra formações históricas da planilha CONTROLE (50K+ registros) para Django'
    ...
    def migrate_data(self, source_file: str, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Encontradas {len(formacoes)} formações na aba '{formacoes_aba}'")
        ...
        self.logger.info(f"Processando registros {start_offset} a {end_offset} ({len(formacoes_slice)} formações)")
    ...
    def process_async(self, formacoes: List, headers: List, options: Dict, start_offset: int) -> Dict[str, Any]:
        """
        ...
        self.logger.info(f"Iniciando processamento assíncrono com batches de {batch_size}")
        ...
        for i in range(0, len(formacoes), batch_size):
            ...
            if not self.dry_run:
                ...
                self.logger.info(f"Batch {current_offset}-{current_offset + len(batch)} enviado para Celery (ID: {task.id})")
            else:
                self.logger.info(f"[DRY RUN] Batch {current_offset}-{current_offset + len(batch)} seria enviado para Celery")
        
        self.stats['async_tasks'] = len(task_ids)
        self.stats['task_ids'] = task_ids
        
        self.logger.info(f"Processamento assíncrono iniciado: {len(task_ids)} tasks enviadas")
        self.logger.info("Use 'celery -A aprender_sistema flower' para monitorar progresso")
        
        return self.stats
    
    def process_sync(self, formacoes: List, headers: List, options: Dict) -> Dict[str, Any]:
        """
        ...
        self.logger.info(f"Iniciando processamento síncrono com batches de {batch_size}")
        
        # Processar em batches para otimizar performance
        for i in range(0, len(formacoes), batch_size):
            batch = formacoes[i:i + batch_size]
            
            self.logger.info(f"Processando batch {i // batch_size + 1}: registros {i+1}-{i + len(batch)}")
            
            if not self.dry_run:
                self.process_batch(batch, headers, options)
            else:
                self.stats['processed'] += len(batch)
                self.logger.info(f"[DRY RUN] Batch de {len(batch)} formações seria processado")
        ...
        try:
            with transaction.atomic():
                for formacao_row in batch:
                    try:
                        formacao_data = self.parse_formacao_row(formacao_row, headers, options)
    ...
    def parse_formacao_row(self, row: List, headers: List, options: Dict) -> Dict[str, Any]:
        """
