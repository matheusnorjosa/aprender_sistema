...
import csv, io, re, unicodedata, datetime, hashlib, urllib.request
...
def _boolish(v: Optional[str]) -> bool:
    ...
    return str(v).strip().lower() in {"1","true","t","yes","y","sim","x","ok","checked","marcado"}
...
def _parse_time_pt(s: Optional[str]) -> Optional[datetime.time]:
    ...
    for fmt in ("%H:%M", "%H:%M:%S"):
        try: return datetime.datetime.strptime(s, fmt).time()
        except: pass
    return None
...
def collect_formadores(row_norm: Dict[str, str]) -> List[str]:
    ...
    if not formadores:
        ...
        if coord: formadores.append(coord)
    seen, out = set(), []
    for f in formadores:
        if f not in seen:
...
class ParsedEvento:
    def __init__(self, origem_aba: str, municipio: str, data: Optional[datetime.date],
                 hora_ini: Optional[datetime.time], hora_fim: Optional[datetime.time],
                 projeto: Optional[str], tipo: Optional[str], coordenador: Optional[str],
                 formadores: List[str], aprovado: bool, cancelado: bool, encontro: Optional[str]):
...
def parse_row_por_aba(origem_aba: str, row: Dict[str,str]) -> Optional[ParsedEvento]:
    ...
    hora_ini  = _parse_time_pt(_pick(rn, "Hora Início","Hora Inicio","Ini","Hora Ini"))
    hora_fim  = _parse_time_pt(_pick(rn, "Hora Fim","Fim","Hora Término","Hora Termino"))
    ...
    if not municipio or not data:
        return None
    return ParsedEvento(origem_aba, municipio, data, hora_ini, hora_fim,
                        projeto, tipo, coord, formadores, aprovado, cancelado, encontro)

class SheetsAdapter:
    """Lê direto do Google Sheets (export?format=csv&gid=...), via GET com redirects."""
    def rows(self, sheet_id: str, gid: str, encoding: str = "utf-8") -> Iterable[Dict[str,str]]:
...
class CsvFolderAdapter:
    ...
    def rows(self, csv_filename: str, encoding: str = "utf-8") -> Iterable[Dict[str,str]]:
        import os
...
def _looks_like_disp_header(cols_norm):
    ...
    if "formador" in c and (c & meses or {"ch","chanual","cargahoraria","horas"} & c):
        return "ANUAL"
    if ({"ano","carga_horaria","ch","horas"} & c) and ("formador" in c or "nome" in c):
        return "ANUAL"
...
def _skip_to_header(csv_text):
    ...
    for _ in range(200):
        ...
        try:
            ...
            if len(parts) == 1 and _norm_key(parts[0]) in {"emmanutencao","manutencao","legenda","cabecalho","aguarde"}:
                continue
        except:
            pass
...
def _preprocess_csv_text_for_disp(raw_bytes):
    try:
        ...
        try: txt = raw_bytes.decode("latin1", "ignore")
        except Exception: txt = raw_bytes.decode("ascii","ignore")
