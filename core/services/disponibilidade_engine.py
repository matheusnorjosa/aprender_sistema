...
class DisponibilidadeEngine:
    ...
    def __init__(self, travel_buffer_minutes: int = None, daily_hour_limit: int = None):
        ...
        """
        self.travel_buffer_minutes = travel_buffer_minutes or self.DEFAULT_TRAVEL_BUFFER_MINUTES
    ...
    def check_availability(
        ...
        code = self._determine_primary_code(all_conflicts, formadores_ok, data_inicio.date())
        summary = self._build_summary_message(all_conflicts, formadores_ok, formadores_conflict)
    ...
    def _check_formador_availability(
        ...
        total_blocks = DisponibilidadeFormadores.objects.filter(
            formador=formador,
            tipo_bloqueio='T',
            data_bloqueio=event_date
        )
        
        for block in total_blocks:
            # Converter para datetime para comparação
            block_start = timezone.make_aware(
                datetime.combine(block.data_bloqueio, block.hora_inicio),
                self.local_tz
            )
            block_end = timezone.make_aware(
                datetime.combine(block.data_bloqueio, block.hora_fim),
                self.local_tz
            )
        ...
        existing_events = Solicitacao.objects.filter(
            formadores=formador,
            status__in=['APROVADO'],  # Usar apenas APROVADO baseado no SolicitacaoStatus
    ...
    def _check_travel_buffer(
        ...
        nearby_events = Solicitacao.objects.filter(
            formadores=formador,
            status__in=['APROVADO']
        ).exclude(municipio=municipio).filter(
...
def check_formador_availability(
    ...
    return engine.check_availability(
        Formador.objects.filter(id=formador.id),
        data_inicio,
        data_fim,
        target_municipio
    )