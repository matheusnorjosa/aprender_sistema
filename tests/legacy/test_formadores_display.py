...
def test_formadores_display():
    ...
    valid_names = [
        label
        for label in formador_labels
        if len(label.strip()) > 0 and not ("<" in label and "@" in label)
    ]
    ...
    if len(valid_names) == len(formador_labels):
        ...
        print(
            f"⚠️ {len(formador_labels) - len(valid_names)} nomes podem ter problemas de formatação"
        )
