...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"✅ {count} credenciais atualizadas com sucesso!"))
            self.stdout.write("")
            ...
            self.stdout.write(self.style.ERROR("❌ Erro: Chave antiga incorreta!"))
            self.stdout.write("")
            self.stdout.write("Verifique se --old-key corresponde ao valor atual de GCAL_ENCRYPTION_KEY.")
            sys.exit(1)

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"❌ Erro ao rotacionar chave: {e}"))
            sys.exit(1)
