...
def migrate_formacoes_task(self, batch_size=500, offset=0):
    ...
    """
    try:
        logger.info(f"Iniciando migração de formações: batch_size={batch_size}, offset={offset}")
...
def sync_google_calendar_task(self, event_data):
    ...
    """
    try:
        logger.info(f"Iniciando sync Google Calendar: {event_data.get('title', 'Sem título')}")
