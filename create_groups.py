...
from core.models import Solicitacao, LogAuditoria, DisponibilidadeFormadores, Aprovacao
...
def create_groups_and_permissions():
    ...
    for user in Usuario.objects.all():
        if user.papel and user.papel in papel_to_group:
            ...
            try:
                ...
                for existing_group in Group.objects.filter(name__in=papel_to_group.values()):
            ...
            print(f"⚠️  Usuário '{user.username}' sem papel definido ou papel inválido: '{user.papel}'")
    ...
    for group_name in groups_permissions.keys():
        ...
        print(f"📊 Grupo '{group_name}': {user_count} usuários, {perm_count} permissões")
