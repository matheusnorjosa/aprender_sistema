...
from core.models import Formador, FormadoresSolicitacao, Solicitacao, Usuario
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--docker", action="store_true", help="Execução no Docker (obrigatório)"
        )
        parser.add_argument(
            "--detailed", action="store_true", help="Relatório detalhado"
        )

    def handle(self, *args, **options):
        if not options["docker"]:
            self.stdout.write(self.style.ERROR("ERRO: Execute apenas no Docker"))
            return
    ...
    def _test_usuario_single_source(self, errors, warnings):
        ...
        if abs(usuarios_formadores - formadores_ativos) > 5:  # Tolerância pequena
            warnings.append(
                f"Inconsistência: {usuarios_formadores} vs {formadores_ativos}"
            )
        else:
    ...
    def _test_services_working(self, errors, warnings):
        ...
        try:
            ...
            self.stdout.write(
                f"   ✅ UsuarioService: {usuarios_ativos} usuários ativos"
            )
            ...
            self.stdout.write(
                f"   ✅ CoordinatorService: {coordenadores} coordenadores"
            )

            # Testar DashboardService
            stats = DashboardService.get_estatisticas_gerais()
            self.stdout.write(
                f'   ✅ DashboardService: {stats["usuarios_total"]} usuários'
            )
    ...
    def _test_query_performance(self, errors, warnings, detailed):
        ...
        if detailed:
            ...
            formadores = list(
                UsuarioService.get_optimized_queryset().filter(formador_ativo=True)[:10]
            )
    ...
    def _test_data_integrity(self, errors, warnings):
        ...
        solicitacoes_orfas = (
            ...
            .prefetch_related("formadores")
            .filter(usuario_solicitante__isnull=True)
            .count()
        )
    ...
    def _print_final_report(self, success_count, errors, warnings, detailed):
        ...
        if not errors:
            self.stdout.write("")
            self.stdout.write(self.style.SUCCESS("🎉 UNIFICAÇÃO VALIDADA COM SUCESSO!"))
            ...
            self.stdout.write("")
            self.stdout.write(self.style.ERROR("❌ UNIFICAÇÃO COM PROBLEMAS"))
