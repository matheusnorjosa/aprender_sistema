...
- **Reconcile v16→v20** (corrige o que o create-only NÃO conserta): [`apps/core/services/export_contract_v20_reconciler.py`](../../../backend/apps/core/services/export_contract_v20_reconciler.py) + command [`management/commands/reconcile_export_contract_v20.py`](../../../backend/apps/core/management/commands/reconcile_export_contract_v20.py). Aplica `reconcile__v16_para_v20.csv` (RELAY-40/41 do sheets.banco): **dry-run por padrão**, `--apply` só escreve classes em `--allow-class`, **guardas estritas** (lookup determinístico; ambíguo/não-encontrado → skip+report, nunca chuta), idempotente, transação all-or-nothing. Classes: `FAMILIA_CORRIGIDA` (repontar `Projeto.projeto_geral`), `FAMILIA_INEXISTENTE` (deletar ProjetoGeral fantasma, guard anti-ref), `TROCA_DE_TITULAR` (reatribui Participation por `external_hash`), `TROCA_DE_PESSOA` (reatribui/remove por `município+data+papel+de-CPF`, exatamente-1), `LINHA_NAO_EMITIDA` (report-only = decisão manual). Recalc de `nr_codigos` (31, `recalcular__codigos_dat.csv`) é passo à parte.
