...
from core.models import Solicitacao, EventoGoogleCalendar, SolicitacaoStatus
...
class ManualCreateEventAPI(View):
    ...
    def post(self, request):
        try:
            ...
            result = management_service.create_event_for_controle(solicitacao, request.user)
    ...
    def get(self, request):
        try:
            ...
            stats = {
                ...
                ).exclude(meet_link__exact='').count()
            }
            
            # Últimos eventos criados
            recent_events = []
            for evento_gc in EventoGoogleCalendar.objects.select_related('solicitacao')[:10]:
                recent_events.append({
                    ...
                    'criado_por': evento_gc.criado_por.username if evento_gc.criado_por else None
                })
            ...
            try:
                ...
                return JsonResponse({
                    ...
                    'solicitacao': {
                        ...
                        'data_fim': solicitacao.data_fim.isoformat() if solicitacao.data_fim else None,
    ...
    def post(self, request):
        try:
            ...
            if not solicitacao_ids:
                return JsonResponse({
                    'success': False,
                    'error': 'IDs das solicitações são obrigatórios'
                })
