...
- **Reconcile v16→v20/v24** (corrige o que o create-only NÃO conserta): [`apps/core/services/export_contract_v20_reconciler.py`](../../../backend/apps/core/services/export_contract_v20_reconciler.py) + command [`management/commands/reconcile_export_contract_v20.py`](../../../backend/apps/core/management/commands/reconcile_export_contract_v20.py). Lê o `reconcile__v16_para_vNN.csv` de **maior versão** no `--path` (RELAY-40/41/43 do sheets.banco): **dry-run por padrão**, `--apply` só escreve classes em `--allow-class`, **guardas estritas** (lookup determinístico; ambíguo/não-encontrado → skip+report, nunca chuta), idempotente, transação all-or-nothing. Classes: `FAMILIA_CORRIGIDA` (repontar `Projeto.projeto_geral`), `FAMILIA_INEXISTENTE` (deletar ProjetoGeral fantasma, guard anti-ref), `TROCA_DE_TITULAR` (reatribui Participation), `TROCA_DE_PESSOA` (reatribui/remove; sub-ações via `o_que_fazer`), `LINHA_NAO_EMITIDA` (report-only = decisão manual). **Lookup das TROCA:** prefere as colunas v24 (`evento_id`=external_hash → solicitação → `participation(sol, de_cpf, papel)`; `de_usuario_origem`=e-mail da caixa como identidade alternativa no titular), com **fallback v20** (`external_hash`/`município+data+papel+de-CPF`, exatamente-1) quando ausentes. Recalc de `nr_codigos` (`recalcular__codigos_dat.csv`) é passo à parte (reconciliação sistema-vs-planilha = decisão de negócio).
...
- **Efeitos colaterais — o que é verdade e o que não é** (CP-02/PA-01):
  ...
  - ✅ **Import não atribui mais grupos privilegiados sem checar o ator** (achado `M03-01`, **corrigido em #1610**, commit `ccbe1e05`). Antes, `usuarios_import._assign_groups` fazia `Group.objects.filter(name__iexact=nome).first()` + `usuario.groups.add(grupo)` para **qualquer** nome vindo da coluna `grupos|groups|perfis|profiles|grupo|perfil`, sem allowlist e sem comparar o ator com o alvo — como o gate do endpoint é só `HasPerm("manage_admin_registries")`, um operador do grupo DAT podia enviar uma linha com o próprio CPF e `grupos="Gerente,Superintendência"` e se auto-promover a aprovador de solicitações, contornando o Tier-0 superuser-only de `GroupViewSet`/`assign_groups`. Hoje a concessão de grupos é **superuser-only**: `_actor_pode_atribuir_grupos` (`usuarios_import.py:273-283`) só retorna `True` para superuser e `:362` + `_assign_groups` (`:495-496`) aplicam o gate — para não-superuser os nomes vão para `grupos_ignorados` e nada é concedido. O importer do export-contract, por sua vez, só concede a allowlist `ALLOWED_USER_GROUPS` (`export_contract_importer.py:1077-1082`). **Residual**: o gate é ator×capability, não ator×alvo abrangente (um superuser ainda concede qualquer grupo) — endereçado no épico #1656.
  - ⚠️ RBAC segue admin-driven (D17) no sentido estrito: import não roda `seed_rbac` nem cria/edita capabilities. Ele apenas **atribui grupos existentes** — agora sob o gate superuser-only descrito acima (M03-01/#1610), com o residual ator×alvo em #1656.
...
Entidades classificadas no importer (`IMPLEMENTED`): `municipio`, `projeto_geral`, `projeto`, `produto`, `usuario`, `gerencia`, `equipe_gerencia`, `tipo_evento`, `dat_coordenador`, `dat_area`, `dat_acao`, `plano_formacao`, `dat_registro`, `dat_cadastro`, `dat_compra`, `solicitacao`, `participation`, `formacao`, `acompanhamento`, `prova`, `availability_block`, `deslocamento` (Refs #1896). **Todas as 22 entidades do contrato são classificadas** — não há mais `not_implemented`.

> ⚠️ `IMPLEMENTED` significa **classificado no dry-run**; nem toda entidade é **gravável no `--apply`**. `_apply_create_only` escreve para: `usuario`, `dat_area`, `municipio`, `projeto_geral` (com config de cálculo v5 + guard de alias ambíguo), `projeto` (Onda A: resolver canon-key #1372, popula o FK `projeto_geral` + `fluxo` autoritativo — `fluxo` ausente = `would_reject` (PA-01, nunca SUPER auto-aprovável); `projeto_geral` **declarado mas desconhecido** = `would_reject` (nunca família errada); **#1897**: `projeto_geral` **VAZIO** = rótulo família-vazia intencional → cria com `projeto_geral` NULL, **salvo** se o projeto tem `dat_registro`/`dat_compra` no pacote (aí a família importa p/ `nr_codigos` → `would_reject(familia_vazia_com_dat)`); homônimo (PG de mesmo nome) resolve o vazio quando existe; **#1897**: importa `setor` (vocabulário setor-de-produto) e `sem_operacao` (v15) — no create e via **reconcile** em projeto existente: `setor` **seed-if-empty** (é editável na conferência #1934 → nunca clobber valor humano), `sem_operacao` **if-differ** (autoritativo do import); reportado em `projeto__reconciled`, escopado e idempotente; sem actor), `tipo_evento`, `equipe_gerencia` (NK `(gerencia, usuario, papel)`; vigência aberta `valid_from`=hoje/`valid_to`=None; grava `Gerencia.setor_canonico` do row; cria a gerência pelo nome canônico via `_get_or_create_gerencia`; **sem actor** — EquipeGerencia não audita; papel fora de {GERENTE, COORDENADOR, APOIO, FORMADOR} → reject; o classify reporta `ignored_fields` — guardrail p/ colunas do CSV não-consumidas — #1895), `dat_acao` (NK `(municipio, projeto, ano)`; `ano` derivado da data da reunião — decisão B; exige actor), `plano_formacao` (NK `(municipio, projeto, ano)`; `ano` DECLARADO do workbook — não derivável de data; `sem_plano`=reserva → reject visível; **coordenadores = as PESSOAS que coordenaram (coluna Coordenador da Agenda), relação N:N por co-liderança (#1957)** — array `coordenadores_cpf` com fallback para a coluna única `coordenador_cpf`, cada CPF → `Usuario` (cpf unique no banco; SEM fallback email/nome — a pessoa é chave de CPF; ausente/inválido/sem match é descartado, #1849). O M2M `PlanoFormacoes.coordenadores` aponta `Usuario`, não `DATCoordenador` (governança, que segue em `dat_acao`/`dat_formacao`). Co-liderança confirmada pela medição do sheets.banco (RELAY 32/34: expandir a chave inflaria a CH — a leitura fiel é 1 plano, N coordenadores; ex.: 2 grupos de UNIÃO DOS PALMARES + 33 de VIDA E MATEMÁTICA/"Elienai & Silvio"). **Reconcile (v18):** em plano JÁ existente o importer sincroniza **só** o M2M `coordenadores` a partir do array (dado autoritativo, read-only na UI), reportado em `plano_formacao__coordenadores_reconciled`, **nunca esvazia** (array vazio = sem sinal) e **nunca toca outros campos** (create-only p/ o resto, anti-silent-update #1628/#1738); exige actor), `dat_coordenador` (fecha o silent-gap M18-09; lê `usuario_cpf` do master; NK existence-based **UNION** cpf∪email∪norm(nome); exige actor), `dat_registro`, `dat_cadastro` e `dat_compra` (estes exigem `--as-user`/actor; `dat_compra` carrega `tipo`/`conta_para_codigos` e dispara o recompute de `nr_codigos`), `solicitacao` (Refs #1896; NK/identidade = `external_hash` = o `evento_id` **estável** do CSV (ledger do sheets.banco; RELAY 52: o `evento_hash_natural` é hash de conteúdo e deriva ~2%/10 dias, então NÃO serve de identidade — vira fallback de join intra-entrega, e `_compute_external_hash`/ADR-012 o último fallback p/ fixtures; participation liga por `evento_id`); **solicitante (dono)** resolvido via `solicitante_cpf`→`solicitante_email`→`coordenador_cpf` (v16.4/RELAY 57: o dono já vem resolvido pela cascata coluna_n + escada-2-tokens + inferência; a coluna N crua vira só fallback), gravado nos DOIS destinos — `usuario` + FK `coordenador` + `Participation(COORDENADOR)`, decisão a; **`linha_completa=false` → reject** (v16.2: linha sem hora/dados essenciais — não inventa hora; coluna ausente = completa, p/ fixtures); carimba `solicitante_procedencia` (auditoria: fato `coluna_n`/`coluna_n_escada` vs inferência `cargo_unico`/`constante_da_aba`); status via `resolve_initial_status` (PA-01: SUPER/desconhecido → pendente); `coord_acompanha` Sim/Não → BooleanField `coordenador_acompanha` (RELAY 50: visual, vazio→False); `inicio`/`fim` montados de data+hora LOCAL America/Fortaleza; **#1896**: além do `segmento` cru, importa `segmento_norm` + `segmento_norm_confianca` (de-para canônico do sheets, v15: 275 grafias→57 valores; read-only na API, espelha `setor_canonico`), com **reconcile** em existente (reportado em `solicitacao__segmento_norm_reconciled`, escopado, idempotente, nunca esvazia); **sem actor** — Solicitacao não tem `created_by`) e `participation` (liga por `evento_id` → `solicitacao.external_hash` (fallback `evento_hash_natural`); **papel vem do CSV/posição, NUNCA do cargo**; usuário resolvido EMAIL-first → CPF → nome — escada #1643; **guarda de relay v16.4/RELAY 57: se `match_procedencia` vem VAZIO (o sheets tentou CPF/e-mail + escada e se absteve), o importer NÃO resolve por nome — o Degrau-2 token-subconjunto casaria quem saiu a um homônimo de nome mais longo → vira `guest_nome`; coluna ausente = sem sinal, mantém o fallback por nome**; sem match mas com nome → `Participation.guest_nome` (D1: preserva quem saiu, não descarta; CheckConstraint relaxado + UniqueConstraint parcial nome-only)), `formacao`/`acompanhamento`/`prova` (Refs #1896; filhas de `PlanoFormacoes` — o plano pai resolve por `(municipio, projeto, ano)`; `ano` vem da **coluna `ano`** do CSV da filha (o mesmo dado do pai, contrato v16.7/RELAY 31 — não derivado da data, que na fonte vem `DD/MM` e completaria o ano errado), com fallback ano-da-data→par-único p/ contratos antigos; par sem plano OU multi-ano ambíguo → não cria filha órfã (não chuta plano arbitrário); NK existence-based: Formacao `(plano, numero_formacao∈1..15)`, Acompanhamento `(plano, tipo∈{primeiro,segundo})`, Prova `(plano, numero_prova∈1..3)` (`realizada` fica **False** — o `marcado`=X significa **não-se-aplica** (o projeto não faz prova, RELAY 33), NÃO realizado; a fonte não tem coluna de "aconteceu" em aba nenhuma → `marcado` é lido e descartado, como `motivo`); guards de faixa/choices evitam `IntegrityError` na transação; **sem actor** — sem `created_by` no model, como `solicitacao`) e `availability_block` (Refs #1896; bloco de disponibilidade de formador — `usuario` por CPF (FK PROTECT NOT NULL → sem match = reject, não cria bloco órfão); `inicio`/`fim` datetime **LOCAL** Fortaleza → armazenado em UTC (RD-06), `fim ≤ inicio` viola a CheckConstraint = reject; `tipo ∈ {T,P}` (CheckConstraint) = reject fora disso; `motivo` sem origem na planilha (RELAY 31) → vazio; status default `aprovado` (save auto-aprova); **exige actor** (created_by); NK existence-based `(usuario, inicio, fim, tipo)`) e `deslocamento` (Refs #1896; formador entre municípios — `origem`/`destino` são **TEXTO** (CharField, não FK); `usuario` por CPF (FK PROTECT NOT NULL → sem match = reject); `start_date`/`end_date` obrigatórios; idempotência por `external_hash` (SHA1 ADR-012, `stable_import_hash`) sobre `(usuario, start, end, origem, destino, observacao)` — a **`observacao` ENTRA na chave** (RELAY 31: senão ida/volta colapsam); **sem actor** — sem `created_by` no model). O `_apply_create_only` itera por `ENTITY_ORDER` (não pela ordem de `--allow-entity`) — coordenador antes de plano, e `solicitacao` antes de `participation`. `--apply --allow-entity produto`/`gerencia` (classificam mas não gravam) agora **levanta `CommandError` (CR-03: `APPLIABLE ⊂ IMPLEMENTED`)**, em vez do antigo `applied=0` silencioso.
...
- **Prioridade de correção declarada** (`ACHADOS_REAIS.md`): `M03-01` (#1610) era a primeira — único achado do módulo alcançável por 3 contas reais e com efeito de escalação de privilégio —, seguida da issue #1649 (`dry_run` fail-open), barata e que reduzia a superfície dos demais. **Ambas já corrigidas** (#1610 em `ccbe1e05`; #1649 em #1852). Seguem os épicos #1659 (invariantes) e #1658 (resolvers), além do residual ator×alvo de M03-01 no épico #1656.
