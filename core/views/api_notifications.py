...
from core.models import Notificacao, LogComunicacao, Usuario
from core.services.notifications_simplified import get_user_notifications, get_unread_notifications_count
        ...
        try:
            ...
            for log in logs:
                data.append({
                    ...
                    'destinatario': {
                        'username': log.usuario_destinatario.username if log.usuario_destinatario else None,
                        'nome': log.usuario_destinatario.get_full_name() if log.usuario_destinatario else None,
                        'email': log.usuario_destinatario.email if log.usuario_destinatario else None,
                    } if log.usuario_destinatario else None,
                    'grupo_destinatario': log.grupo_destinatario,
                    'endereco_destinatario': log.endereco_destinatario,
                    'assunto': log.assunto,
                    'conteudo': log.conteudo[:200] + '...' if len(log.conteudo) > 200 else log.conteudo,
                    ...
                    'entidade_relacionada': {
                        'id': str(log.entidade_relacionada_id) if log.entidade_relacionada_id else None,
                        'tipo': log.entidade_relacionada_tipo
                    } if log.entidade_relacionada_id else None,
            ...
            stats = {
                'total': total_count,
                'por_tipo': dict(
                    LogComunicacao.objects.values('tipo_comunicacao').annotate(count=Count('id'))
                    .values_list('tipo_comunicacao', 'count')
                ),
                'por_status': dict(
                    LogComunicacao.objects.values('status_envio').annotate(count=Count('id'))
        ...
        try:
            ...
            stats = {
                ...
                'usuarios_mais_notificados': list(
                    ...
                    .values('usuario_destinatario__username', 'usuario_destinatario__first_name')
                    .annotate(count=Count('id'))
                    .order_by('-count')[:10]
                ),
