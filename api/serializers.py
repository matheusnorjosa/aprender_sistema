...
from core.models import (
    Projeto, Municipio, Formador, TipoEvento, 
    Solicitacao, FormadoresSolicitacao, Aprovacao,
    EventoGoogleCalendar, DisponibilidadeFormadores,
    LogAuditoria, Deslocamento
)
...
class FormadorListSerializer(serializers.ModelSerializer):
    ...
    areas_atuacao_display = serializers.CharField(
        source='get_areas_atuacao_display', 
        read_only=True
    )
...
class FormadorDetailSerializer(FormadorListSerializer):
    ...
    class Meta(FormadorListSerializer.Meta):
        fields = FormadorListSerializer.Meta.fields + [
            'areas_atuacao', 'observacoes', 'data_cadastro',
            'municipios', 'tipos_evento'
        ]
...
class SolicitacaoListSerializer(serializers.ModelSerializer):
    ...
    usuario_solicitante_nome = serializers.CharField(
        source='usuario_solicitante.get_full_name', 
        read_only=True
    )
    ...
    class Meta:
        model = Solicitacao
        fields = [
            'id', 'titulo_evento', 'data_inicio', 'data_fim',
            'status', 'status_display', 'data_solicitacao',
            'projeto_nome', 'municipio_nome', 'tipo_evento_nome',
...
class SolicitacaoDetailSerializer(SolicitacaoListSerializer):
    ...
    formadores_detail = FormadoresSolicitacaoSerializer(
        source='formadoressolicitacao_set',
        many=True,
        read_only=True
    )
    ...
    class Meta(SolicitacaoListSerializer.Meta):
        fields = SolicitacaoListSerializer.Meta.fields + [
            'descricao_evento', 'publico_alvo', 'observacoes',
            'data_aprovacao_rejeicao', 'justificativa_rejeicao',
            'projeto', 'municipio', 'tipo_evento', 
            'usuario_solicitante', 'usuario_aprovador',
            'formadores_detail'
        ]
...
class SolicitacaoCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Solicitacao
        fields = [
            'titulo_evento', 'descricao_evento', 'data_inicio', 'data_fim',
            'projeto', 'municipio', 'tipo_evento', 
            'publico_alvo', 'observacoes'
        ]
    ...
    def validate(self, data):
        ...
        if duracao.total_seconds() > 12 * 3600:
            raise serializers.ValidationError(
                "Duração máxima de evento é 12 horas"
            )
...
class AprovacaoSerializer(serializers.ModelSerializer):
    ...
    solicitacao_titulo = serializers.CharField(
        source='solicitacao.titulo_evento', 
        read_only=True
    )
    usuario_aprovador_nome = serializers.CharField(
        source='usuario_aprovador.get_full_name',
        read_only=True
    )
    status_display = serializers.CharField(
        source='get_status_decisao_display',
        read_only=True
    )
    ...
    class Meta:
        model = Aprovacao
        fields = [
            'id', 'solicitacao', 'solicitacao_titulo',
            'usuario_aprovador', 'usuario_aprovador_nome',
            'status_decisao', 'status_display',
            'justificativa', 'data_decisao'
        ]
...
class EventoGoogleCalendarSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'google_event_id', 'solicitacao', 'solicitacao_titulo',
            'titulo', 'data_inicio_google', 'data_fim_google',
            'link_meet', 'data_criacao', 'data_atualizacao'
        ]
        read_only_fields = [
...
class DisponibilidadeFormadoresSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'formador', 'formador_nome', 'data', 'codigo',
            'codigo_display', 'observacoes', 'data_atualizacao'
        ]
...
class LogAuditoriaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LogAuditoria
        fields = [
            'id', 'usuario', 'usuario_nome', 'acao',
            'entidade_afetada_id', 'detalhes', 'data_hora'
        ]
...
class DeslocamentoSerializer(serializers.ModelSerializer):
    ...
    municipio_origem_nome = serializers.CharField(
        source='municipio_origem.nome',
        read_only=True
    )
    municipio_destino_nome = serializers.CharField(
        source='municipio_destino.nome', 
        read_only=True
    )
    ...
    class Meta:
        model = Deslocamento
        fields = [
            'id', 'formador', 'formador_nome',
            'municipio_origem', 'municipio_origem_nome',
            'municipio_destino', 'municipio_destino_nome',
            'data_saida', 'data_chegada', 'tempo_estimado',
