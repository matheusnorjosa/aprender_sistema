...
from core.services.integrations.google_calendar import (
    GoogleCalendarService, is_enabled as google_enabled
)
...
class GoogleCalendarManagementService:
    ...
    @transaction.atomic
    def create_event_for_controle(self, solicitacao: Solicitacao, user) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(f"Erro na criação manual pelo Controle - solicitação {solicitacao.id}: {e}")
    ...
    def _create_calendar_event_manual(self, solicitacao: Solicitacao, user) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Evento criado com sucesso para solicitação {solicitacao.id}: {api_result['id']}")
            ...
            logger.error(f"Erro ao criar evento no calendar para solicitação {solicitacao.id}: {e}")
            raise
    ...
    def update_calendar_event(self, solicitacao: Solicitacao, user) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao atualizar evento para solicitação {solicitacao.id}: {e}")
    ...
    def delete_calendar_event(self, solicitacao: Solicitacao, user) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao deletar evento para solicitação {solicitacao.id}: {e}")
    ...
    def bulk_create_events_for_controle(self, user) -> Dict[str, Any]:
        ...
        for solicitacao in Solicitacao.objects.filter(status=SolicitacaoStatus.PRE_AGENDA):
            if not EventoGoogleCalendar.objects.filter(solicitacao=solicitacao).exists():
        ...
        for solicitacao in solicitacoes_pendentes:
            try:
                ...
                logger.error(f"Erro na criação em lote - solicitação {solicitacao.id}: {e}")
    ...
    def get_pre_agenda_summary(self) -> Dict[str, Any]:
        ...
        for solicitacao in solicitacoes_pre_agenda:
            ...
            solicitacao_data = {
                ...
                'municipio': solicitacao.municipio.nome if solicitacao.municipio else '',
                'data_inicio': solicitacao.data_inicio.isoformat() if solicitacao.data_inicio else None,
                'formadores': [f.nome for f in solicitacao.formadores.all()],
                'solicitante': solicitacao.usuario_solicitante.username if solicitacao.usuario_solicitante else ''
            }
