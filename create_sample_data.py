...
from core.models import (
    Usuario, Formador, Municipio, Projeto, TipoEvento, Setor,
    Solicitacao, SolicitacaoStatus, FormadoresSolicitacao
)
...
def create_sample_data():
    ...
    for mes_offset in range(12):  # 12 meses atrás até hoje
        ...
        for i in range(num_eventos):
            ...
            formadores_selecionados = choice(formadores) if num_formadores == 1 else \
                                    list(set([choice(formadores) for _ in range(num_formadores)]))
    ...
    print(f"Solicitacoes aprovadas: {Solicitacao.objects.filter(status=SolicitacaoStatus.APROVADO).count()}")
    print(f"Solicitacoes pendentes: {Solicitacao.objects.filter(status=SolicitacaoStatus.PENDENTE).count()}")
    print(f"Solicitacoes reprovadas: {Solicitacao.objects.filter(status=SolicitacaoStatus.REPROVADO).count()}")
