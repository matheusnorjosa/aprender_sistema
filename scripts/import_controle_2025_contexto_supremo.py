...
class ImportadorControle2025ContextoSupremo:
    ...
    def processar_aba_acoes(self, spreadsheet):
        ...
        try:
            ...
            for i, row in enumerate(data_rows):
                ...
                logger.info(
                    f"🏢 Ação {i+1}: {acao_data.get('Município', 'N/A')} - {acao_data.get('Projeto', 'N/A')}"
                )
    ...
    def processar_aba_compras(self, spreadsheet):
        ...
        try:
            ...
            for i, row in enumerate(data_rows):
                ...
                logger.info(
                    f"🛒 Compra {i+1}: {compra_data.get('Produto', 'N/A')} - {compra_data.get('Município', 'N/A')}"
                )
    ...
    def processar_aba_filtro_prod(self, spreadsheet):
        ...
        try:
            ...
            for i, row in enumerate(data_rows):
                ...
                logger.info(
                    f"🔍 Filtro {i+1}: {filtro_data.get('Município - UF', 'N/A')} - {filtro_data.get('Projeto', 'N/A')}"
                )
    ...
    def processar_aba_formacoes(self, spreadsheet):
        ...
        try:
            ...
            for i, row in enumerate(data_rows):
                ...
                logger.info(
                    f"📚 Formação {i+1}: {formacao_data.get('Município', 'N/A')} - {formacao_data.get('Projeto', 'N/A')}"
                )
    ...
    def processar_aba_dat(self, spreadsheet):
        ...
        try:
            ...
            for i, row in enumerate(data_rows):
                ...
                logger.info(
                    f"💻 DAT {i+1}: {dat_data.get('Município', 'N/A')} - {dat_data.get('Tipo de Ação', 'N/A')}"
                )
    ...
    def processar_aba_cadastros(self, spreadsheet):
        ...
        try:
            ...
            for i, row in enumerate(data_rows):
                ...
                logger.info(
                    f"📝 Cadastro {i+1}: {cadastro_data.get('Município', 'N/A')} - {cadastro_data.get('Tipo de Ação', 'N/A')}"
                )
    ...
    def gerar_relatorio_final(self):
        ...
        logger.info(
            f"   - Compras processadas: {self.estatisticas['compras_processadas']}"
        )
        logger.info(
            f"   - Filtro PROD processado: {self.estatisticas['filtro_prod_processado']}"
        )
        logger.info(
            f"   - Formações processadas: {self.estatisticas['formacoes_processadas']}"
        )
        logger.info(f"   - DAT processado: {self.estatisticas['dat_processado']}")
        logger.info(
            f"   - Cadastros processados: {self.estatisticas['cadastros_processados']}"
        )
        ...
        with open(
            "relatorio_importacao_controle_2025.json", "w", encoding="utf-8"
        ) as f:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Importar dados da planilha Controle 2025"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
