...
def exchange_code_for_token(auth_code):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"ERRO: Falha na troca de token: {response.status_code}")
...
def save_credentials(token_data):
    ...
    try:
        ...
        print("SUCESSO: Credenciais salvas em google_authorized_user.json")
        return True
...
def main():
    if len(sys.argv) != 2:
        print("Uso: python complete_auth_simple.py 'URL_DE_REDIRECIONAMENTO'")
        sys.exit(1)

    redirect_url = sys.argv[1]

    print("COMPLETANDO AUTENTICACAO GOOGLE OAUTH2")
    print("=" * 50)
    ...
    print(f"Codigo extraido: {auth_code[:10]}...")
    ...
    if save_credentials(token_data):
        print("\nAUTENTICACAO RENOVADA COM SUCESSO!")
        print("\nPROXIMOS PASSOS:")
        print("1. Teste a conexao:")
        print(
            "   python manage.py import_super_colunas_e_t --spreadsheet-key 16ul8qvHb-1CRs5Z7zYcVP9Rh2munCefWWNsAiJfZYYU --dry-run --verbose"
        )
        print("\n2. Se funcionar, execute sem --dry-run para importar os dados reais")
    else:
        print("ERRO: Falha ao salvar credenciais")
        sys.exit(1)
