...
class DataComparisonAnalyst:
    ...
    def carregar_dados_fonte_unica(self):
        ...
        if os.path.exists('fonte_unica_dados/dados_principais'):
            arquivos = [f for f in os.listdir('fonte_unica_dados/dados_principais') 
                       if f.startswith('fonte_unica_dados_') and f.endswith('.json')]
            
            if arquivos:
                arquivo_mais_recente = max(arquivos)
                caminho_arquivo = f'fonte_unica_dados/dados_principais/{arquivo_mais_recente}'
                ...
                try:
                    ...
                    print(f"   • Total de eventos: {len(self.dados_fonte_unica.get('eventos', []))}")
                    print(f"   • Municípios: {len(self.dados_fonte_unica.get('municipios', {}))}")
                    print(f"   • Projetos: {len(self.dados_fonte_unica.get('projetos', {}))}")
                    print(f"   • Formadores: {len(self.dados_fonte_unica.get('formadores', {}))}")
    ...
    def analisar_estrutura_dados_originais(self):
        ...
        for nome_aba, dados in self.dados_originais.items():
            ...
            for campo in campos_disponiveis:
                valores_nao_vazios = [registro.get(campo, '') for registro in dados if registro.get(campo, '')]
            ...
            for campo in campos_importantes:
                if campo in campos_preenchidos:
                    percentual = analise_estrutura[nome_aba]['percentual_preenchimento'][campo]
    ...
    def comparar_metricas(self, metricas_originais, metricas_fonte_unica):
        ...
        comparacao = {
            'total_registros': {
                ...
                'diferenca': metricas_fonte_unica['total_registros'] - metricas_originais['total_registros'],
            ...
            'municipios_unicos': {
                ...
                'diferenca': metricas_fonte_unica['municipios_unicos'] - len(metricas_originais['municipios_unicos']),
                'percentual_diferenca': 0
            },
            'projetos_unicos': {
                'original': len(metricas_originais['projetos_unicos']),
                'fonte_unica': metricas_fonte_unica['projetos_unicos'],
                'diferenca': metricas_fonte_unica['projetos_unicos'] - len(metricas_originais['projetos_unicos']),
                'percentual_diferenca': 0
            },
            'formadores_unicos': {
                'original': len(metricas_originais['formadores_unicos']),
                'fonte_unica': metricas_fonte_unica['formadores_unicos'],
                'diferenca': metricas_fonte_unica['formadores_unicos'] - len(metricas_originais['formadores_unicos']),
                'percentual_diferenca': 0
            }
        }
        ...
        for metrica, dados in comparacao.items():
            if dados['original'] > 0:
                dados['percentual_diferenca'] = (dados['diferenca'] / dados['original']) * 100
        ...
        print(f"     - Diferença: {comparacao['total_registros']['diferenca']} ({comparacao['total_registros']['percentual_diferenca']:+.1f}%)")
        ...
        print(f"     - Diferença: {comparacao['municipios_unicos']['diferenca']} ({comparacao['municipios_unicos']['percentual_diferenca']:+.1f}%)")
        ...
        print(f"     - Diferença: {comparacao['projetos_unicos']['diferenca']} ({comparacao['projetos_unicos']['percentual_diferenca']:+.1f}%)")
        ...
        print(f"     - Diferença: {comparacao['formadores_unicos']['diferenca']} ({comparacao['formadores_unicos']['percentual_diferenca']:+.1f}%)")
    ...
    def identificar_discrepancias(self, metricas_originais, metricas_fonte_unica):
        ...
        if metricas_originais['total_registros'] != metricas_fonte_unica['total_registros']:
            discrepancia = {
    ...
    def gerar_relatorio_comparativo(self, analise_estrutura, metricas_originais, metricas_fonte_unica, comparacao, discrepancias):
        ...
        relatorio = {
            ...
            'resumo_executivo': {
                ...
                'percentual_diferenca': comparacao['total_registros']['percentual_diferenca'],
                'total_discrepancias': len(discrepancias),
                'discrepancias_alta_severidade': len([d for d in discrepancias if d['severidade'] == 'alta'])
            },
    ...
    def _gerar_conclusoes(self, comparacao, discrepancias):
        ...
        if comparacao['total_registros']['diferenca'] == 0:
            conclusoes.append("✅ INTEGRIDADE: Total de registros mantido - nenhuma perda de dados")
        elif comparacao['total_registros']['diferenca'] > 0:
            conclusoes.append(f"⚠️ INTEGRIDADE: {comparacao['total_registros']['diferenca']} registros adicionados na fonte única")
        else:
            conclusoes.append(f"❌ INTEGRIDADE: {abs(comparacao['total_registros']['diferenca'])} registros perdidos na fonte única")
        
        # Análise de qualidade
        discrepancias_alta = [d for d in discrepancias if d['severidade'] == 'alta']
        if not discrepancias_alta:
            conclusoes.append("✅ QUALIDADE: Nenhuma discrepância de alta severidade identificada")
        else:
            conclusoes.append(f"❌ QUALIDADE: {len(discrepancias_alta)} discrepâncias de alta severidade identificadas")
        
        # Análise de consistência
        if len(discrepancias) <= 2:
            conclusoes.append("✅ CONSISTÊNCIA: Dados altamente consistentes entre origem e fonte única")
        elif len(discrepancias) <= 5:
            conclusoes.append("⚠️ CONSISTÊNCIA: Algumas inconsistências identificadas, mas dentro do aceitável")
        else:
            conclusoes.append("❌ CONSISTÊNCIA: Múltiplas inconsistências identificadas - revisão necessária")
    ...
    def _gerar_relatorio_markdown(self, relatorio, arquivo):
        ...
        with open(arquivo, 'w', encoding='utf-8') as f:
            ...
            f.write(f"- **Total de registros originais:** {resumo['total_registros_original']}\n")
            f.write(f"- **Total de registros fonte única:** {resumo['total_registros_fonte_unica']}\n")
            f.write(f"- **Diferença:** {resumo['diferenca_registros']} ({resumo['percentual_diferenca']:+.1f}%)\n")
            f.write(f"- **Total de discrepâncias:** {resumo['total_discrepancias']}\n")
            f.write(f"- **Discrepâncias alta severidade:** {resumo['discrepancias_alta_severidade']}\n\n")
            ...
            for metrica, dados in comparacao.items():
                f.write(f"| {metrica.replace('_', ' ').title()} | {dados['original']} | {dados['fonte_unica']} | {dados['diferenca']:+d} | {dados['percentual_diferenca']:+.1f}% |\n")
            
            f.write(f"\n## Discrepâncias Identificadas\n\n")
            for i, discrepancia in enumerate(relatorio['discrepancias'], 1):
                f.write(f"### {i}. {discrepancia['tipo'].replace('_', ' ').title()}\n\n")
                f.write(f"- **Descrição:** {discrepancia['descricao']}\n")
                f.write(f"- **Severidade:** {discrepancia['severidade']}\n")
                if 'detalhes' in discrepancia:
                    f.write(f"- **Detalhes:** {', '.join(discrepancia['detalhes'][:10])}\n")
                f.write(f"\n")
    ...
    def executar_analise_completa(self):
        ...
        discrepancias = self.identificar_discrepancias(metricas_originais, metricas_fonte_unica)
        
        # 8. Gerar relatório comparativo
        relatorio = self.gerar_relatorio_comparativo(
            analise_estrutura, metricas_originais, metricas_fonte_unica, 
            comparacao, discrepancias
        )
        
        print(f"\n🎉 ANÁLISE COMPARATIVA CONCLUÍDA COM SUCESSO!")
        print(f"   ✅ Dados originais analisados: {metricas_originais['total_registros']} registros")
        print(f"   ✅ Dados fonte única analisados: {metricas_fonte_unica['total_registros']} registros")
        print(f"   ✅ Discrepâncias identificadas: {len(discrepancias)}")
...
def main():
    ...
    if relatorio:
        print(f"\n📊 RESUMO FINAL:")
        resumo = relatorio['resumo_executivo']
        print(f"   • Integridade dos dados: {resumo['diferenca_registros']:+d} registros")
        print(f"   • Qualidade: {resumo['discrepancias_alta_severidade']} discrepâncias críticas")
        print(f"   • Consistência: {resumo['total_discrepancias']} discrepâncias totais")
