...
class BaseMigrationCommand(BaseCommand, ABC):
    ...
    def setup_logging(self) -> logging.Logger:
        ...
        if not logger.handlers:
            ...
            handler = logging.FileHandler(
                settings.BASE_DIR / 'logs' / 'migration.log',
                encoding='utf-8'
            )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--worksheet',
            type=str,
            help='Nome da aba da planilha para processar'
        )
        ...
        parser.add_argument(
            '--dry-run',
            action='store_true',
            help='Simula migração sem salvar dados'
        )
    ...
    def load_records_as_dicts(self, source_file: str, worksheet_name: str) -> List[Dict[str, str]]:
        """
        ...
        try:
            ...
            if self.verbose:
                self.stdout.write(f"Carregados {len(records)} registros de {worksheet_name}")
    ...
    def bulk_create_objects(self, model_class: Type[models.Model], 
    ...
    def print_final_stats(self):
        ...
        if self.stats['total_records'] > 0:
            success_rate = ((self.stats['created'] + self.stats['updated']) / self.stats['total_records']) * 100
    ...
    def log_error(self, message: str, record: Dict[str, Any] = None):
        ...
        if record:
            ...
            context = {k: v for k, v in record.items() if k in ['Nome', 'Email', 'CPF', 'id']}
