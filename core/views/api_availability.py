    ...
    def post(self, request):
        try:
            ...
            if not formador_ids:
                return JsonResponse({
                    'success': False,
                    'error': 'Formadores devem ser selecionados'
                })
            ...
            try:
                ...
                return JsonResponse({
                    'success': False,
                    'error': f'Erro ao processar datas: {str(e)}'
                })
            ...
            for solicitacao in conflitos.get('solicitacoes', []):
                formadores_nomes = ", ".join([f.nome for f in solicitacao.formadores.all()])
                conflict_details.append({
                    'type': 'evento',
                    'code': 'E',
...
class FormadorDetailsAPI(View):
    ...
    def get(self, request):
        ...
        if not formador_ids:
            return JsonResponse({
                'success': False,
                'error': 'IDs de formadores não fornecidos'
            })
        ...
        try:
            ...
            return JsonResponse({
                'success': True,
                'formadores': list(formadores)
            })
            
        except Exception as e:
            return JsonResponse({
                'success': False,
                'error': f'Erro ao buscar formadores: {str(e)}'
            })