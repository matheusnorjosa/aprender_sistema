        ...
        for categoria, info in projetos_estrutura.items():
            ...
            try:
                    ...
                    if len(linha) >= len(cabecalhos):
                        ...
                        if projeto_linha:
                            if coordenador_linha:
                                coordenadores_por_projeto[projeto_linha].add(
                                    coordenador_linha
                                )
                ...
                for projeto in sorted(projetos_encontrados):
                    ...
                    for form in sorted(
                        list(formadores_por_projeto[projeto])[:5]
                    ):  # Primeiros 5
                        print(f"         - {form}")
                    if form_count > 5:
...
def analisar_projetos_pequenos(gc):
    ...
    try:
        ...
        cabecalhos = dados[0]
        projetos_pequenos = defaultdict(
            lambda: {"coordenadores": set(), "formadores": set(), "total_registros": 0}
        )
...
def main():
    ...
    analise_completa = {
        ...
        "projetos_pequenos": {
            k: {
                **v,
                "coordenadores": list(v["coordenadores"]),
                "formadores": list(v["formadores"]),
            }
            for k, v in projetos_pequenos.items()
        },
