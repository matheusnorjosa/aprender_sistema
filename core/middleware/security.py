...
class SecurityHeadersMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        if not (
            ...
            or getattr(settings, "IS_STAGING", False)
        ):
            return response
    ...
    def process_request(self, request):
        ...
        if self.request_counts[key] > 100:
            ...
            return HttpResponseTooManyRequests(
                "Rate limit exceeded. Try again later.", content_type="text/plain"
            )
...
class AuditLogMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        is_sensitive = any(
            path.startswith(sensitive) for sensitive in self.SENSITIVE_PATHS
        )
        ...
        if is_sensitive:
            audit_data = {
                ...
                and int((timezone.now().timestamp() - request._start_time) * 1000)
                or None,
            }
