...
class Command(BaseCommand):
    ...
    def _create_or_update_event(self, row: Dict, aba_name: str, source: str, row_num: int) -> str:
    ...
    def _determine_status(self, data_str: str, aba_name: str, aprovacao: str, cancelado: bool) -> str:
        """
        ...
        if data_evento <= self.corte_temporal:
            return SolicitacaoStatus.CANCELADO if cancelado else SolicitacaoStatus.REALIZADO

        # Eventos futuros
        if aba_name.strip().lower() == 'super' and (aprovacao or '').strip().upper() == 'SIM':
    ...
    def _get_or_create_usuario(self, nome: str, origem: str = 'planilha') -> Optional[Usuario]:
        """
        ...
        if not self.dry_run:
            ...
            if self.verbose:
                self.stdout.write(
                    self.style.WARNING(f"⚠️  Usuário provisório criado: {nome} ({username})")
                )

        return usuario

    def _add_participants(self, solicitacao: Solicitacao, coordenador: Usuario, coord_acompanha: str, row: Dict):
        ...
        if convidados_str:
            ...
            convidados = [c.strip() for c in convidados_str.replace(';', ',').split(',')]
