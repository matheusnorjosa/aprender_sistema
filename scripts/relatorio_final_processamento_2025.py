...
def gerar_relatorio_final():
    ...
    for arquivo, nome in arquivos:
        if os.path.exists(f'/app/{arquivo}'):
            try:
                ...
                relatorio['planilhas_processadas'][nome]['total_linhas'] = linhas_planilha
            ...
            relatorio['problemas_encontrados'].append(f'Arquivo {arquivo} não encontrado')
    ...
    if 'Usuários' in relatorio['planilhas_processadas']:
        if relatorio['planilhas_processadas']['Usuários']['status'] != '✅ Processada com sucesso':
            relatorio['problemas_encontrados'].append('Planilha de Usuários: Erro 404 - ID incorreto ou não compartilhada')
    
    # Salvar relatório
    with open('/app/relatorio_final_processamento_2025.json', 'w', encoding='utf-8') as f:
    ...
    print(f'✅ Processadas com sucesso: {relatorio["resumo_geral"]["planilhas_processadas"]}')
    print(f'❌ Com erro: {relatorio["resumo_geral"]["planilhas_com_erro"]}')
    print(f'📈 Taxa de sucesso: {relatorio["resumo_geral"]["taxa_sucesso"]}')
    print(f'📝 Total de linhas processadas: {relatorio["resumo_geral"].get("total_linhas", 0):,}')
    print()
