    ...
    def get_context_data(self, **kwargs):
        ...
        if municipio_id:
            municipio_obj = (
                ...
                .filter(id=municipio_id)
                .first()
            )
        elif municipio_nome_param:
            municipio_obj = (
                Municipio.objects.prefetch_related("solicitacao_set", "usuario_set")
                .filter(nome__iexact=municipio_nome_param)
                .order_by("uf")
                .first()
            )
        ...
        context.update(
            {
                ...
                "municipio_id_selecionado": (
                    str(municipio_obj.id) if municipio_obj else ""
                ),
                "municipio_nome_selecionado": (
                    municipio_obj.nome if municipio_obj else municipio_nome_param
                ),
                "uf_selecionada": uf_param
                or (municipio_obj.uf if municipio_obj else ""),
        ...
        context.update(
            {
                ...
                "solicitacoes_aprovadas_ano": stats_gerais[
                    "solicitacoes_aprovadas_ano"
                ],
...
class DiretoriaAPIMetricsView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        metrics = {
            "solicitacoes_mes": Solicitacao.objects.select_related(
                "municipio", "projeto", "tipo_evento", "solicitante"
            )
            .prefetch_related("formadores")
            .filter(data_inicio__gte=data_limite)
            .count(),
            "aprovacoes_mes": Solicitacao.objects.select_related(
                "municipio", "projeto", "tipo_evento", "solicitante"
            )
            .prefetch_related("formadores")
            .filter(data_inicio__gte=data_limite, status=SolicitacaoStatus.APROVADO)
            .count(),
        }
...
class DashboardChartsAPIView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get_monthly_evolution(self):
        ...
        eventos_por_mes = (
            Solicitacao.objects.filter(data_inicio__gte=inicio_ano)
    ...
    def get_top_formadores(self):
        ...
        formadores = (
            FormadoresSolicitacao.objects.select_related(
                "usuario", "usuario__area_atuacao", "solicitacao"
            )
            .filter(
    ...
    def get_distribuicao_setores(self):
        ...
        setores = (
            get_optimized_solicitacao_queryset()
            .filter(data_inicio__gte=inicio_ano, status=SolicitacaoStatus.APROVADO)
    ...
    def get_municipios_atendidos(self):
        ...
        municipios = (
            get_optimized_solicitacao_queryset()
            .filter(
                data_inicio__gte=tres_meses_atras, status=SolicitacaoStatus.APROVADO
            )
    ...
    def get_tipos_evento(self):
        ...
        for tipo in tipos:
            data.append(
                {"tipo": tipo["tipo_evento__nome"], "quantidade": tipo["quantidade"]}
            )
    ...
    def get_projetos_stats(self):
        ...
        for projeto in projetos:
            data.append(
                {"nome": projeto["projeto__nome"], "eventos": projeto["eventos"]}
            )
...
class ChartJSServeView(View):
    ...
    def get(self, request):
        chart_path = os.path.join(
            settings.BASE_DIR, "core", "static", "core", "js", "chart.min.js"
        )
    ...
    def get(self, request):
        ...
        try:
            ...
            return self.get_error_response(
                f"Erro ao buscar dados de coordenadores: {str(e)}"
            )
