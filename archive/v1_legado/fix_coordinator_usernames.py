...
def fix_coordinator_usernames():
    ...
    for coordenador in coordenadores:
        ...
        if (
            not old_username
            or Usuario.objects.filter(username=old_username)
            .exclude(id=coordenador.id)
            .exists()
        ):
            new_username = generate_username(
                coordenador.first_name, coordenador.last_name
            )
            ...
            if new_username:
                ...
                try:
                    ...
                    updated_count += 1
                    print(
                        f"   ✅ {coordenador.first_name} {coordenador.last_name}: {old_username} -> {new_username}"
                    )
                except Exception as e:
                    error_count += 1
                    print(
                        f"   ❌ Erro ao atualizar {coordenador.first_name} {coordenador.last_name}: {e}"
                    )
            else:
                error_count += 1
                print(
                    f"   ❌ Não foi possível gerar username para {coordenador.first_name} {coordenador.last_name}"
                )
            ...
            if new_username:
                ...
                try:
                    ...
                    if error_count <= 5:  # Mostrar apenas os primeiros 5 erros
                        print(
                            f"   ❌ Erro ao atualizar {usuario.first_name} {usuario.last_name}: {e}"
                        )
            else:
                error_count += 1
                if error_count <= 5:
                    print(
                        f"   ❌ Não foi possível gerar username para {usuario.first_name} {usuario.last_name}"
                    )
...
def validate_usernames():
    ...
    duplicates = (
        Usuario.objects.values("username")
