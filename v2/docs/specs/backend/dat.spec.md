...
sources_of_truth:
  ...
  - v2/backend/apps/core/services/dat_codigos.py
  - v2/backend/apps/core/services/dat_registro_split.py
...
- **Unicidade (constraints de BD, não podem ser violadas):**
  ...
  - `DATCadastro`: único por `(municipio, projeto_geral, plataforma, ano)` — `unique_dat_cadastro_mun_proj_plat_ano`, `nulls_distinct=False` (migration 0093): um cadastro por plataforma/**ano**. O `ano` é derivado da 1ª etapa com data no import (fallback etapa1→4); `ano=NULL` = pendente.
...
> ⚠️ **Drift FE↔BE** (`ACHADOS_REAIS.md`, épicos #1655 e #1654). Parte dos itens abaixo já foi **corrigida** (marcada *corrigido em #NNNN*); os demais descrevem o comportamento **real** ainda vivo em produção, não o pretendido.

- **Datas do formulário de Registros — corrigido em #1638 (`M16-07`).** Havia duas falhas: no *FORMAR*, `handleSave` formatava **apenas** `reuniao_dat` e os demais `DatePicker` caíam no spread `...values` como Dayjs (datetime ISO completo → **400** no `DateField`); no *AVALIAR*, o form usava nomes **no singular** (`alunos_recebidos_data`…) que o DRF descartava em silêncio. Hoje `buildDATRegistroPayload` (`DATRegistrosPage.tsx`) formata os **4 campos FORMAR** via `toISODate` (`YYYY-MM-DD`) e os **3 AVALIAR** via `toISODateArray` + `<DatePicker multiple>`, batendo com o `DateField`/`JSONField` de lista do model/serializer (`models/dat_registro.py`). O serializer nunca foi a causa — os 7 campos sempre estiveram corretos em `serializers/dat_registro.py` (`DATRegistroListSerializer`/`Create`/`Update`).
- **Choices de status UI↔backend — corrigido em #1712 (`M16-08`, issue #1639).** O front tinha um único `STATUS_OPTIONS` (3 valores) usado inclusive em `turma_formar_status`, o que violava os choices do backend. Hoje esse conjunto foi partido em dois em `DATRegistros/constants.tsx`: `TURMA_STATUS_OPTIONS` (3 — `criada, pendente, erro`, casa `TURMA_STATUS_CHOICES`) e `ETAPA_STATUS_OPTIONS` (5 — `concluido, pendente, em_andamento, nao_aplicavel, erro`, casa `STATUS_CHOICES`). O campo `turma_formar_status` usa `TURMA_STATUS_OPTIONS` e as etapas FORMAR/AVALIAR usam `ETAPA_STATUS_OPTIONS` (`DATRegistrosPage.tsx`); o contrato é travado pelo teste `statusOptionsContract.test.ts`.
