...
from core.models import Solicitacao, Aprovacao, FormadoresSolicitacao, SolicitacaoStatus, EventoGoogleCalendar
...
class SolicitacaoToolset(ModelQueryToolset):
    ...
    accessible_fields = {
        'Solicitacao': [
            'id', 'titulo_evento', 'data_inicio', 'data_fim', 'status',
            'projeto', 'municipio', 'tipo_evento', 'usuario_solicitante',
            'formadores', 'data_solicitacao', 'coordenador_acompanha',
            'observacoes', 'numero_encontro_formativo'
        ],
        'Aprovacao': [
            'id', 'solicitacao', 'usuario_aprovador', 'data_aprovacao',
            'status_decisao', 'justificativa'
        ],
        'FormadoresSolicitacao': [
            'id', 'solicitacao', 'formador'
        ],
        'EventoGoogleCalendar': [
            'id', 'solicitacao', 'provider_event_id', 'html_link', 'meet_link',
            'status_sincronizacao', 'data_criacao'
        ],
    }
    ...
    def can_create(self, model, user=None):
        ...
        if model == Solicitacao:
            ...
            return user.groups.filter(name__in=['coordenador', 'superintendencia', 'admin']).exists()
    ...
    def can_update(self, model, obj, user=None):
        ...
        if model == Solicitacao:
            ...
            if obj.usuario_solicitante == user and obj.status == SolicitacaoStatus.PENDENTE:
                return True
    ...
    def can_delete(self, model, obj, user=None):
        ...
        if (model == Solicitacao and 
            obj.usuario_solicitante == user and 
            obj.status == SolicitacaoStatus.PENDENTE and
    ...
    def get_solicitacoes_by_status(self, status, user=None):
        ...
        return list(queryset.values(
            ...
            'projeto__nome', 'municipio__nome', 'usuario_solicitante__username'
        ))
    ...
    def get_monthly_statistics(self, year, month, user=None):
        ...
        queryset = Solicitacao.objects.filter(
            ...
            data_inicio__date__lt=end_date
        )
        queryset = self.filter_queryset_by_user(queryset, Solicitacao, user)
