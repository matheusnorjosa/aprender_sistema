...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--health-check", action="store_true", help="Run system health checks"
        )
        ...
        parser.add_argument(
            "--metrics-test", action="store_true", help="Test metrics collection"
        )

        parser.add_argument(
            "--logging-test", action="store_true", help="Test structured logging"
        )

        parser.add_argument(
            "--sentry-test", action="store_true", help="Test Sentry integration"
        )

        parser.add_argument(
            "--load-test", action="store_true", help="Run load testing simulation"
        )

        parser.add_argument(
            "--all", action="store_true", help="Run all monitoring tests"
        )
    ...
    def test_health_checks(self):
        ...
        try:
            ...
            self.stdout.write(
                f"[{self._get_status_symbol(db_result['status'])}] Database: {db_result['status']}"
            )
            if "response_time_ms" in db_result:
                self.stdout.write(
                    f"     Response time: {db_result['response_time_ms']}ms"
                )

            # Cache check
            cache_result = health_checker._check_cache()
            self.stdout.write(
                f"[{self._get_status_symbol(cache_result['status'])}] Cache: {cache_result['status']}"
            )

            # Disk space check
            disk_result = health_checker._check_disk_space()
            self.stdout.write(
                f"[{self._get_status_symbol(disk_result['status'])}] Disk Space: {disk_result['status']}"
            )
            ...
            self.stdout.write(
                f"[{self._get_status_symbol(memory_result['status'])}] Memory: {memory_result['status']}"
            )
            ...
            self.stdout.write(
                f"[{self._get_status_symbol(overall_status)}] Overall Status: {overall_status}"
            )
            ...
            if health_status["checks"]:
                ...
                for name, result in health_status["checks"].items():
                    ...
                    self.stdout.write(
                        f"     {status_symbol} {name}: {result['status']} ({result.get('response_time_ms', 0):.1f}ms)"
                    )
                    if "error" in result:
            ...
            self.stdout.write(
                f"[OK] Summary generated: {summary['healthy_checks']}/{summary['total_checks']} checks healthy"
            )

            if summary["failing_checks"]:
                self.stdout.write(
                    f"     Failing checks: {', '.join(summary['failing_checks'])}"
                )
        ...
        try:
            ...
            if alerts:
                ...
                for alert in alerts[:3]:  # Show first 3
                    self.stdout.write(
                        f"     Alert: {alert['operation']} took {alert['duration_ms']}ms"
                    )
            else:
    ...
    def test_metrics_collection(self):
        ...
        try:
            ...
            prometheus_metrics.record_http_request(
                "GET", "/api/solicitacoes", 200, 0.15
            )
            prometheus_metrics.record_http_request(
                "POST", "/api/solicitacoes", 201, 0.25
            )
    ...
    def test_sentry_integration(self):
        ...
        try:
            if not sentry_monitoring.enabled:
                ...
                self.stdout.write(
                    "     Configure SENTRY_DSN in settings for full functionality"
                )
                return
            ...
            sentry_monitoring.capture_performance_issue(
                "test_slow_query", 1500.0, 1000.0  # 1.5 seconds  # 1 second threshold
            )
        ...
        try:
            ...
            for i in range(50):
                ...
                if prometheus_metrics.enabled:
                    ...
                    prometheus_metrics.record_http_request(
                        "POST", f"/api/{operation}", status, base_time
                    )
            ...
            if perf_summary:
                total_operations = sum(
                    stats["count"] for stats in perf_summary.values()
                )
                avg_response_time = (
                    sum(
                ...
                sorted_ops = sorted(
                    perf_summary.items(), key=lambda x: x[1]["avg_ms"], reverse=True
                )
