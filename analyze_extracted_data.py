...
def analyze_data_quality():
    ...
    try:
        ...
        for field, data in stats.items():
            preenchimento = (data['preenchidos'] / data['total']) * 100 if data['total'] > 0 else 0
...
def analyze_cpf_patterns():
    ...
    try:
        ...
        duplicados = [cpf for cpf, count in cpf_patterns['duplicados'].items() if count > 1]
...
def analyze_email_patterns():
    ...
    try:
        ...
        duplicados = [email for email, count in email_stats['duplicados'].items() if count > 1]
...
def analyze_perfil_distribution():
    ...
    try:
        ...
        for ws_name, ws_data in worksheets.items():
            ...
            records = json_loader.get_records_as_dicts('extracted_usuarios.json', ws_name)
