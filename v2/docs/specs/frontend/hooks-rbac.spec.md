...
- **Capabilities (autorizacao, policy-pura)**: [`v2/frontend/src/hooks/useCapabilities.ts`](../../../frontend/src/hooks/useCapabilities.ts) — funcao pura `computeCapabilities(policies)` + hook `useCapabilities(policies)`. **Sem fallback legacy e sem organograma**: cada flag `can*` espelha 1:1 uma policy key, cobrindo as 21 `PUBLIC_POLICY_KEYS` do backend (`rbac/policies.py`). E o hook que o [`AppSidebar`](../../../frontend/src/components/AppSidebar.tsx) usa desde o #1270 (`AppSidebar.tsx`).
- **Identidade (display-only)**: [`v2/frontend/src/hooks/useIdentity.ts`](../../../frontend/src/hooks/useIdentity.ts) — `computeIdentity(user)` / `useIdentity(user)`. Setor/funcao + `displayName` + `badge`. O proprio arquivo declara que **nao decide autorizacao**; `useIdentity.*` em contexto de guard e smell.
- **Identidade e capabilities legacy (`@deprecated`)**: [`v2/frontend/src/hooks/usePermissions.ts`](../../../frontend/src/hooks/usePermissions.ts) — funcao pura `computePermissions(user)` + hook memoizado `usePermissions(user)`. Marcado `@deprecated` (remocao prevista na onda 4.5) por misturar "quem e" e "o que pode". Computa flags de role (`isCoordenador`, `isFormador`, `isGerente`), setor (`inSuperintendencia`, `inDAT`, `inControle`, `inDiretoria`, `inGerencia`) e capability derivada (`canControle`, `canDAT`, `canDisponibilidade`, `canDashboard*`, `canSeeAllSectors`) a partir de `user.setores` / `user.funcoes` / `user.is_superuser`. **Continua em uso**: alimenta os guards `allow=` dos composites de rota (ver [`pages.spec.md`](./pages.spec.md)).
- **Camada de traducao (policy + legacy)**: [`v2/frontend/src/hooks/useCanAccess.ts`](../../../frontend/src/hooks/useCanAccess.ts) — `computeAccess(policies, legacy)` + hook `useCanAccess`. Derived flags: `canAccessApprovals`, `canAccessBlocks`, `canCreateSolicitation`, `canViewComprasDashboard`, `canManageInternalActions`, e o predicado generico `can(key)`. Depois do #1271 sobrou pouco call site: `App.tsx` o chama **sem** flags legacy, e `AppRoutes.tsx` passa **apenas** `canBloqueios` — logo `canCreateSolicitation` e `canViewComprasDashboard` operam ali como policy pura, sem fallback.
- **Guard de rota**: [`v2/frontend/src/components/access/RequirePolicy.tsx`](../../../frontend/src/components/access/RequirePolicy.tsx) — consome as flags/policies e nega com um `Result` generico. A decisao e fail-secure quando faltam `policy` e `allow`.
...
- **Clientes HTTP**: [`v2/frontend/src/api/me.ts`](../../../frontend/src/api/me.ts) (`getMyPolicies()` le `/api/me/policies/`) e [`v2/frontend/src/api/auth.ts`](../../../frontend/src/api/auth.ts) (`login`/`logout`; `checkAuth()` le `/api/me/`, mas **quem o `App.tsx` usa no boot e `getMe()` de `api/availability.ts`** — `checkAuth` sobrou como checagem pontual de `is_superuser` em `AdminDAT/UsuariosPage.tsx` e `AdminDAT/GruposPage.tsx`).
...
- **Paridade de flags `useCapabilities` x `PUBLIC_POLICY_KEYS`.** As 21 keys do backend (`rbac/policies.py`) tem flag nomeada correspondente em `computeCapabilities` (`useCapabilities.ts`). Policy publica nova = flag nova + caso no teste de completude.
...
- `<RequirePolicy policy="<key>" policies={policies}>` ou `<RequirePolicy allow={<boolean>}>`; `allow` tem precedencia, e sem nenhum dos dois o resultado e **negar** (`RequirePolicy.tsx`).
...
- **`M05-07` (#1655) — `HomePage` nao usa a policy.** [`pages/Home/HomePage.tsx`](../../../frontend/src/pages/Home/HomePage.tsx) chama `computeAccess(policies)` so para aprovacoes; para criacao de solicitacao ele usa `perms.canCoordenador`, isto e, `is_superuser || isCoordenador || inDAT` (`canCoordenador` em `usePermissions.ts`). Os cards "Enviar Solicitacao" e "Minhas Solicitacoes" e o KPI correspondente seguem essa flag de organograma, enquanto as rotas de destino exigem a policy `create_solicitation` (`AppRoutes.tsx`). Consequencia: quem tem a policy sem estar no organograma nao ve o atalho, e quem esta no organograma sem a policy ve um atalho que a rota nega. E o caso-tipo do invariante "quem e != o que pode" ainda nao cumprido.
