...
from core.models import (
    ...
    Solicitacao, EventoGoogleCalendar, LogAuditoria
)
...
class CompleteEntityMappingAuditor:
    ...
    def audit_coordenadores(self):
        ...
        try:
            ...
            coordenadores_grupo = Usuario.objects.filter(
                groups__name='coordenador'
            ).select_related('setor', 'municipio').distinct()
            ...
            self.audit_results['coordenadores'] = {
                ...
                'dados': list(coordenadores.values('id', 'first_name', 'last_name', 'email', 'cargo', 'setor__nome', 'municipio__nome'))
            }
            ...
            print(f"  🏢 Com setor: {coordenadores.filter(setor__isnull=False).count()}")
            print(f"  🏘️ Com município: {coordenadores.filter(municipio__isnull=False).count()}")
    ...
    def audit_municipios(self):
        ...
        try:
            ...
            self.audit_results['municipios'] = {
                ...
                'ufs_unicas': municipios.values_list('uf', flat=True).distinct().count(),
            ...
            print(f"  🗺️ Com UF: {municipios.exclude(uf__isnull=True).exclude(uf='').count()}")
            print(f"  🌍 UFs únicas: {municipios.values_list('uf', flat=True).distinct().count()}")
    ...
    def audit_colecoes(self):
        ...
        try:
            ...
            colecao_models = [model for model in models if 'colecao' in model.__name__.lower()]
    ...
    def audit_produtos(self):
        ...
        try:
            ...
            produto_models = [model for model in models if 'produto' in model.__name__.lower()]
    ...
    def audit_ids_produtos(self):
        ...
        try:
            ...
            for model in models:
                ...
                if any('produto' in field.lower() and 'id' in field.lower() for field in fields):
        ...
        try:
            ...
            self.audit_results['eventos'] = {
                ...
                'eventos_calendar': {
                    ...
                    'dados': list(eventos_calendar.values('id', 'titulo', 'data_inicio', 'data_fim')[:10])
                }
            }
    ...
    def audit_relationships(self):
        ...
        try:
            ...
            relationships = {
                ...
                'usuario_municipio': Usuario.objects.filter(municipio__isnull=False).count(),
                'projeto_setor': Projeto.objects.filter(setor__isnull=False).count(),
                'solicitacao_projeto': Solicitacao.objects.filter(projeto__isnull=False).count(),
                'solicitacao_municipio': Solicitacao.objects.filter(municipio__isnull=False).count(),
                'solicitacao_tipo_evento': Solicitacao.objects.filter(tipo_evento__isnull=False).count(),
            }
    ...
    def audit_data_integrity(self):
        ...
        try:
            ...
            solicitacoes_sem_projeto = Solicitacao.objects.filter(projeto__isnull=True).count()
            if solicitacoes_sem_projeto > 0:
                integrity_issues.append(f"{solicitacoes_sem_projeto} solicitações sem projeto")
    ...
    def audit_services_mapping(self):
        ...
        try:
            ...
            from core.services import (
                ...
                DashboardService, MunicipioService, SetorService,
                TipoEventoService
            )
    ...
    def print_complete_audit_report(self):
        ...
        # Resumo por entidade
        print(f"\n📋 RESUMO POR ENTIDADE:")
        entities = ['coordenadores', 'municipios', 'projetos', 'colecoes', 'produtos', 'ids_produtos', 'eventos', 'tipos_evento']
        for entity in entities:
            if entity in self.audit_results and self.audit_results[entity]:
                if isinstance(self.audit_results[entity], dict) and 'total' in self.audit_results[entity]:
