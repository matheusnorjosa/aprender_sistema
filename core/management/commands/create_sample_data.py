...
from core.models import (
    Usuario, Municipio, Projeto, TipoEvento, Formador,
    Solicitacao, SolicitacaoStatus, Aprovacao, AprovacaoStatus,
    DisponibilidadeFormadores, Deslocamento, LogAuditoria
)
...
class Command(BaseCommand):
    ...
    def criar_solicitacoes(self, quantidade):
        ...
        for i in range(quantidade):
            ...
            if solicitacao.status != SolicitacaoStatus.PENDENTE:
                superintendentes = Usuario.objects.filter(groups__name='superintendencia')
                if superintendentes.exists():
                    # Mapear status da solicitação para status de aprovação
                    status_aprovacao = AprovacaoStatus.APROVADO if solicitacao.status == SolicitacaoStatus.APROVADO else AprovacaoStatus.REPROVADO
    ...
    def criar_bloqueios(self, quantidade):
        ...
        if not formadores:
            self.stdout.write('Nenhum formador encontrado, pulando criação de bloqueios.')
            return
        
        tipos_bloqueio = ['Férias', 'Licença', 'Compromisso pessoal', 'Viagem', 'Outros']
    ...
    def criar_deslocamentos(self, quantidade):
        ...
        usuarios = list(Usuario.objects.filter(groups__name__in=['formador', 'coordenador']))
        municipios = list(Municipio.objects.all())
        ...
        for i in range(quantidade):
            ...
            if formadores_disponiveis.exists():
                ...
                num_formadores = random.randint(1, min(3, formadores_disponiveis.count()))
                formadores_selecionados = random.sample(list(formadores_disponiveis), num_formadores)
