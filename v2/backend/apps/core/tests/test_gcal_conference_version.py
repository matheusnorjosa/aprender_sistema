...
class TestConferenceDataVersion:
    ...
    def test_insert_passes_conference_data_version(self, mock_sa, mock_build):
        ...
        payload = {
            ...
            "conferenceData": {
                "createRequest": {"requestId": "meet-123-abc", "conferenceSolutionKey": {"type": "hangoutsMeet"}}
            },
        }

        # Chamar insert
        result = client.insert(calendar_id="primary", event_id="test-event", payload=payload)

        # Verificar que insert foi chamado com conferenceDataVersion=1
        mock_events.insert.assert_called_once()
        call_kwargs = mock_events.insert.call_args[1]

        assert "conferenceDataVersion" in call_kwargs, "conferenceDataVersion deve ser passado para insert()"
        assert (
            call_kwargs["conferenceDataVersion"] == 1
    ...
    def test_patch_passes_conference_data_version(self, mock_sa, mock_build):
        ...
        mock_patch.execute.return_value = {"id": "test-event-456", "hangoutLink": "https://meet.google.com/xyz-uvw-rst"}
        ...
        payload = {
            ...
            "conferenceData": {
                "createRequest": {"requestId": "meet-456-def", "conferenceSolutionKey": {"type": "hangoutsMeet"}}
            },
        }

        # Chamar update
        result = client.update(calendar_id="primary", event_id="test-event", payload=payload)
        ...
        assert "conferenceDataVersion" in call_kwargs, "conferenceDataVersion deve ser passado para patch()"
        assert (
            call_kwargs["conferenceDataVersion"] == 1
        ), f"conferenceDataVersion deve ser 1, got {call_kwargs.get('conferenceDataVersion')}"
    ...
    def test_patch_works_without_conference_data(self, mock_sa, mock_build):
        ...
        # Payload SEM conferenceData
        payload = {"summary": "Updated Event Title", "description": "New description"}
