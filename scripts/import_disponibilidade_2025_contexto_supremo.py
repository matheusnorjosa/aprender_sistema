...
class ImportadorDisponibilidade2025ContextoSupremo:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.abas_config = {
            'MENSAL': {
                ...
                'metricas': {
                    'ch_formacoes': ['AN', 'AO', 'AP'],
                    'linhas': [4, 5]
                },
                'dados': {
                    'municipio_data': ['AM', 'AO', 'AP', 'AQ'],
                    'linha': 7
                },
                'deslocamentos': {
                    'colunas': ['AM', 'AN', 'AO'],
    ...
    def processar_aba_mensal(self, spreadsheet):
        ...
        try:
            ...
            # Filtro de mês (B3)
            try:
                mes_filtro = all_values[2][1] if len(all_values) > 2 and len(all_values[2]) > 1 else None
            ...
            try:
                formadores_filtro = all_values[2][38] if len(all_values) > 2 and len(all_values[2]) > 38 else None
                ...
                for linha in config['metricas']['linhas']:
                    if len(all_values) > linha:
                        ...
                        if len(linha_data) > 39:  # Coluna AN (índice 39)
                            ...
                            logger.info(f"📈 Linha {linha}: Mês={mes}, Ano={ano}, Posição={posicao_mes}")
            except Exception as e:
            ...
            try:
                ...
                if len(linha_municipios) > 38:  # Coluna AM (índice 38)
                    municipio = linha_municipios[38] if len(linha_municipios) > 38 else ''
                    ...
                    logger.info(f"🏘️ Município: {municipio}, Data: {data}, Início: {inicio}, Fim: {fim}, Tipo: {tipo}")
            except Exception as e:
                ...
                for linha in config['deslocamentos']['linhas']:
                    if len(all_values) > linha:
                        ...
                        if len(linha_data) > 38:  # Coluna AM (índice 38)
                            ...
                            logger.info(f"🚗 Linha {linha}: Origem={origem}, Data={data}, Destino={destino}")
            except Exception as e:
        ...
        try:
            ...
            if len(all_values) > 0:
                ...
                for i, row in enumerate(all_values[1:], 1):
                    ...
                    logger.info(f"👤 Formador: {formador_nome}, Carga Anual: {carga_anual}")
    ...
    def gerar_relatorio_final(self):
        ...
        logger.info(f"   - Interface gráfica processada: {self.estatisticas['interface_grafica_processada']}")
        logger.info(f"   - Carga horária processada: {self.estatisticas['carga_horaria_processada']}")
        logger.info(f"   - Deslocamentos processados: {self.estatisticas['deslocamentos_processados']}")
        logger.info(f"   - Bloqueios processados: {self.estatisticas['bloqueios_processados']}")
        ...
        with open('relatorio_importacao_disponibilidade_2025.json', 'w', encoding='utf-8') as f:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Importar dados da planilha Disponibilidade 2025')
