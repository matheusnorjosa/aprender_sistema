...
class JSONDataLoader:
    ...
    def load_extracted_data(self, filename: str, use_cache: bool = True) -> Dict[str, Any]:
        """
    ...
    def get_worksheet_data(self, filename: str, worksheet_name: str) -> Dict[str, Any]:
        ...
        if worksheet_name not in worksheets:
            ...
            raise KeyError(f"Aba '{worksheet_name}' não encontrada. Disponíveis: {available}")
        
        return worksheets[worksheet_name]
    
    def get_worksheet_records(self, filename: str, worksheet_name: str, 
    ...
    def get_records_as_dicts(self, filename: str, worksheet_name: str) -> List[Dict[str, str]]:
        """
        ...
        for record in records:
            ...
            for i, header in enumerate(headers):
                ...
                record_dict[header] = value.strip() if isinstance(value, str) else str(value)
    ...
    def get_file_stats(self, filename: str) -> Dict[str, Any]:
        ...
        if 'worksheets' in data:
            ...
            total_records = sum(
                ws_data.get('total_rows', 0) 
                for ws_data in data['worksheets'].values()
            )
