...
class GeradorExcelEstruturado:
    ...
    def carregar_dados_tratados(self):
        ...
        if os.path.exists(arquivo_organizados):
            ...
            print(f"✅ Dados organizados carregados: {len(self.dados_organizados)} categorias")
    ...
    def gerar_excel_relatorio_tratamento(self):
        ...
        resumo_data = [
            ...
            ['Total de Abas Processadas', len(self.relatorio_tratamento.get('dados_originais', {}))],
            ['Total de Transformações', len(self.relatorio_tratamento.get('transformacoes_aplicadas', []))],
            ['Total de Problemas', len(self.relatorio_tratamento.get('problemas_encontrados', []))],
            ['Total de Registros Originais', sum(len(dados) for dados in self.dados_tratados.values())],
            ['Total de Registros Tratados', sum(len(dados) for dados in self.dados_tratados.values())]
        ]
        ...
        transformacoes_data = [['Aba', 'Registros Originais', 'Registros Tratados', 'Transformações']]
        
        for transformacao in self.relatorio_tratamento.get('transformacoes_aplicadas', []):
    ...
    def gerar_excel_relatorio_final(self):
        ...
        resumo_data = [
            ['Métrica', 'Valor'],
            ['Total de Registros Tratados', resumo_executivo.get('total_registros_tratados', 0)],
        ...
        categorias_data = [['Categoria', 'Total Registros', 'Total Abas', 'Percentual do Total']]
        
        for categoria, stats in resumo_executivo.get('estatisticas_por_categoria', {}).items():
        ...
        qualidade_data = [['Categoria', 'Aba', 'Total Registros', 'Total Colunas', 'Campos Importantes']]
        
        for categoria, stats in self.relatorio_final.get('analise_qualidade', {}).items():
        ...
        for i, recomendacao in enumerate(self.relatorio_final.get('recomendacoes_finais', []), 1):
    ...
    def gerar_excel_consolidado_geral(self):
        ...
        dashboard_data = [
            ...
            ['Total de Planilhas', len(set(dados[0]['planilha_original'] for dados in self.dados_tratados.values() if dados))],
...
if __name__ == "__main__":
    ...
    if sucesso:
        ...
        print("📋 Dados tratados organizados em planilhas Excel com formatação profissional")
    else:
