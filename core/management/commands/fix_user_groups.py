...
class Command(BaseCommand):
            ...
            with transaction.atomic():
                ...
                for usuario in usuarios_sem_grupo:
                    try:
                        ...
                        if grupo_sugerido:
                            ...
                            if not dry_run:
                                try:
                                    ...
                                    self.stdout.write(
                                        self.style.ERROR(f'  Grupo "{grupo_sugerido}" nao encontrado para {usuario.username}')
                                    )
                                    stats['sem_grupo'] += 1
                                    continue
                        ...
                        self.stdout.write(
                            self.style.ERROR(f'Erro ao processar {usuario.username}: {str(e)}')
                        )
                        stats['sem_grupo'] += 1
    ...
    def identificar_grupo(self, usuario):
        ...
        if any([
            ...
            re.match(r'coordenacao\d+@', email)  # coordenacao1@, coordenacao2@, etc.
        ]):
            return 'coordenador'
        ...
        if any([
            ...
            username in ['mateus', 'matheusadm', 'admin']  # Usuários administrativos
        ]):
        ...
        if any([
            ...
            'admin' in email,
            email.endswith('@aprendereditora.com.br') and any([
                'sistema' in email,
                'tech' in email,
                'ti' in email
            ])
