...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Erro na importacao: {str(e)}"))
            raise
    ...
    def load_extracted_data(self):
        ...
        try:
            with open(
                "archive/temp_data/extracted_all_data.json", "r", encoding="utf-8"
            ) as f:
            ...
            usuarios_count = len(
                self.extracted_data.get("usuarios", {})
                .get("worksheets", {})
                .get("Ativos", {})
                .get("data", [])
            )
    ...
    def clear_existing_data(self):
        ...
        Usuario.objects.select_related("municipio", "projeto").prefetch_related(
    ...
    def create_basic_structure(self):
        ...
        for municipio_nome in municipios_basicos:
            municipio, created = Municipio.objects.get_or_create(
                nome=municipio_nome, defaults={"ativo": True}
            )
            if created and not self.dry_run:
        ...
        for tipo_data in tipos_evento:
            tipo, created = TipoEvento.objects.get_or_create(
                nome=tipo_data["nome"],
                defaults={"online": tipo_data["online"], "ativo": True},
            )
            if created and not self.dry_run:
        ...
        for projeto_data in projetos_basicos:
            try:
                ...
                projeto, created = Projeto.objects.get_or_create(
                    nome=projeto_data["nome"], defaults={"setor": setor, "ativo": True}
                )
                if created and not self.dry_run:
                    ...
                    if created:
                        ...
                        if grupo_name:
                            try:
                                ...
                                self.stdout.write(
                                    self.style.WARNING(
                                        f"Grupo nao encontrado: {grupo_name}"
                                    )
                                )
    ...
    def create_formadores(self):
        ...
        usuarios_formadores = (
            ...
            .filter(groups=formadores_group)
        )
        created_count = 0
                ...
                try:
                    ...
                    for group_name in user_data["groups"]:
                        try:
                            ...
                            self.stdout.write(
                                self.style.WARNING(
                                    f"Grupo nao encontrado: {group_name}"
                                )
                            )
                    ...
                    created_count += 1
                    self.stdout.write(
                        f"  Usuario de teste criado: {user_data['username']}"
                    )

                except Exception as e:
                    self.stdout.write(
                        self.style.WARNING(
                            f"Erro ao criar usuario {user_data['username']}: {str(e)}"
                        )
                    )
            else:
                created_count += 1  # Simular para dry-run
