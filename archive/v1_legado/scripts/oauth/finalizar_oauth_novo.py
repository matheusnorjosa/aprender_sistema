...
def finalizar_oauth(auth_code):
    ...
    try:
        ...
        try:
            ...
            print(
                f"[INFO] Aba Super: {super_ws.row_count} linhas x {super_ws.col_count} colunas"
            )
            ...
            for i, row in enumerate(sample_data, 2):
                if len(row) >= aprovacao_col:
                    valor_aprovacao = (
                        row[aprovacao_col - 1].strip() if row[aprovacao_col - 1] else ""
                    )
                    ...
                    print(
                        f"  Linha {i:2d}: {status:8} | '{valor_aprovacao:3}' | {municipio[:20]}"
                    )
...
if __name__ == "__main__":
    ...
    auth_code = (
        "4/0AVGzR1CVd11r_77kfFv61mE-2pQ5pr4GdNeTwLhQLRsYG3HECICwOxVL-Re6GfbPwK9peg"
    )
