...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--list-prompts", action="store_true", help="List available system prompts"
        )

        parser.add_argument("--category", type=str, help="Filter prompts by category")

        parser.add_argument(
            "--generate-prompt", type=str, help="Generate specific prompt by name"
        )

        parser.add_argument(
            "--prompt-context", type=str, help="JSON context for prompt generation"
        )

        parser.add_argument(
            "--test-templates", action="store_true", help="Test prompt templates"
        )
        ...
        parser.add_argument(
            "--prompt-name", type=str, help="Name for custom prompt registration"
        )

        parser.add_argument("--all", action="store_true", help="Run all TIER 3 tests")
    ...
    def handle(self, *args, **options):
        ...
        if options["all"]:
            ...
            self.generate_prompt(
                options["generate_prompt"], options.get("prompt_context")
            )
    ...
    def test_prompts(self):
        ...
        try:
            ...
            success = register_custom_prompt(
                "test_prompt", test_prompt, "testing", "Test prompt for validation"
            )
            ...
            perf_prompt = get_educational_prompt(
                "analyze_student_performance", perf_context
            )
            if perf_prompt:
                self.stdout.write("[OK] Student performance prompt generated")
            else:
                self.stdout.write(
                    "[ERROR] Failed to generate student performance prompt"
                )
            ...
            schedule_prompt = get_educational_prompt(
                "optimize_class_schedule", schedule_context
            )
            if schedule_prompt:
                self.stdout.write("[OK] Schedule optimization prompt generated")
            else:
                self.stdout.write(
                    "[ERROR] Failed to generate schedule optimization prompt"
                )
            ...
            try:
                ...
                if formador:
                    formador_prompt = (
                        educational_prompts.generate_formador_analysis_prompt(formador)
                    )
                    if formador_prompt:
                        self.stdout.write("[OK] Formador analysis prompt generated")
                    else:
                        self.stdout.write(
                            "[WARNING] Formador prompt generation returned None"
                        )
                else:
        ...
        try:
            ...
            if google_apis_manager.enabled:
                ...
                if google_apis_manager.credentials:
                    ...
                    self.stdout.write(
                        "     Set up OAuth2 credentials for full functionality"
                    )
            else:
                self.stdout.write("[WARNING] Google APIs not available")
                self.stdout.write(
                    "     Install google-api-python-client for full functionality"
                )
            ...
            if calendar_service.calendar_service:
                ...
                try:
                    ...
                    self.stdout.write(
                        f"[OK] Conflict checking available (found {len(conflicts)} conflicts)"
                    )
                except Exception as e:
                    self.stdout.write(
                        f"[INFO] Conflict checking needs authentication: {str(e)[:50]}..."
                    )
            ...
            if drive_service.drive_service:
                ...
                self.stdout.write(
                    "     Folder creation and permission management available"
                )
            else:
            ...
            try:
                ...
                if solicitacao:
                    self.stdout.write(
                        f"[OK] Found test solicitacao: {solicitacao.titulo_evento}"
                    )
                    ...
                    self.stdout.write(
                        f"[OK] Attendees list generation works ({len(attendees)} attendees)"
                    )
    ...
    def test_templates(self):
        ...
        try:
            ...
            lesson_plan = template_library.render_template(
                "lesson_plan", lesson_context
            )
            if lesson_plan:
            ...
            rubric = template_library.render_template(
                "assessment_rubric", rubric_context
            )
            if rubric:
                self.stdout.write(
                    "[OK] Assessment rubric template rendered successfully"
                )
            else:
            ...
            feedback = template_library.render_template(
                "feedback_analysis", feedback_context
            )
            if feedback:
                self.stdout.write(
                    "[OK] Feedback analysis template rendered successfully"
                )
            else:
        ...
        try:
            ...
            if context_json:
                try:
                    ...
                    self.stdout.write(
                        "[WARNING] Invalid JSON context, using empty context"
                    )
    ...
    def register_prompt(self, name, template):
        ...
        try:
            success = register_custom_prompt(
                name, template, "custom", f"Custom prompt registered via CLI: {name}"
            )
