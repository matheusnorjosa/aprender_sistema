            ...
            with transaction.atomic():
                ...
                for project_name, sector_name in project_sector_mapping.items():
                    try:
                        ...
                        self.fixes_applied.append(f"Projeto '{project_name}' vinculado ao setor '{sector_name}'")
                        print(f"  ✅ Projeto '{project_name}' → Setor '{sector_name}'")
                        
                    except (Projeto.DoesNotExist, Setor.DoesNotExist) as e:
                        self.errors.append(f"Erro ao vincular projeto {project_name}: {str(e)}")
                        print(f"  ❌ Erro: {str(e)}")
        ...
        try:
            with transaction.atomic():
                ...
                for user in users_without_sector:
                    ...
                    if 'superintendência' in user.email.lower() or 'superintendencia' in user.email.lower():
                    ...
                    try:
                        ...
                        self.fixes_applied.append(f"Usuário '{user.first_name} {user.last_name}' → Setor '{sector_name}'")
                        print(f"  ✅ {user.first_name} {user.last_name} → Setor '{sector_name}'")
                        
                    except Setor.DoesNotExist:
                        self.errors.append(f"Setor '{sector_name}' não encontrado para usuário {user.first_name}")
                ...
                for cpf, count in duplicate_cpfs:
                    ...
                    if users_with_cpf.count() > 1:
                        for i, user in enumerate(users_with_cpf[1:], 1):
                            ...
                            user.save()
                            self.fixes_applied.append(f"Usuário duplicado {user.first_name} marcado como inativo")
                            print(f"    ✅ Usuário duplicado {user.first_name} marcado como inativo")
            else:
    ...
    def fix_missing_data(self):
        ...
        if users_without_cpf.exists():
            ...
            for user in users_without_cpf:
                if user.username and user.username.isdigit() and len(user.username) == 11:
                    # Username é um CPF válido
                    user.cpf = user.username
                    user.save()
                    self.fixes_applied.append(f"CPF {user.username} adicionado ao usuário {user.first_name}")
                    print(f"    ✅ CPF {user.username} adicionado ao usuário {user.first_name}")
                else:
