...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--test-crypto", action="store_true", help="Test cryptographic functions"
        )

        parser.add_argument(
            "--password", type=str, help="Password to analyze or encrypt with"
        )
        ...
        parser.add_argument(
            "--encrypt-text", type=str, help="Text to encrypt using modern cryptography"
        )

        parser.add_argument("--all", action="store_true", help="Run all TIER 2 tests")
    ...
    def test_analytics(self):
        ...
        try:
            ...
            if "error" not in perf_data:
                ...
                self.stdout.write(
                    f"     Formadores in data: {len(perf_data.get('formadores', []))}"
                )
            else:
            ...
            if "error" not in municipal_data:
                self.stdout.write(
                    f"[OK] Found {len(municipal_data.get('municipios', []))} municipalities"
                )
            else:
            ...
            if "error" not in dashboard_data:
                ...
                self.stdout.write(
                    f"     Statistics: {len(dashboard_data.get('statistics', {}))}"
                )
                self.stdout.write(
                    f"     Charts: {len(dashboard_data.get('charts', {}))}"
                )
        ...
        try:
            ...
            if password:
                ...
                for pwd in test_passwords:
                    ...
                    self.stdout.write(
                        f"     '{pwd}': {analysis['strength_level']} ({analysis['strength_score']}/100)"
                    )
            ...
            if security_utils.modern_crypto.enabled:
                ...
                # Test Scrypt
                scrypt_hash = security_utils.modern_crypto.hash_password_advanced(
                    test_pwd, "scrypt"
                )
                if scrypt_hash:
                    self.stdout.write(f"[OK] Scrypt hash: {scrypt_hash[:50]}...")

                    verified = security_utils.modern_crypto.verify_password_advanced(
                        test_pwd, scrypt_hash
                    )
                    self.stdout.write(f"[OK] Scrypt verification: {verified}")

                # Test PBKDF2
                pbkdf2_hash = security_utils.modern_crypto.hash_password_advanced(
                    test_pwd, "pbkdf2"
                )
                if pbkdf2_hash:
    ...
    def test_ciphers(self):
        ...
        try:
            ...
            vigenere_decrypted = ciphers.vigenere_cipher(
                vigenere_encrypted, vigenere_key, decrypt=True
            )
            ...
            atbash_decrypted = ciphers.atbash_cipher(
                atbash_encrypted
            ...
            rail_decrypted = ciphers.rail_fence_cipher(
                rail_encrypted, rail_key, decrypt=True
            )
        ...
        try:
            ...
            if charts:
                ...
                for chart_name, chart_data in charts.items():
                    ...
                    self.stdout.write(
                        f"  {chart_name.replace('_', ' ').title()}: {status}"
                    )
