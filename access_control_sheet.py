...
def access_control_sheet():
    ...
    try:
        ...
        for i, row in enumerate(all_data):
            municipio = row.get('Município', '') or row.get('município', '') or row.get('Municipio', '')
            ...
            if municipio or projeto or coordenador:
                control_data.append({
                    'linha': i + 2,  # +2 porque começa na linha 2 (linha 1 é cabeçalho)
                    'municipio': municipio,
                    'projeto': projeto,
        ...
        for i, item in enumerate(control_data[:10]):
            print(f"   {i+1}. Linha {item['linha']}: {item['municipio']} | {item['projeto']} | {item['coordenador']}")
        
        # Análise dos dados
        print("\n📊 ANÁLISE DOS DADOS:")
        
        # Municípios únicos
        municipios = set(item['municipio'] for item in control_data if item['municipio'])
        ...
        coordenadores = set(item['coordenador'] for item in control_data if item['coordenador'])
