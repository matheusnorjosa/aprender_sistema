...
class Command(BaseCommand):
    ...
    def _processar_planilhas(self):
        ...
        for planilha_path in planilhas_encontradas:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"      Erro ao processar {planilha_path.name}: {str(e)}"
                    )
                )
    ...
    def _extrair_dados_usuario(self, row, indices, projeto_nome):
        ...
        try:
            ...
            nome_completo = (
                row[indices.get("nome_completo", indices["nome"])]
                if indices.get("nome_completo") is not None
                else nome
            )
            ...
            grupo = (
                row[indices.get("grupo", indices["cargo"])]
                if indices.get("grupo") is not None
                else cargo
            )
            projeto = (
                row[indices["projeto"]]
                if indices.get("projeto") is not None
                else projeto_nome
            )
            ...
            grupo_django = self.cargo_para_grupo.get(
                cargo, self.cargo_para_grupo.get(grupo, "formador")
            )
                ...
                for formador_nome in formadores_agenda:
                    ...
                    if not self._usuario_ja_existe(formador_nome):
                        usuario_data = {
                            ...
                            "problemas": [
                                "Dados incompletos - apenas nome identificado"
                            ],
                        }
                        self._adicionar_usuario_unico(usuario_data)
    ...
    def _gerar_chave_usuario(self, usuario_data):
        ...
        if usuario_data["cpf"]:
            ...
            nome_normalizado = (
                re.sub(r"[^\w\s]", "", usuario_data["nome"]).lower().replace(" ", "_")
            )
        ...
        self.duplicatas_encontradas.append(
            {
                ...
                "fontes": [
                    usuario_existente["fonte_planilha"],
                    novo_usuario["fonte_planilha"],
                ],
            }
        )
    ...
    def _eliminar_duplicatas(self):
        ...
        for chave1, chave2 in duplicatas_por_nome:
            if chave1 in self.usuarios_unicos and chave2 in self.usuarios_unicos:
                self.stdout.write(
                    f"   🔗 Mesclando: {self.usuarios_unicos[chave1]['nome']} ↔ {self.usuarios_unicos[chave2]['nome']}"
                )
        ...
        with transaction.atomic():
            for usuario_data in self.usuarios_unicos.values():
                try:
                    ...
                    if self.verbose:
                        self.stdout.write(
                            f"   ❌ Erro ao processar {usuario_data['nome']}: {str(e)}"
                        )

        self.stdout.write(
            f"   ✅ Criados: {criados}, Atualizados: {atualizados}, Erros: {erros}"
        )
    ...
    def _criar_ou_atualizar_usuario(self, usuario_data):
        ...
        # Separar nome
        nome_parts = (
            usuario_data["nome_completo"].split()
            if usuario_data["nome_completo"]
            else usuario_data["nome"].split()
        )
        if nome_parts:
    ...
    def _exibir_estatisticas(self):
        ...
        self.stdout.write(f"\n📊 ESTATÍSTICAS FINAIS:")
        self.stdout.write(
            f"   Total de registros processados: {self.estatisticas['total_registros_processados']}"
        )
        self.stdout.write(
            f"   Usuários únicos encontrados: {self.estatisticas['usuarios_unicos_encontrados']}"
        )
        self.stdout.write(
            f"   Duplicatas eliminadas: {self.estatisticas['duplicatas_eliminadas']}"
        )
        self.stdout.write(
            f"   Emails faltantes: {self.estatisticas['emails_faltantes']}"
        )
        self.stdout.write(f"   CPFs inválidos: {self.estatisticas['cpfs_invalidos']}")
        self.stdout.write(
            f"   Nomes problemáticos: {self.estatisticas['nomes_problematicos']}"
        )
