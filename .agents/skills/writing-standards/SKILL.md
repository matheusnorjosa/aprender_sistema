...
description: Apply AS v2 technical-writing standards (clarity, brevity, active voice, Python/Django conventions). Use when writing a README, a docstring (PEP 257), a commit message, a PR description, an error message, or any technical doc.
---
...
### 1. Be Concise
Every word must earn its place. Delete redundant words ("in order to" → "to") and weak verbs ("is able to" → "can").
...
2. **Business context**:
   ...
   if user_has_any_perm(user, "pode_aprovar_superintendencia"):
...
fix(imports): handle empty CSV files in import_export_contract
...
### Avoid
(Conciseness and active voice are covered in Core Principles §1–§2.)
...
class SolicitacaoSerializer(serializers.ModelSerializer[Solicitacao]):
    ...
    Optional fields: observacoes, is_online
