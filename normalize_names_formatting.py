...
def normalize_municipios():
    ...
    for municipio in Municipio.objects.all():
        ...
        if old_name != new_name:
            ...
            existing = Municipio.objects.filter(nome=new_name, uf=municipio.uf).exclude(id=municipio.id).first()
            ...
            if existing:
                ...
                if solicitacoes.exists():
                    ...
                    print(f"   🔄 {old_name} -> {new_name} (mesclado, {solicitacoes.count()} solicitações)")
                municipio.delete()
...
def normalize_coordenadores():
    ...
    for usuario in Usuario.objects.filter(cargo='coordenador', is_active=True):
        ...
        if old_first_name != new_first_name or old_last_name != new_last_name:
            ...
            usuario.save()
            updated_count += 1
            print(f"   ✅ {old_first_name} {old_last_name} -> {new_first_name} {new_last_name}")
