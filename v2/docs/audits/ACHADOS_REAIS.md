...
| `M19-03` | **P2** | em andamento | DAT/PlanoFormacoes: excluir plano faz HARD DELETE em cascata de formações/acompanhamentos/provas — inclusive as concluídas (realizada=True, registro histórico) — sem bloqueio, sem soft-delete e sem auditoria (auditoria dinâmica 2026-08-17). Fix: `perform_destroy` bloqueia (409) quando há concluídas + audita a exclusão permitida; UI gateia o botão | Superintendência (`execute_restricted_operations`) + superuser | #1740 | — |
