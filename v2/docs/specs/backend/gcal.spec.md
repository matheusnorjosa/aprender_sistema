...
- **Modo de cliente vs modo de auth são variáveis distintas**: `GCAL_CLIENT` decide cliente real vs in-memory; `GCAL_AUTH_MODE` decide a autenticação no modo real. **Não existe `GCAL_CLIENT_MODE`** (o nome só sobrevive em doc arquivada). Defaults: `GCAL_CLIENT=fake` (`settings.py`), `GCAL_AUTH_MODE=service_account`. ⚠️ Os conjuntos `{fake, google}` e `{service_account, oauth}` são **convenção documental, não invariante**: nenhuma das duas é validada no boot (contraste com `GCAL_SEND_UPDATES`, validada em `settings.py`). Na prática, qualquer valor `!= "google"` cai no fake (`get_gcal_client_and_calendar_id`) e qualquer valor `!= "oauth"` cai em service account (`get_oauth_client_for_user`) — um typo degrada em silêncio.
...
- **Domínio restrito (OAuth)**: o callback só aceita contas `@{GCAL_ALLOWED_DOMAIN}` (default `aprendereditora.com.br`, lido de `os.getenv` em `exchange_code_for_tokens`); outras levantam `ValueError` na troca de código, que vira redirect `?google=error&reason=validation` (`google_oauth_callback`).
- **O binding de identidade do `state` do OAuth é feito pelo cache, não pelo sufixo do state** (era achado `M12-15` / issue #1652, **RESOLVIDO em #1760**, commit `9328227f`; issue CLOSED). Até então a checagem "este state é meu" era auto-declarada pelo portador: `validate_oauth_state` comparava o **terceiro campo do state** (`f"{csrf_token}|{return_to}|{user.id}"`, montado em `build_authorization_url` e 100% controlado por quem gerava a URL) com `request.user.id`, e o `user_id` gravado no cache nunca era lido. Hoje `validate_oauth_state` autentica o dono **pelo cache** — compara `cached_state.get("user_id")` com o `user_id` autenticado e **rejeita no mismatch** (`State ownership mismatch`); o sufixo do state virou dado morto, mantido só por compatibilidade de formato. O `csrf_token` continua one-time (`cache.delete`, consumo atômico anti-replay) com TTL de 600 s, e a credencial resultante é sempre gravada para `request.user`. Coberto por `test_validate_state_rejeita_dono_diferente_do_cache`.
...
Beat automático: `preview_then_apply_gcal`, chave `gcal-sync-every-5-minutes`, `schedule=300.0`. **Não fica em `config/celery.py`** e sim em [`config/settings.py`](../../../backend/config/settings.py), atrás de `FEATURE_AUTO_APPLY_ENABLED` (default `"0"` → `CELERY_BEAT_SCHEDULE = {}`). O merge com o schedule fixo do `celery.py` acontece em `app.conf.update(CELERY_BEAT_SCHEDULE=…)`. **Desabilitado por default.**
...
- **A seção "Equipe" da descrição sai vazia** (comportamento real, achado `M10-06`, épico #1666). `build_event_payload` monta a Seção 6 a partir da M2M **legada** `s.formadores` e da FK `coordenador`. Mas o fluxo de escrita atual não popula `formadores`: `views_solicitacao._update_formadores` grava `Participation(role="FORMADOR")` (via `bulk_create`) a partir de `extra_participants`. As únicas escritas em `.formadores` no repo estão em testes. Como o bloco só é emitido se `equipe_parts` for não-vazio (`gcal/payload.py`), eventos criados hoje publicam **sem a linha "Formador(es)"** — sobra no máximo o "Coordenador(a)". Contraste dentro do próprio arquivo: os **attendees** já usam a fonte certa (`build_attendees_for_solicitacao`, `s.participations.filter(role__in=...)`). A regra pretendida é ler `Participation` também na descrição.
- **Edição/exclusão bloqueadas enquanto `gcal_status=PENDING`** (era achado `M10-03` / issue #1625, **RESOLVIDO em #1777**, commit `f115dd45`; issue CLOSED). Até então `perform_destroy` só barrava `PUBLISHED` e existia, entre o `mark_gcal(PENDING)` (`publish_to_gcal`) e a execução da task, uma janela para alterar o conteúdo que seria publicado. Hoje `perform_destroy` barra `PUBLISHED` **e** `PENDING`; a **edição** durante `PENDING` é bloqueada em `SolicitacaoSerializer.validate()` — **não** em `perform_update` — porque o DRF chama `validate()` via `is_valid()` antes do `perform_update`. Ambos levantam `ValidationError`; a janela não existe mais.
- **`cancel` não valida `status == "aprovado"`** (só `publish`/`resync` validam — `publish_to_gcal`/`resync_to_gcal`); ele valida apenas "está publicado" (`cancel_from_gcal`). Intencional, mas não é simétrico com o resto.
