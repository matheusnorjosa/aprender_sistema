...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--worksheet-name", type=str, help="Nome da aba específica para analisar"
        )

        parser.add_argument(
            "--export-json", type=str, help="Caminho para exportar dados em JSON"
        )
        ...
        try:
            ...
            for ws_info in spreadsheet_info["worksheets"]:
                ...
                try:
                    ...
                    analysis_data["resumo_geral"]["total_registros"] += aba_analysis[
                        "total_registros"
                    ]
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"  [X] Erro ao processar aba {ws_name}: {e}")
                    )
                    analysis_data["abas_analisadas"][ws_name] = {
                        "erro": str(e),
                        "registros": 0,
            ...
            for key in analysis_data["resumo_geral"]:
                if isinstance(analysis_data["resumo_geral"][key], set):
                    analysis_data["resumo_geral"][key] = list(
                        analysis_data["resumo_geral"][key]
                    )
    ...
    def analyze_worksheet(self, ws_name, data, detailed=False):
        ...
        if detailed and sample_size > 0:
            self.stdout.write(
                f"  [*] Amostra de dados (primeiros {sample_size} registros):"
            )
    ...
    def analyze_event_worksheet(self, ws_name, data, analysis, detailed=False):
        ...
        analysis["observacoes"].append(
            f"Aba identificada como: SOLICITAÇÕES DE EVENTOS"
        )
        ...
        for record in data:
            for key in record.keys():
                ...
                if any(term in key_lower for term in ["data", "dia", "quando"]):
                    ...
                    term in key_lower for term in ["hora", "horario", "inicio", "fim"]
                ):
        ...
        if date_fields:
            analysis["observacoes"].append(
                f"Campos de data encontrados: {', '.join(date_fields)}"
            )
        if time_fields:
            analysis["observacoes"].append(
                f"Campos de horário encontrados: {', '.join(time_fields)}"
            )

    def analyze_config_worksheet(self, data, analysis, detailed=False):
        """Análise específica para aba de configurações"""
        analysis["observacoes"].append(
            "Aba identificada como: CONFIGURAÇÕES DO SISTEMA"
        )
        ...
        for record in data:
            for key, value in record.items():
                if value and any(
                    term in key.lower() for term in ["config", "parametro", "opcao"]
                ):
    ...
    def display_final_summary(self, analysis_data):
        ...
        self.stdout.write(
            f"[#] Total de registros analisados: {resumo['total_registros']}"
        )
        self.stdout.write(
            f"[+] Total de abas processadas: {len(analysis_data['abas_analisadas'])}"
        )
        self.stdout.write("")
        ...
        for aba_nome, aba_data in analysis_data["abas_analisadas"].items():
            if "erro" in aba_data:
                ...
                self.stdout.write(
                    f"  [OK] {aba_nome}: {registros} registros, {colunas} colunas"
                )
