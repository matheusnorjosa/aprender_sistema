...
class Command(BaseCommand):
        ...
        with connection.cursor() as cursor:
            ...
            if not self.dry_run and count > 0:
                ...
                for user_data in concatenated_users:
                    ...
                    if self.verbose:
                        self.stdout.write(f'  ❌ Removendo: "{first_name}" (ID: {user_id})')
    ...
    def _cleanup_user_relationships(self, user):
        ...
        with connection.cursor() as cursor:
            ...
            try:
                cursor.execute("DELETE FROM core_formador WHERE usuario_id = %s", [user_id])
    ...
    def _print_final_stats(self, stats, before_count, after_count):
        ...
        self.stdout.write(f'\n{"="*70}')
        self.stdout.write(self.style.SUCCESS('📊 RELATÓRIO FINAL - CORREÇÃO + DEDUPLICAÇÃO'))
