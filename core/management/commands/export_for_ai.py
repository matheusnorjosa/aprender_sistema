...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--domain',
            type=str,
            choices=['usuarios', 'municipios', 'formadores', 'projetos', 'business_context', 'all'],
        ...
        parser.add_argument(
            '--output',
            type=str,
            help='Diretório de saída (default: pasta atual)'
        )
    ...
    def handle(self, *args, **options):
        ...
        if domain == 'all':
            return self.export_all_domains(format_type, output_dir, for_oraculo, anonymize)
    ...
    def get_usuarios_data(self, anonymize: bool = False) -> Dict[str, Any]:
        ...
        usuarios = Usuario.objects.select_related('municipio').prefetch_related('groups')
        ...
        for user in usuarios:
            user_data = {
                ...
                'cpf': self.anonymize_cpf(user.cpf) if anonymize and user.cpf else user.cpf,
    ...
    def get_formadores_data(self, anonymize: bool = False) -> Dict[str, Any]:
        ...
        for formador in formadores:
            formador_data = {
                'id': str(formador.id),
                'nome': formador.nome,
                'email': self.anonymize_email(formador.email) if anonymize else formador.email,
                'municipio_base': formador.municipio_base.nome if formador.municipio_base else None,
                'especialidades': getattr(formador, 'especialidades', []),
                'ativo': formador.ativo,
                'data_cadastro': formador.created_at if hasattr(formador, 'created_at') else None
            }
    ...
    def export_business_summary(self, data: Dict[str, Any], domain: str, output_dir: str) -> str:
    ...
    def export_json(self, data: Dict[str, Any], domain: str, output_dir: str) -> str:
        ...
        filename = f"aprender_sistema_{domain}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        filepath = f"{output_dir}/{filename}"
        ...
        self.stdout.write(
            self.style.SUCCESS(f"OK JSON exportado: {filepath}")
        )
        return filepath
    
    def export_all_domains(self, format_type: str, output_dir: str, for_oraculo: bool, anonymize: bool):
        """Exporta todos os domínios"""
        
        domains = ['business_context', 'usuarios', 'municipios', 'formadores', 'projetos']
        exported_files = []
    ...
    def create_oraculo_consolidated_file(self, exported_files: List[str], output_dir: str):
        ...
        with open(consolidated_file, 'w', encoding='utf-8') as consolidated:
            ...
            consolidated.write("Este arquivo contém todos os dados estruturados do Aprender Sistema\n")
            consolidated.write("para alimentar o Oráculo AI como assistente de negócios inteligente.\n\n")
