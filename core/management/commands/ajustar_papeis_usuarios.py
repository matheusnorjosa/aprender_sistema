...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        coordenadores_conhecidos = {
            "Ellen Damares": {
                "email": "coordenacao41@aprendereditora.com.br",
                "cpf": "87521105320",
            },
            "Rafael Rabelo": {
                "email": "coordenacao60@aprendereditora.com.br",
                "cpf": "96288795372",
            },
            "Maria Nadir": {
                "nome_variacao": "Maria Nadir"
            },  # Aparece como coordenadora nas planilhas
        }
        ...
        with transaction.atomic():
            ...
            for nome, dados in coordenadores_conhecidos.items():
                try:
                    ...
                    if usuario:
                        if verbose:
                            self.stdout.write(
                                f"Definindo {usuario.nome_completo} como COORDENADOR"
                            )
                        ...
                        if verbose:
                            self.stdout.write(
                                f"AVISO: Coordenador {nome} nao encontrado"
                            )
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Erro ao processar coordenador {nome}: {str(e)}"
                        )
                    )
            ...
            for nome, dados in gerentes_conhecidos.items():
                try:
                    ...
                    if usuarios_nome.count() == 1:
                        ...
                        if verbose:
                            self.stdout.write(
                                f"Definindo {usuario.nome_completo} como GERENTE/SUPERINTENDENCIA"
                            )

                        if not dry_run:
                            usuario.groups.clear()
                            usuario.groups.add(
                                Group.objects.get(name="superintendencia")
                            )
                        ...
                        if verbose:
                            self.stdout.write(
                                f"AVISO: Gerente {nome} nao encontrado ou ambiguo"
                            )
            ...
            coordenadores_definidos = (
                Usuario.objects.select_related("municipio", "projeto")
                .prefetch_related("groups", "user_permissions")
                .filter(groups__name="coordenador")
            )
            gerentes_definidos = (
                Usuario.objects.select_related("municipio", "projeto")
                .prefetch_related("groups", "user_permissions")
                .filter(groups__name="superintendencia")
            )

            for usuario in formadores_ativos:
                # Não sobrescrever se já foi definido como coordenador ou gerente
                if (
                    not coordenadores_definidos.filter(id=usuario.id).exists()
                    and not gerentes_definidos.filter(id=usuario.id).exists()
                ):

                    if verbose:
                        self.stdout.write(
                            f"Mantendo {usuario.nome_completo} como FORMADOR"
                        )
            ...
            for papel, username in usuarios_especiais.items():
                try:
                    ...
                    if verbose:
                        self.stdout.write(
                            f"Definindo {usuario.username} como {papel.upper()}"
                        )
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Erro ao processar usuario especial {username}: {str(e)}"
                        )
                    )
        ...
        self.stdout.write(f"\nRELATORIO FINAL:")
        self.stdout.write(
            f"   Coordenadores definidos: {estatisticas['coordenadores_definidos']}"
        )
        self.stdout.write(
            f"   Gerentes/Superintendencia: {estatisticas['gerentes_definidos']}"
        )
        self.stdout.write(
            f"   Formadores mantidos: {estatisticas['formadores_mantidos']}"
        )
        self.stdout.write(
            f"   Usuarios especiais: {estatisticas['usuarios_especiais']}"
        )
