...
from core.models import Setor, Projeto
...
class Command(BaseCommand):
                ...
                if keyword in projeto_lower:
                    try:
                        ...
                        if projeto.setor != setor:
                            old_setor = projeto.setor.nome if projeto.setor else "SEM SETOR"
                            projeto.setor = setor
                            projeto.save()
                            moved_count += 1
                            self.stdout.write(
        ...
        for project_data in real_projects:
            try:
                ...
                if created:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'✓ Criado: {projeto.nome} → {setor.nome} ({status})')
                    )
                else:
                    existing_count += 1
                    ...
                    if projeto.setor != setor:
                        ...
                        projeto.save()
                        self.stdout.write(
                            self.style.WARNING(f'⚠ Atualizado: {projeto.nome} → {setor.nome}')
                        )
                    else:
    ...
    def show_final_summary(self):
        ...
        projetos_super = Projeto.objects.filter(setor__vinculado_superintendencia=True).count()
        projetos_direto = Projeto.objects.filter(setor__vinculado_superintendencia=False).count()
        
        self.stdout.write(f'\n=== ESTATÍSTICAS GERAIS ===')
        self.stdout.write(f'Total de projetos: {total_projetos}')
        self.stdout.write(f'Projetos que requerem aprovação superintendência: {projetos_super}')
        ...
        if total_projetos > 0:
            ...
            self.stdout.write(f'Distribuição: {pct_super:.1f}% superintendência, {pct_direto:.1f}% direto')