...
from .error_handling import safe_google_calendar_operation, GoogleCalendarError
...
class GoogleCalendarService:
    ...
    def _get_service(self):
        ...
        try:
            ...
            oauth_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'google_authorized_user.json')
            ...
            raise ValueError("Nenhuma credencial válida encontrada (OAuth2 ou Service Account)")
            
        except ImportError as e:
            logger.error(f"Bibliotecas Google não instaladas: {e}")
            raise RuntimeError("Instale: pip install google-api-python-client google-auth")
    ...
    def list_events(self, max_results: int = 10, time_min: Optional[datetime] = None) -> list:
        """
