    ...
    for sheet_name, sheet_data in all_data.items():
        ...
        for worksheet in sheet_data.get('worksheets', []):
            ...
            for record in records:
                ...
                for field in ['Nome', 'nome', 'Coordenador', 'coordenador', 'Formador', 'formador']:
                ...
                # Contar eventos
                if any(field in record for field in ['Data', 'data', 'Evento', 'evento']):
                    analysis['eventos'] += 1
...
def identify_data_sources(all_data):
    ...
    for sheet_name, sheet_data in all_data.items():
        ...
        for worksheet in sheet_data.get('worksheets', []):
            ...
            for record in records:
                # Usuários
                if any(field in record for field in ['Nome', 'nome', 'CPF', 'cpf', 'Email', 'email']):
                ...
                # Municípios
                if any(field in record for field in ['Município', 'município', 'Municipio', 'municipio']):
                ...
                # Eventos
                if any(field in record for field in ['Data', 'data', 'Evento', 'evento', 'Título', 'titulo']):
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            'data_sources_summary': {
                data_type: len(sources) for data_type, sources in data_sources.items()
            }
        }, f, ensure_ascii=False, indent=2)
...
def main():
    ...
    complete_file, summary_file = save_complete_mapping(all_data, analysis, data_sources)
