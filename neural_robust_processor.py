...
class NeuralRobustProcessor:
    ...
    def _process_usuarios_sheet(self, sheet_data: Dict[str, Any]):
        ...
        for aba in sheet_data.get('abas', []):
            ...
            status = 'ativo' if 'ativo' in aba_name.lower() else \
                    'inativo' if 'inativo' in aba_name.lower() else \
                    'pendente' if 'pendente' in aba_name.lower() else 'ativo'
    ...
    def _extract_pessoa_data(self, row: List[str], col_indices: Dict[str, int], status: str) -> Optional[Dict[str, Any]]:
        """Extrai dados de uma pessoa de uma linha"""
        
        # Extrair dados básicos
        nome_completo = self._get_cell_value(row, col_indices, ['nome completo', 'nome'])
        ...
        gerencia = self._get_cell_value(row, col_indices, ['gerência', 'gerencia', 'projeto'])
    ...
    def _get_cell_value(self, row: List[str], col_indices: Dict[str, int], possible_keys: List[str]) -> str:
    ...
    def _calculate_final_statistics(self):
        ...
        start_time = datetime.fromisoformat(self.processed_data['processing_stats']['start_time'])
    ...
    def _print_final_report(self):
        ...
        if processing_stats['warnings']:
            print(f"\n⚠️ AVISOS GERADOS:")
            for warning in processing_stats['warnings'][:5]:  # Mostrar apenas os primeiros 5
                print(f"  • {warning}")
