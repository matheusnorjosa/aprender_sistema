...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f"Erro ao mapear eventos: {str(e)}"))
    ...
    def _map_yearly_events(self, service, year: int) -> Dict[str, Any]:
        ...
        return {"events": all_events, "errors": errors, "total_batches": batch_count}
    ...
    def _generate_statistics(
        self, events: List[Dict[str, Any]], year: int
    ) -> Dict[str, Any]:
        ...
        return {
            "summary": {
                ...
                "meet_percentage": (
                    round((events_with_meet / total_events) * 100, 2)
                    if total_events > 0
                    else 0
                ),
    ...
    def _display_summary(self, result: Dict[str, Any]):
        ...
        self.stdout.write(
            f'Total de eventos: {self.style.SUCCESS(str(summary.get("total_events", 0)))}'
        )
        ...
        if monthly:
            ...
            for month, data in monthly.items():
                ...
                if count > 0:
                    self.stdout.write(
                        f"   - {month}: {self.style.SUCCESS(str(count))} eventos"
                    )
