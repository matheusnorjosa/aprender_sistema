...
class DataReferencesAuditor:
    ...
    def audit_relationships(self):
        ...
        projects_with_valid_sectors = Projeto.objects.filter(setor__isnull=False).count()
        print(f"Projetos com setor válido: {projects_with_valid_sectors}")
        
        # Verificar consistência de setores
        user_sectors = set(Usuario.objects.filter(setor__isnull=False).values_list('setor__nome', flat=True))
        project_sectors = set(Projeto.objects.filter(setor__isnull=False).values_list('setor__nome', flat=True))
    ...
    def audit_data_consistency(self):
        ...
        with connection.cursor() as cursor:
            ...
            if duplicate_cpfs:
                self.issues.append(f"CPFs duplicados encontrados: {len(duplicate_cpfs)}")
        ...
        with connection.cursor() as cursor:
            ...
            if duplicate_emails:
                self.issues.append(f"Emails duplicados encontrados: {len(duplicate_emails)}")
