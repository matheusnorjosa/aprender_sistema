...
class DashboardFiltersTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.tipo_evento = TipoEvento.objects.create(
            nome="Workshop Filtros", online=False
        )

        self.formador = Formador.objects.create(
            nome="João Filtros", email="joao.filtros@test.com", ativo=True
        )
    ...
    def test_filter_by_period_7_days(self):
        ...
        self.assertEqual(
            data["periodo_referencia"]["eventos_periodo"], "últimos 7 dias"
        )
    ...
    def test_filter_by_municipality(self):
        ...
        self.assertIn(
            "Município: Rio de Janeiro/RJ", data["filtros"]["filtros_aplicados"]
        )
