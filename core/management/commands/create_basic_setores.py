...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
                ...
                for setor_info in setores_data:
                    setor, created = Setor.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            'vinculado_superintendencia': setor_info['vinculado_superintendencia'],
                            'ativo': True,
                        }
                    )
                    ...
                    if created:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(f'✓ Criado: {setor.nome} ({setor.sigla}) - {status}')
                        )
                    else:
                        existing_count += 1
            ...
            self.stdout.write(
                self.style.ERROR(f'Erro ao criar setores: {e}')
            )
            raise
    ...
    def update_existing_projects(self):
        ...
        try:
            ...
            if projetos_sem_setor.exists():
                ...
                self.stdout.write(f'✓ {count} projetos vinculados ao setor Superintendência (padrão)')
                ...
                if projetos_sem_setor.count() > 5:
                    self.stdout.write(f'  ... e mais {projetos_sem_setor.count() - 5} projetos')
            else:
                self.stdout.write('• Todos os projetos já têm setor definido')
                
        except Setor.DoesNotExist:
            self.stdout.write(
                self.style.ERROR('Setor Superintendência não encontrado')
            )