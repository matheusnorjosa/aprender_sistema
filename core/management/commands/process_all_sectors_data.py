...
class Command(BaseCommand):
    ...
    def process_all_sectors(self):
        ...
        for planilha_file, setor_nome in sector_mappings.items():
            try:
                ...
                try:
                    ...
                    self.stdout.write(f'⚠ Arquivo {planilha_file} não encontrado - pulando')
                    continue
                ...
                for key in ['usuarios_criados', 'usuarios_atualizados', 'usuarios_vinculados_setor', 'usuarios_vinculados_grupo', 'erros']:
                    global_stats[key] += stats[key]
                
                # Mostrar resumo do setor
                self.stdout.write(f'Setor {setor_nome}: {stats["usuarios_criados"]} criados, {stats["usuarios_atualizados"]} atualizados')
        ...
        self.stdout.write(f'Usuários atualizados: {global_stats["usuarios_atualizados"]}')
        self.stdout.write(f'Usuários vinculados a setores: {global_stats["usuarios_vinculados_setor"]}')
        self.stdout.write(f'Usuários vinculados a grupos: {global_stats["usuarios_vinculados_grupo"]}')
    ...
    def process_sector_users(self, df, setor, cargo_map):
        ...
        for index, row in df.iterrows():
            try:
                ...
                nome_completo = str(row['Nome Completo']).strip() if pd.notna(row['Nome Completo']) else nome
                ...
                if created:
                    ...
                    if updated:
                        ...
                        self.stdout.write(f'⚠ Atualizado: {nome} ({email}) - {cargo_planilha}')
                    else:
    ...
    def show_final_summary(self):
        ...
        for setor in Setor.objects.all().order_by('nome'):
            ...
            status = "REQUER APROVAÇÃO" if setor.vinculado_superintendencia else "APROVAÇÃO DIRETA"
        ...
        usuarios_super = Usuario.objects.filter(setor__vinculado_superintendencia=True).count()
        usuarios_direto = Usuario.objects.filter(setor__vinculado_superintendencia=False).count()
