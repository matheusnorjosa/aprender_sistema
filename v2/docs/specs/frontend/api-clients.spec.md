...
- **`401` emite o evento global `auth:expired`** (`config.ts`, Issue #1376): `fetchAPI` faz `window.dispatchEvent(new Event('auth:expired'))` em **qualquer** `401`. O tratamento e unico e vive em `handleAuthExpired` (`App.tsx`), que so age se ja havia sessao (`userRef.current`) — o guard evita loop no load inicial e em rota publica. Telas nao devem tratar `401` isoladamente.
