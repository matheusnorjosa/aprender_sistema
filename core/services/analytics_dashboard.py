...
class EducationalAnalytics:
        ...
        try:
            ...
            formador_stats = (
                Solicitacao.objects.filter(
                    data_solicitacao__gte=start_date, status=SolicitacaoStatus.APROVADO
                )
                .values(
                    "formadores__usuario__first_name", "formadores__usuario__last_name"
                )
    ...
    def get_municipal_distribution_data(self) -> Dict[str, Any]:
        ...
        try:
            municipal_stats = (
                Solicitacao.objects.values("municipio__nome")
                .annotate(
    ...
    def get_project_timeline_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            timeline_data = (
                Solicitacao.objects.filter(data_solicitacao__gte=start_date)
    ...
    def create_formador_performance_chart(self, data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            fig.update_layout(
                ...
                yaxis2=dict(
                    title="Duração Média (horas)", overlaying="y", side="right"
                ),
                height=500,
                showlegend=True,
                hovermode="x unified",
    ...
    def generate_comprehensive_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not ("error" in formador_data):
                charts["formador_performance"] = self.create_formador_performance_chart(
                    formador_data
                )

            if not ("error" in municipal_data):
                charts["municipal_distribution"] = self.create_municipal_pie_chart(
                    municipal_data
                )
            ...
            stats = {
                ...
                "pending_solicitacoes": Solicitacao.objects.select_related(
                    "municipio", "projeto", "tipo_evento", "solicitante"
                )
                .prefetch_related("formadores")
                .filter(status=SolicitacaoStatus.PENDENTE)
                .count(),
                "approved_this_month": Solicitacao.objects.select_related(
                    "municipio", "projeto", "tipo_evento", "solicitante"
                )
                .prefetch_related("formadores")
                .filter(
...
class DashInteractiveApp:
    ...
    def _setup_layout(self):
        ...
        self.app.layout = html.Div(
            [
                html.H1(
                    "Aprender Sistema - Dashboard Analytics", className="header-title"
                ),
                html.Div(
                    [
                        html.Div(
                            [html.H3("Métricas Gerais"), html.Div(id="general-stats")],
                ...
                html.Div(
                    [dcc.Graph(id="project-heatmap")], className="full-width-chart"
                ),
                dcc.Interval(
