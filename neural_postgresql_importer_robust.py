                ...
                with transaction.atomic():
                    setor, created = Setor.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            "ativo": True,
                            "vinculado_superintendencia": "superintendência"
                            in setor_nome.lower(),
                        },
                    )
                ...
                with transaction.atomic():
                    ...
                    if pessoa_data.get("gerencia"):
                        try:
                            ...
                            logger.warning(
                                f"  ⚠️ Setor não encontrado: {pessoa_data['gerencia']}"
                            )
                    ...
                    if cpf:
                        try:
                            ...
                            usuario.first_name = self._extract_first_name(
                                pessoa_data["nome_completo"]
                            )
                            usuario.last_name = self._extract_last_name(
                                pessoa_data["nome_completo"]
                            )
                        ...
                        if email:
                            ...
                            logger.warning(
                                f"  ⚠️ Pessoa sem CPF nem email: {pessoa_data['nome_completo']}"
                            )
                            continue
                    ...
                    if created:
                        ...
                        logger.info(
                            f"  ✅ Pessoa criada: {pessoa_data['nome_completo']}"
                        )
                    else:
                        self.import_stats["pessoas_updated"] += 1
                        logger.info(
                            f"  🔄 Pessoa atualizada: {pessoa_data['nome_completo']}"
                        )
            ...
            try:
                with transaction.atomic():
                    municipio, created = Municipio.objects.get_or_create(
                        ...
                        defaults={"uf": municipio_data["uf"], "ativo": True},
                    )
                    ...
                    if created:
                        ...
                        logger.info(
                            f"  ℹ️ Município já existe: {municipio_data['nome']}"
                        )

            except Exception as e:
                error_msg = (
                    f"Erro ao criar município {municipio_data['nome']}: {str(e)}"
                )
                logger.error(f"  ❌ {error_msg}")
                self.import_stats["errors"].append(error_msg)
            ...
            try:
                with transaction.atomic():
                    ...
                    if created:
                        ...
                        logger.info(
                            f"  ℹ️ Tipo de evento já existe: {tipo_data['nome']}"
                        )

            except Exception as e:
                error_msg = (
                    f"Erro ao criar tipo de evento {tipo_data['nome']}: {str(e)}"
                )
                logger.error(f"  ❌ {error_msg}")
                self.import_stats["errors"].append(error_msg)
...
def main():
    ...
    try:
        ...
        print(
            f"📊 Total de pessoas importadas: {result['pessoas_created'] + result['pessoas_updated']}"
        )
