...
def analisar_pessoas_unicas_multiplos_papeis(gc):
    ...
    try:
        ...
        pessoas_unicas = defaultdict(lambda: {
            'coordenador_em': set(),
            'formador_em': set(),
            'gerente_em': set(),
            'total_projetos': 0,
                    ...
                    for linha in dados[1:]:
                        if len(linha) >= len(cabecalhos):
                            ...
                            for i, cabecalho in enumerate(cabecalhos):
                                ...
                                if valor:
                                    if 'projeto' in cabecalho.lower() and valor == projeto_nome:
                                        projeto_linha = valor
                            ...
                            if projeto_linha == projeto_nome:
                                ...
                                if coordenador_linha:
                                    nome_normalizado = normalizar_nome(coordenador_linha)
                                    pessoas_unicas[nome_normalizado]['coordenador_em'].add(f"{categoria} - {projeto_nome}")
                                    pessoas_unicas[nome_normalizado]['total_projetos'] += 1
                                    pessoas_unicas[nome_normalizado]['total_registros'] += 1
                                
                                # Formadores
                                for formador in formadores_linha:
                                    if formador:
                                        nome_normalizado = normalizar_nome(formador)
                                        pessoas_unicas[nome_normalizado]['formador_em'].add(f"{categoria} - {projeto_nome}")
                                        pessoas_unicas[nome_normalizado]['total_projetos'] += 1
                                        pessoas_unicas[nome_normalizado]['total_registros'] += 1
                                ...
                                if gerente_linha:
                                    ...
                                    pessoas_unicas[nome_normalizado]['gerente_em'].add(f"{categoria} - {projeto_nome}")
                                    pessoas_unicas[nome_normalizado]['total_projetos'] += 1
                                    pessoas_unicas[nome_normalizado]['total_registros'] += 1
        ...
        for pessoa, dados in pessoas_unicas.items():
            ...
            total_papeis = len(dados['coordenador_em']) + len(dados['formador_em']) + len(dados['gerente_em'])
            if total_papeis > 1:
        ...
        for pessoa, dados, total_papeis in pessoas_multiplos_papeis:
            ...
            if dados['coordenador_em']:
                print(f"   👨‍💼 COORDENADOR EM ({len(dados['coordenador_em'])} projetos):")
        ...
        print(f"   📊 Pessoas com apenas 1 papel: {len(pessoas_unicas) - len(pessoas_multiplos_papeis) - 1}")  # -1 para "SOLICITADO"
        
        # Top 10 pessoas com mais papéis
        print(f"\n🏆 TOP 10 PESSOAS COM MAIS PAPÉIS:")
        for i, (pessoa, dados, total_papeis) in enumerate(pessoas_multiplos_papeis[:10], 1):
                ...
                for i, cabecalho in enumerate(cabecalhos):
                    ...
                    if valor:
                        if 'projeto' in cabecalho.lower():
                            projeto = valor
                        elif 'coordenador' in cabecalho.lower() and 'ana paula' in valor.lower():
                            coordenador = valor
...
def main():
    ...
    # Executar análises
    pessoas_unicas, pessoas_multiplos_papeis = analisar_pessoas_unicas_multiplos_papeis(gc)
