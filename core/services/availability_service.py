...
from core.models import (
    Solicitacao, Formador, Municipio, DisponibilidadeFormadores,
    LogAuditoria, TipoEvento, Projeto
)
from core.services.disponibilidade_engine import (
    DisponibilidadeEngine, AvailabilityCheck, ConflictInfo,
...
class AvailabilityService:
    ...
    def validate_solicitacao(
        ...
        if has_critical:
            can_request = False
            warnings.insert(0, "❌ Solicitação não pode ser criada devido a conflitos críticos")
        elif check_result.conflicts:
            warnings.insert(0, "⚠️ Solicitação será criada com ressalvas - requer aprovação cuidadosa")
        
        # Auditoria
        if user:
            self._log_validation_attempt(
                user, check_result, data_inicio, data_fim, 
                formadores, municipio, can_request
            )
    ...
    def create_solicitacao_with_validation(
        ...
        try:
            ...
            success_msg = f"✅ Solicitação criada com sucesso para {len(formadores)} formador(es)"
    ...
    def get_availability_matrix(
        ...
        end_date = timezone.make_aware(
            datetime(ano, mes, last_day, 17, 0),
            timezone.get_current_timezone()
        )
        ...
        for formador in formadores:
            ...
            for day in range(1, last_day + 1):
                ...
                # Eventos do dia
                day_events = [
                    e for e in events_data.get(formador.id, [])
                    if e.data_inicio.date() == day_date
                ]
                
                # Bloqueios do dia
                day_blocks = [
                    b for b in blocks_data.get(formador.id, [])
                ...
                if len(day_events) >= 2:
                    ...
                    for i, event1 in enumerate(day_events):
                        for event2 in day_events[i+1:]:
                            if (event1.data_inicio < event2.data_fim and 
                                event2.data_inicio < event1.data_fim):
                                code = 'X'
                                color = 'red'
                                break
                        if code == 'X':
        ...
        for day_offset in range(1, days_ahead + 1):
            ...
            for hour in range(8, 17):  # 8h às 17h
                slot_start = timezone.make_aware(
                    datetime.combine(check_date, datetime.min.time().replace(hour=hour)),
                    timezone.get_current_timezone()
                )
                ...
                if check_result.available:
                    ...
                    conflicted_formadores = set(c.formador.id for c in check_result.conflicts)
    ...
    def _get_period_events(self, formadores: QuerySet, start: datetime, end: datetime) -> Dict[int, List]:
        ...
        events = Solicitacao.objects.filter(
            ...
            data_fim__gte=start
        ).select_related('municipio').prefetch_related('formadores')
    ...
    def _get_period_blocks(self, formadores: QuerySet, start: datetime, end: datetime) -> Dict[int, List]:
