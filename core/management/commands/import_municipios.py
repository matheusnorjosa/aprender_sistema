...
from core.models import Municipio, LogAuditoria
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            user = None
            self.stdout.write(
                self.style.WARNING(f'Usuário "{username}" não encontrado. Auditoria será registrada sem usuário.')
            )
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING('MODO DRY-RUN: Nenhuma alteração será salva no banco'))
        ...
        try:
            with open(arquivo, 'r', encoding=encoding, newline='') as csvfile:
                ...
                for row_num, row in enumerate(reader, 1):
                    ...
                    nome = self.extract_field(normalized_row, ['nome', 'municipio', 'cidade', 'nome_municipio'])
                    uf = self.extract_field(normalized_row, ['uf', 'estado', 'sigla', 'sigla_estado'])
                    ativo_str = self.extract_field(normalized_row, ['ativo', 'status', 'ativo_inativo'], 'True')
                    
                    if not nome:
                        self.stdout.write(self.style.WARNING(f'Linha {row_num}: Nome do município não encontrado. Pulando.'))
                        continue
        ...
        try:
            with open(arquivo, 'r', encoding='utf-8') as jsonfile:
                ...
                if isinstance(data, list):
                    ...
                    raise CommandError('Formato JSON inválido. Esperado lista ou {municipios: [...]}')
    ...
    def load_from_excel(self, arquivo):
        ...
        try:
            ...
            raise CommandError('openpyxl não está instalado. Execute: pip install openpyxl')
    ...
    def display_result(self, resultado):
        ...
        if sucesso > 0 and resultado["erros"] == 0:
            ...
            self.stdout.write(self.style.WARNING('Importação concluída com alguns erros'))
        else:
