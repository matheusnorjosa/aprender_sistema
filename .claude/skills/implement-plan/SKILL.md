---
name: implement-plan
description: Implement an approved plan wave-by-wave with verification gates. Use when a plan path is given, when resuming after /clear, or when create-plan output is ready to build.
---

# Implement Plan

Implement an approved technical plan. Plans live in `thoughts/shared/plans/`
(where `create-plan` writes) or `v2/docs/plans/` (project-canonical for new
...
8. **Transition.** Commit if the wave is independently valuable (CP-06). If
   context > 70%, save the continuity ledger (`continuity-ledger` skill) and
