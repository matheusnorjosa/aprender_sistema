...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                f"📊 Esperado: {aprovados_esperados} APROVADO, {pendentes_esperados} PENDENTE"
            )
            ...
            if not self.dry_run:
                with transaction.atomic():
                    ...
                    ).prefetch_related("formadores").update(
                        status=SolicitacaoStatus.APROVADO
                    )

                    # Marcar os últimos X% como PENDENTE para atingir a proporção
                    proporção_pendente = pendentes_esperados / (
                        aprovados_esperados + pendentes_esperados
                    )
                    num_pendentes = int(total_solicitacoes * proporção_pendente)

                    # Marcar as últimas N solicitações como PENDENTE
                    solicitacoes_pendentes = Solicitacao.objects.order_by("-id")[
                        :num_pendentes
                    ]
                    ids_pendentes = [s.id for s in solicitacoes_pendentes]
                    Solicitacao.objects.select_related(
                        "municipio", "projeto", "tipo_evento", "solicitante"
                    ).prefetch_related("formadores").filter(
                        id__in=ids_pendentes
                    ).update(
                ...
                self.stdout.write(
                    "🔍 Esta operação atualizaria todos os status proporcionalmente"
                )
