    ...
    def set(self, key: str, value: Any, timeout: Union[int, str] = "medium") -> bool:
        ...
        try:
            ...
            if isinstance(timeout, str):
                timeout = self.CACHE_TIMEOUTS.get(
                    timeout, self.CACHE_TIMEOUTS["medium"]
                )
    ...
    def clear_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            if hasattr(cache, "delete_pattern"):
                ...
                logger.info(
                    f"Cache CLEAR pattern '{pattern}': {deleted_count} keys deleted"
                )
...
def cached_property_method(
    timeout: Union[int, str] = "short", key_prefix: str = "property"
):
    """
...
def cache_formador_availability(
    formador_id: int, start_date: str, end_date: str, data: Dict
):
    """Cache formador availability data"""
    key = cache_service.generate_key(
        "availability", "formador", formador_id, start_date, end_date
    )
    cache_service.set(key, data, "medium")


def get_cached_formador_availability(
    formador_id: int, start_date: str, end_date: str
) -> Optional[Dict]:
