...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        expected_groups = ['admin', 'superintendencia', 'coordenador', 'formador', 'controle', 'diretoria']
        missing_groups = []
        ...
        if missing_groups and not force:
            self.stdout.write(
                self.style.ERROR(f'ERRO: Os seguintes grupos não existem: {", ".join(missing_groups)}')
            )
            self.stdout.write(
                self.style.WARNING('Execute a migration 0002_setup_groups_and_permissions primeiro, ou use --force para prosseguir.')
            )
            return
