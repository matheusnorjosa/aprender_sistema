        ...
        for aba in abas:
            ...
            try:
                ...
                for linha in dados_aba[1:]:  # Pular cabeçalho
                    if any(
                        celula.strip() for celula in linha
                    ):  # Se tem pelo menos uma célula não vazia
                        registros_validos += 1
                ...
                dados_aba_info = {
                    ...
                    "amostra_dados": (
                        dados_aba[:5] if len(dados_aba) > 5 else dados_aba
                    ),  # Primeiras 5 linhas
                }
...
def detectar_tipos_dados(dados):
    ...
    for i, cabecalho in enumerate(cabecalhos):
        ...
        # Verificar se são CPFs
            ...
            len(v.replace(".", "").replace("-", "")) == 11
            and v.replace(".", "").replace("-", "").isdigit()
            for v in valores_nao_vazios[:5]
        ):
            tipo = "cpf"
        ...
        # Verificar se são números
            ...
            for v in valores_nao_vazios[:10]
        ):
            tipo = "numero"
...
def main():
                        ...
                        if i < len(aba["cabecalhos"]):
                            ...
                            if valor.strip():
                                ...
                                # Detectar CPFs
                                    ...
                                    len(valor.replace(".", "").replace("-", "")) == 11
                                    and valor.replace(".", "")
                                    .replace("-", "")
                                    .isdigit()
                                ):
                                    total_cpfs_unicos.add(
                                        valor.replace(".", "").replace("-", "")
                                    )
                                # Detectar nomes (heurística simples)
                                elif any(
                                    palavra in cabecalho.lower()
                                    for palavra in ["nome", "name", "usuario", "user"]
                                ):
    ...
    for nome, dados in mapeamento_completo["planilhas"].items():
        ...
        for aba in dados["abas"]:
            print(
                f"      📄 {aba['nome']}: {aba['registros_validos']} registros, {aba['colunas']} colunas"
            )
