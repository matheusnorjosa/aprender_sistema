...
from core.models import Projeto, LogAuditoria
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            # Processar dados
            resultado = self.process_projetos(projetos_data, dry_run, user, update_existing)
        ...
        try:
            with open(arquivo, 'r', encoding=encoding, newline='') as csvfile:
                ...
                for row_num, row in enumerate(reader, 1):
                    ...
                    nome = self.extract_field(normalized_row, ['nome', 'projeto', 'nome_projeto', 'title'])
                    descricao = self.extract_field(normalized_row, ['descricao', 'description', 'desc', 'detalhes'])
                    ativo_str = self.extract_field(normalized_row, ['ativo', 'status', 'ativo_inativo'], 'True')
                    vinculado_str = self.extract_field(normalized_row, ['vinculado_superintendencia', 'vinculado', 'superintendencia'], 'False')
                    
                    if not nome:
                        self.stdout.write(self.style.WARNING(f'Linha {row_num}: Nome do projeto nao encontrado. Pulando.'))
                        continue
        ...
        try:
            with open(arquivo, 'r', encoding='utf-8') as jsonfile:
                ...
                if isinstance(data, list):
                    ...
                    raise CommandError('Formato JSON invalido. Esperado lista ou {projetos: [...]}')
                ...
                for item in projetos_data:
                    projetos.append({
                        ...
                        'vinculado_superintendencia': self.parse_boolean(item.get('vinculado_superintendencia', False))
                    })
    ...
    def load_from_excel(self, arquivo):
        ...
        try:
            ...
            for row_num, row in enumerate(sheet.iter_rows(min_row=2), 2):
                ...
                descricao = self.extract_field(row_data, ['descricao', 'description', 'desc'])
                ativo_str = self.extract_field(row_data, ['ativo', 'status'], 'True')
                vinculado_str = self.extract_field(row_data, ['vinculado_superintendencia', 'vinculado'], 'False')
                    ...
                    if created:
                        ...
                        if update_existing:
                            ...
                            if projeto.ativo != ativo:
                                projeto.ativo = ativo
                                updated = True
                            if projeto.vinculado_superintendencia != vinculado_superintendencia:
                                projeto.vinculado_superintendencia = vinculado_superintendencia
                                updated = True
