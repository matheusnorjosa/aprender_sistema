...
class Command(BaseCommand):
    ...
    def preservar_admin_essencial(self):
        ...
        if (
            not Usuario.objects.select_related("municipio", "projeto")
            .prefetch_related("groups", "user_permissions")
    ...
    def verificar_dados_reais_restantes(self):
        ...
        if formadores_reais:
            self.logger.info(
                f"👥 Exemplo de formadores reais: {list(formadores_reais)}"
            )
    ...
    def handle(self, *args, **options):
        ...
        if not options["confirmar"]:
            ...
            self.logger.error(
                "⚠️  Use --confirmar para confirmar a limpeza dos dados de teste"
            )
        ...
        self.logger.warning(
            "⚠️  ATENÇÃO: Esta operação removerá TODOS os dados de teste!"
        )
        ...
        try:
            ...
            self.logger.info(
                "✅ LIMPEZA CONCLUÍDA - Sistema agora contém apenas dados reais!"
            )
            self.logger.info("=" * 80)
