...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if calendar_id:
            ...
            self.stdout.write(self.style.WARNING(
                "Configure GOOGLE_CALENDAR_CALENDAR_ID no settings"
            ))
                ...
                try:
                    ...
                    with open(credentials_path, 'r') as f:
                        ...
                        if 'client_email' in creds_data:
                            ...
                            self.stdout.write("⚠️ Arquivo não parece ser service account")
                except json.JSONDecodeError:
                    self.stdout.write("ERRO Arquivo de credenciais inválido (JSON malformado)")
            ...
            try:
                ...
                if calendar_id:
                    ...
                    if test_event_id:
                        ...
                        self.stdout.write("🎉 Teste completo de criação/exclusão bem-sucedido!")
                    else:
                ...
                if "quota" in str(e).lower():
                    self.stdout.write("💡 Dica: Verifique se você não excedeu a quota da API")
        ...
        if options.get('list_events') is not None or options.get('list_events'):
            ...
            self._list_calendar_events(calendar_id, count, sync_enabled, credentials_path)
    ...
    def _check_conflicts(self, calendar_id, sync_enabled, credentials_path):
        ...
        try:
            from core.models import Solicitacao, SolicitacaoStatus, EventoGoogleCalendar
            from datetime import datetime, timedelta
            
            # Solicitações aprovadas sem evento no calendar
            aprovadas_sem_evento = Solicitacao.objects.filter(
                status=SolicitacaoStatus.PRE_AGENDA,
                data_inicio__gte=datetime.now() - timedelta(days=7)
            ).exclude(
                id__in=EventoGoogleCalendar.objects.values_list('solicitacao_id', flat=True)
            ).count()
            ...
            self.stdout.write(f"📊 Solicitações aprovadas sem evento no Calendar: {aprovadas_sem_evento}")
            self.stdout.write(f"📊 Eventos no Calendar sem solicitação: {eventos_sem_solicitacao}")
    ...
    def _show_sync_stats(self):
        ...
        try:
            from core.models import (
                Solicitacao, SolicitacaoStatus, EventoGoogleCalendar, 
                LogAuditoria
            )
            ...
            recent_approvals = LogAuditoria.objects.filter(
                data_hora__gte=last_30_days,
                acao__icontains='RF04'
            ).count()
    ...
    def _validate_quota(self, calendar_id, sync_enabled, credentials_path):
        ...
        try:
            ...
            if "quota" in str(e).lower() or "limit" in str(e).lower():
                self.stdout.write("🚫 Quota da API excedida - aguarde ou aumente limites")